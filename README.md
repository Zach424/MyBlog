...
- 发布状态：Vercel 原生 Next.js、GitHub `main` 自动部署、Studio editorial workflow 与只读内容复核队列、Obsidian `--push`、内部链接/标题锚点门禁、反向引用、公开知识地图、RSS 与 JSON Feed 1.1、可供 Obsidian/自动化发现的 `/content.json` 清单、带内容 ETag/条件请求的文章/项目可移植 Markdown 源文、详情页规范链接分享与 Markdown 引用复制、可解释的 raw/gzip HTML 双层预算、版本化永久重定向、生产冒烟、回滚恢复与六处官方 Actions 不可变 SHA 门禁均已验收
...
MyBlog Publisher 当前版本为 1.35.0。Vercel 部署完成后，在 Obsidian 命令面板运行“检查生产内容同步状态”，即可把本地正式内容与真实 `/content.json` 比较为已上线、待部署、生产缺失或生产多出；网络/协议错误单独失败，不会改文章、提交、推送或自动重试。命令行可运行 `npm run content:production`。
