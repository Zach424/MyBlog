...
MyBlog Publisher 当前版本为 1.36.0。推送正式内容后，在 Obsidian 中打开对应的 `content/posts|projects/<slug>.md`，运行“等待当前正式内容上线”，即可冻结该文件的原始 SHA-256 与本地公开 Markdown ETag，并以条件 GET 在三分钟有界时限内等待稳定生产站收敛；新一轮会取消旧轮，关闭插件也会取消，来源漂移、协议错误或生产多出会立即失败，整个过程零写入、零提交、零推送。命令行等价入口为 `npm run content:production:wait -- --source content/projects/myblog.md`。全库一次性核对仍使用“检查生产内容同步状态”或 `npm run content:production`。
