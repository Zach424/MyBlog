...
| Obsidian 写作 | done | Vault、三类受信模板、桌面插件 1.32.0、文件名唯一草稿身份、本地原子新建/安全改名/旧身份取证与严格清理、source-scoped 当前草稿作者意图、原始来源字节 SHA-256 摘要/导航双重绑定、命令到异步复核的 latest-wins generation 与卸载失效、媒体 COVER/BODY 来源/逐次替代文本/作者或文件名回退取证/精确变换、ALT 与 LINK occurrence 精确源码行导航、四个新发布/复核事务的 single-flight lease、阶段/输出活动脉冲、会话内最近终态回执与自动 doctor 联锁、13 项本机前置电路、统一只读 Git 交付分诊、版本化维护台账/Author Proof v3、两类安全重送/可信回执、deferred 并行草稿和新稿 `--check-only`/`--push` |
...
- 本轮实现提交：`aecea1d050230caef3667716d45ee1e08819db1c`（当前草稿作者意图命令到异步摘要复核的 latest-wins generation）；
- 自动交付：实现提交已推送；[Quality Gate #145](https://github.com/Zach424/MyBlog/actions/runs/31075902284) 与 [Verify Vercel production #138](https://github.com/Zach424/MyBlog/actions/runs/31075933884) 均成功，归档提交继续按同一链路独立验证；
- 最新完成迭代：0081 当前草稿作者意图 latest-wins 生命周期；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md` 与 `docs/iterations/*.md` 可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

MyBlog Publisher 1.32.0 为每次真正运行“查看当前草稿发布意图”创建一个冻结 generation token；新的运行立即替换旧 token，插件卸载则置空。子进程 close/error 在隐藏自身进度并清理 active run 后，只有当前 generation 才能解释 JSON、发送失败或继续异步 `vault.read`；读取开始、异常和 await 恢复后再次验证 token，旧完成项因此不能打开 Modal、发送成功/失败 Notice 或自动重试。当前 generation 继续执行路径、`TFile`、SHA-256、行界和视图守卫。没有视觉或发布动作变化。失败优先 31/204，定向回归 205/205、完整 366/366、45 页构建、19/19 应用测试和生产依赖审计 0 均通过；真实 doctor 13/13 ready、插件 1.32.0，真实 inbox v6 为空且全部安全声明为 false。
...
4. Current record 已有 Studio、Obsidian 与每周 Actions 三个只读视图；Obsidian 1.32.0 已闭合本地草稿创建、旧身份取证/严格清理、文件名改名、source-scoped 当前草稿作者意图、媒体用途/逐次 alt/来源/变换、ALT/LINK occurrence 源码行导航、两阶段来源 SHA-256 绑定和意图检查 latest-wins/unload 生命周期，并继续保留四事务 owner-checked lease、活动脉冲、会话内终态回执、Author Proof v3、统一 Git 交付 switchyard、两类独立交付状态、精确重送与可信回执。作者意图页已有 DOM/CSS、键盘焦点、跨 ALT/REF 的重复点击单航班、ready/scheduled/blocked、媒体与链接证据，以及 stale success/failure/error、路径、`TFile`、摘要、异步读取、卸载、磁盘/Editor 行界、视图和打开失败测试，但没有真实 Obsidian 宿主像素/交互快照。摘要只是本地新鲜度证据而非签名；被替代的只读子进程会静默完成而不再报告，但当前不会主动终止，其进度 Notice 在进程终态前仍存在。相邻的“检查当前草稿身份”也会异步读取后打开 Modal，却尚无 repeated-run/unload generation，是下一轮更基础的同类生命周期缺口。聚焦模式轻量解析全部草稿并读取全部已发布内容作为链接目标，是保留共享附件和正式链接正确性的明确成本；带注释、引号、anchor/tag、缩进、重复键或不匹配值的旧 slug 故意保持只读；
...
下一轮唯一主任务：给“检查当前草稿身份”的异步 `vault.read` 与 Modal 打开链增加独立 latest-wins generation，并在插件卸载时失效；重复运行只允许最新读取显示证据，旧读取成功或失败均保持静默。不得改变 `Vault.process` 严格清理 lease、正文、发布事务、Git、网络或云 API。
