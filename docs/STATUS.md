...
| Obsidian 写作 | done | Vault、模板、桌面插件 1.5.0、版本化维护台账与 Author Proof、正式内容精确同步、收件箱总览和新稿 `--check-only`/`--push` |
...
- 发布：Decap CMS 3.14.1、GitHub OAuth、stable slug 自定义控件、同源媒体清单、内存会话账本、per-input generation 与 SHA-256 冲突确认、官方 preview template、同源只读公式/全字段预检端点、public-only 内容维护快照与严格浏览器契约、内容字段 allowlist、Obsidian 自有插件 1.5.0、版本化维护 deadline ledger/复核 Author Proof/inbox 只读 Modal、精确 Vault 笔记打开、review-note 领域/Git 双门、两条纯文本降级、统一子进程生命周期与 Node 发布脚本；
...
- 本轮实现提交：`8d4c7346ae85a3956ebd7d3710813c4261bbbf03`（版本化 Author Proof、质量日志隔离、严格插件 schema、复核迁移轨和纯文本降级）；
- 自动交付：本轮实现 Quality Gate `30988816791`（#95）与 Vercel Production 验证 `30988856799`（#88）均 completed/success；
- 最新完成迭代：0054 Obsidian 结构化正式内容 Author Proof；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md` 与 `docs/iterations/*.md` 可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

MyBlog Publisher 1.5.0 为 check-only 增加结构化 Author Proof。CLI 只在完整 `npm run check` 通过后输出 `version: 1` 的 review/git/qualityGate：标题和稳定身份、HEAD/当前 reviewedAt、前后 updatedAt、substantiveChanged、main、唯一 changed/committable、空 staged/untracked 和 passed。JSON 模式以 5,000,000 字符包络捕获质量输出；250,000 字符夹具证明 stdout 仍只有小于 5,000 字符的 Proof。插件 exact-validate 全部关系及活动路径后，显示无按钮的日期迁移轨与规则线证据账本，并明确尚未暂存/提交/推送；JSON/schema/path/UI 异常会重新执行纯文本 check-only。同步命令、Git 双门和失败恢复保持 1.4.0 语义。最终门为 176/176 单元、45 条构建页面、19/19 生产应用测试与 audit 0；公开静态资源仍为 1,818,133 B，插件 main/manifest/styles 合计 43,548 B且不进入公开客户端。
...
4. Current record 已有 Studio、Obsidian 与每周 Actions 三个只读视图；Obsidian 1.5.0 已显示严格 Author Proof，并保持同步独立。同日仍不能因日期精度重复声明复核，push 失败会保留合法本地提交；额外 inbox 草稿目前会被零 untracked 门阻断。真实主题组合与大队列仍由 DOM/CSS 契约而非宿主像素验收覆盖，插件更新后必须重启或重新启用；
...
下一轮唯一主任务：给正式复核增加共享 worktree impact classifier，支持作者同时保留安全的 deferred work。精确识别 `content/inbox/<slug>.md` 与根 `public/uploads/<file>` 草稿附件，证明它们不会进入正式内容 build/commit 后允许保留，并在 Proof 中显式列出；任何 staged、其他正式内容、代码、配置、嵌套已归档媒体或未知路径继续阻断。完整门前后必须复算分类，`git commit --only` 和唯一 tree 不变；不得用笼统 ignore 放宽可复现性。
