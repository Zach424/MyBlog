...
| Obsidian 写作 | done | Vault、三类受信模板、桌面插件 1.25.0、文件名唯一草稿身份、本地原子新建/安全改名/旧身份取证与严格清理、source-scoped 当前草稿作者意图、精确媒体变换与链接 trace、四个新发布/复核事务的 single-flight lease、阶段/输出活动脉冲、会话内最近终态回执与自动 doctor 联锁、13 项本机前置电路、统一只读 Git 交付分诊、版本化维护台账/Author Proof v3、两类安全重送/可信回执、deferred 并行草稿和新稿 `--check-only`/`--push` |
| Inbox 发布就绪 | done | version 2/read-only 全草稿 ready/scheduled/blocked、Article/TIL/Project、精确站内目标/源码行/重复次数、真实媒体候选、目标/共享附件诊断、CLI 全库或 `--source` 聚焦 JSON 与 Obsidian 当前草稿原生摘要 |
...
- 本轮实现提交：`f26fe0f4adb732cf5ace7e3b523fa1993627f654`（当前草稿精确媒体变换 trace）；
- 自动交付：实现提交已推送；远端 Quality、Vercel 验证与生产状态仍由 GitHub/Vercel 按提交独立报告；
- 最新完成迭代：0074 当前草稿精确媒体变换轨迹；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md` 与 `docs/iterations/*.md` 可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

MyBlog Publisher 1.25.0 把 `MEDIA` 计数背后的 readiness 媒体准备包络提升为只读 `MEDIA TRACE`：每个附件显示 source、repository target、public URL、输入/输出格式、宽高、帧数、字节和 optimized/preserved/unproven 状态。插件不读图也不复制媒体策略，而是严格交叉检查路径、扩展名、字节差、保留稳定性、静态 WebP 优化与不放大约束；ready/scheduled 缺少 preparation 或 blocked 缺少同来源问题时失败关闭。160/160 定向回归、完整 321/321、45 页构建、19/19 应用测试与生产依赖审计 0 均通过；真实 inbox 为空且 version 2 安全声明保持全 false。
...
4. Current record 已有 Studio、Obsidian 与每周 Actions 三个只读视图；Obsidian 1.25.0 已闭合本地草稿创建、旧身份取证/严格清理、文件名改名、source-scoped 当前草稿作者意图、精确媒体变换和链接 trace，并继续保留四事务 owner-checked lease、活动脉冲、会话内终态回执、Author Proof v3、统一 Git 交付 switchyard、两类独立交付状态、精确重送与可信回执。作者意图页已有 DOM/CSS、ready/scheduled/blocked、optimized/preserved/unproven 媒体包络、文章/项目/自链接、重复次数、源码行、schema/safety/计数漂移、单 entry 和活动文件漂移测试，但没有真实 Obsidian 宿主像素快照；媒体仍未显示 COVER/BODY 角色与来源行。聚焦模式仍轻量解析全部草稿并读取全部已发布内容作为链接目标，这是保留共享附件和正式链接正确性的明确成本，不再对无关附件生成媒体候选；带注释、引号、anchor/tag、缩进、重复键或不匹配值的旧 slug 故意保持只读；
...
下一轮唯一主任务：为 `MEDIA TRACE` 补充每个附件在当前草稿中的 `COVER / BODY` 角色、出现次数与全部源码行，直接在现有 cover/附件规范化过程或共享 Markdown AST 中产出；不得重新读取图片、在插件中实现第二套媒体解析、改变发布边界、引入云服务/真实 API、自动发布或自动修复。
