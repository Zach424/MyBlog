...
| Markdown | done | GFM、代码高亮、语言标签、渐进增强的一键复制、与实际渲染一致的 H1–H6 heading id、H2/H3 目录与原生永久链接、A4 打印/PDF 版式、阅读时间、相邻文章与响应式正文图片 |
...
- 阅读：react-markdown、remark-gfm、rehype-slug、rehype-highlight；服务端 Markdown、MarkdownHeading 永久链接与 PrintSource 可信来源，最小 CodeBlock 客户端岛、Clipboard API 与 aria-live；H2/H3 直接使用 renderer id，GFM mdast 与 GitHubSlugger 继续复现同一标题和链接语义；A4 `@page` 与 scoped print CSS 只重排既有语义 DOM；
...
- 本轮实现提交：`df671ec`（文章/项目服务端打印来源、A4 详情页纸面布局、分页与长 URL/代码/表格/引用契约）；
- 自动交付：Quality Gate `30961145805`、Production verification `30961173906` 均成功；GitHub Production deployment `5753354034` 精确对应实现 SHA 且状态为 success，稳定生产域名保持公开；
- 最新完成迭代：0044 可打印技术内容；
...
文章与项目详情页现在具有独立的 A4 技术记录版式。服务器把可信 canonical URL 输出为仅打印可见来源；纸面保留完整标题、摘要、标签、五列事实、封面、正文、代码、图片、表格、外链地址和必要引用，隐藏站点头尾、面包屑、目录、邻接、复制按钮与 permalink。分页规则避免标题孤立、代码/图片/表格行被随意切断；代码改为浅色且自动折行，引用账本压缩为标题和相对路径。屏幕版、无 JavaScript 能力和客户端 bundle 均未改变。完整门禁为 126/126 单元测试、37 个构建页面和 17/17 HTTP 测试；稳定域名 24 路由、OAuth 302；生产 Edge print media 计算样式正确且控制台 0 error/0 warning；最终文章 3 页、项目 5 页，8 页 PDF 全部渲染复核通过。

## 风险与下一步

1. Studio 已完成真实格式/预算、生产/会话摘要和快速重选竞态边界，但有意不自动缩放或转 WebP；读者侧代码复制、章节 permalink 与 A4 打印/PDF 已闭环。下一直接缺口是 Obsidian 常用的 `[^id]` 脚注还不能进入网页语义：需要选择与现有 Markdown AST/renderer 兼容的解析链，保证编号、脚注区、回链、可访问名称、打印分页和内部关系抽取不把脚注定义误判为独立导航；
...
下一轮唯一主任务：支持 Obsidian 兼容的 Markdown 脚注/尾注。作者应能使用 `[^id]` 与对应定义记录证据来源，网页输出语义化编号、脚注区和可返回正文的链接；同一脚注多次引用、中文 id、脚注内链接与代码、无 JavaScript、键盘/屏幕阅读器、打印分页和深浅色都要有明确契约。实现不得让脚注定义污染搜索摘要、目录或内容关系，也不能引入外部服务；先核对当前 unified/remark 官方能力与依赖边界，再以真实 Obsidian 草稿和生产 HTML/PDF 验证。
