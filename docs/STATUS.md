...
| 内容知识网络 | pending | 内部文章链接、反向链接和相关知识导航尚未实现 |
...
Obsidian 粘贴图片现在可以保留作者的自然写作动作：发布前检查能展示归档计划；正式发布会把图片移动到 `public/uploads/<内容 slug>/`，把空格/中文文件名稳定化并重写正文链接。若完整质量门失败，草稿和附件一起回滚。与此同时，生产 HTTP 测试不再硬编码初始内容日期和条目数，而是验证首页最新日期一致性以及 RSS/Sitemap 内容集合一致性，因此真实内容增长不会再被测试误判。

## 风险与下一步

1. 图片仍以原始体积进入 Git，缺少大小预算与压缩；
2. Obsidian 内部笔记链接还不能自动转换为站点 URL，公开文章之间也没有反向链接；
3. Studio OAuth origin、GitHub 凭据和 Vercel Hobby 回滚范围仍需按运行手册维护；
4. 统计、评论和自定义域名需要所有者最终选择，现阶段不主动接入。
5. `decap-cms` 的开发依赖树仍有上游无修复的高危审计项；它不进入公开服务端生产依赖，但其浏览器编辑器包仅对已授权作者开放，后续应单独评估升级或替代方案。

下一轮唯一主任务：实现 Obsidian 内部文章/项目链接到稳定站点 URL 的转换，并在详情页生成反向链接，形成第一版知识网络。
