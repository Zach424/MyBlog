...
| Obsidian 写作 | done | Vault、模板、桌面插件 1.15.0、四个新发布/复核事务的 single-flight lease 与自动 doctor 联锁、13 项本机前置电路、统一只读 Git 交付分诊、版本化维护台账/Author Proof v3、两类安全重送/可信回执、deferred 并行草稿和新稿 `--check-only`/`--push` |
...
- 发布：Decap CMS 3.14.1、GitHub OAuth、stable slug 自定义控件、同源媒体清单、内存会话账本、per-input generation 与 SHA-256 冲突确认、官方 preview template、同源只读公式/全字段预检端点、public-only 内容维护快照与严格浏览器契约、内容字段 allowlist、Obsidian 自有插件 1.15.0、四事务 single-flight lease/自动 author-doctor interlock/version 1 的 13 项 preflight circuit、单快照 Git 交付 switchyard、版本化维护 deadline ledger/复核 Author Proof v3/两类本地交付 rail 与 sealed receipt/发布 Commit Envelope/inbox 只读 Modal、候选 SHA-256 与 Git-clean blob 绑定、两类待交付提交识别和独立安全重送、精确 OID refspec、防重复发布、manifest 稳定性、共享 worktree impact classifier、deferred 路径证据、review-note/publish-note 领域与 Git 交付门、六条纯文本降级、统一子进程生命周期与 Node 发布脚本；
...
- 本轮实现提交：`2ce4380cbc70ec3efe3c1765ff66e0ca37705a6d`（四个 Obsidian 作者事务的 single-flight lease）；
- 自动交付：实现提交已推送；两条 GitHub Actions check 与 Vercel commit status 共 3/3 success，均绑定实现提交 SHA；
- 最新完成迭代：0064 作者事务 single-flight lease；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md` 与 `docs/iterations/*.md` 可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

MyBlog Publisher 1.15.0 为四个新作者事务增加一个 owner-checked single-flight lease。租约在 doctor spawn 前占用，跨越 ready continuation、publish/review 领域命令、author-doctor 纯文本降级和 review Proof 纯文本降级；只有当前 `lease.child` 能释放。并发调用只显示 `AUTHOR TRANSACTION / BUSY`、原操作与冻结 sourcePath，不排队、不启动第二个进程链。成功、非零退出、同步/异步 spawn error、回调异常与插件卸载都会释放或作废；旧 close/error 不能清除后来 owner。显式 doctor、只读报告、统一分诊、复核/发布状态与两类 deliver 有意绕过租约。真实仓库为 13/13 ready、32/32 依赖匹配；定向测试 68/68，完整门为 234/234 单元与集成、45 条构建页面、19/19 生产应用测试、依赖审计 0。
...
4. Current record 已有 Studio、Obsidian 与每周 Actions 三个只读视图；Obsidian 1.15.0 已为四个新发布/复核事务增加 owner-checked single-flight lease，并保留严格 Author Proof v3、统一 Git 交付 switchyard、两类独立交付状态、精确重送与可信回执。并发事务不再各自启动质量门，但租约当前只保存 operation/sourcePath/owning child，长任务的 phase、开始时间和 elapsed 尚不可查询；BUSY Notice 不能区分 preflight、domain 与 diagnostic。tracking ref 只是最后本地观察，inspect 不会自动修复；真实主题组合与大媒体清单仍由 DOM/CSS 契约而非宿主像素验收覆盖；
...
下一轮唯一主任务：为 Obsidian 作者事务增加只读活动快照。租约记录 `preflight / domain / diagnostic` phase、startedAt 与基于当前时钟计算的 elapsed；BUSY Notice 使用同一快照，并新增“查看当前作者事务”命令，在 active/idle 两态中只展示事实和等待建议。它不提供取消或重试按钮，不猜百分比，不持久化运行记录；显式 doctor、报告、分诊、状态与 deliver 继续绕过，不接云 API。
