...
| Markdown | done | GFM、代码高亮、语言标签、渐进增强的一键复制、与实际渲染一致的 H1–H6 heading id、H2/H3 目录与原生永久链接、Obsidian 兼容脚注/尾注、行内/块级数学公式、Callout、受限 Mermaid 服务端 SVG、受限本地静音 MP4、受限本地 MP3 + 完整文字稿、受限多图画廊、受限技术表格、受限只读任务清单、受限参考资料清单与受限步骤流程、A4 打印/PDF 版式、阅读时间、相邻文章与响应式媒体 |
...
| 网页写作 | done | `/studio`、GitHub OAuth、Decap workflow、PR、按 slug 归档媒体、稳定 slug 锁定、双层 SHA-256 冲突预检、快速重选 latest-wins、共享生产管线的公式/Callout/Mermaid/视频/音频/画廊/技术表格/只读任务/参考资料/步骤流程预览、自定义本地 MP3/静音 MP4 组件、可排序画廊、结构化技术表格/任务清单/参考资料/步骤流程组件与全字段只读发布清单 |
| Obsidian 写作 | done | Vault、三类受信模板、桌面插件 1.48.0、文件名唯一草稿身份、图片/画廊/MP3/MP4 原子归档、画廊/技术表格/任务清单/音频/参考资料/步骤流程模板插入、source-scoped 作者意图与来源行导航、发布/复核 single-flight、三方版本联锁、bundle SHA-256 完整性、四路径 Git HEAD/index/worktree provenance、sealed Git 交付恢复、维护台账、全库生产同步、手动单篇收敛与正常/恢复交付自动接力 |
...
- 本轮功能提交 `13d5658` 与归档提交 `11ffa59` 已推送到 `origin/main`；本地证据覆盖 622/622 全量单测、5/5 图表测试、73 个生成页面/资源、35/35 应用测试、125.4 秒完整发布总门、生产依赖审计 0 漏洞、插件 3/3 SHA-256 和真实浏览器桌面/390 px 预览；
- 自动交付：Vercel 已收敛到包含 `/studio/steps-editor.mjs` 的版本；稳定生产 smoke 通过 27 条路由、GitHub OAuth 302、`procedureCount: 1`、`procedureStepCount: 2`、Runbook Path HTML 及全部 HTML/发现资源预算。本轮没有改变公开内容集合或结构化端点正文，因此不重置既有生产预算基线；
- 最新完成迭代：0138 受约束步骤流程、Studio/Obsidian 作者入口、搜索降噪与 Runbook Path 阅读展示；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md`、`docs/iterations/*.md` 与 `docs/knowledge/*.md` 是同一份本地文件，可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

作者现在可以在 Studio 或 Obsidian 发布受约束的步骤流程。每组有明确标题和 2–10 个顺序步骤，每步必须包含粗体名称和操作说明，可增加一条验证条件；每篇最多 3 组/24 步。Studio 提供可排序结构化组件与共享生产预览，Obsidian 1.48.0 提供一键模板。读者页使用无交互 Runbook Path 展示稳定编号、连续顺序轨与 CHECK 证据层；搜索保留标题、步骤、说明和验证正文，桌面与 390 px 均无横向溢出，打印可在步骤边界自然分页。
...
下一轮唯一主任务：建立受约束的术语定义表。用可迁移 Markdown 表达术语、定义与可选别名/上下文，冻结单组/单篇预算、Studio/Obsidian 作者入口、搜索、窄屏、打印和语义 `<dl>` 阅读投影；不加入远程词典抓取、自动翻译、知识数据库或客户端筛选。
