...
| 内容发现 | done | SEO、内容级 OG/Twitter 封面、首页唯一 `WebSite`、文章/项目纯生成器维护的稳定身份与站点引用、文章 `wordCount`/`timeRequired`、四类详情可见路径与 `BreadcrumbList` JSON-LD、统一年月时间档案、可见订阅目录、标签/专题级 RSS 与页面自动发现、首页/Sitemap 共享公开路由事实、OpenSearch 1.1、version 1 公开内容清单及 Draft 2020-12 JSON Schema、JSON Feed 1.1、保留首发 `pubDate` 且以 `dcterms:modified` 表达更新的 RSS、Feed/清单/单篇 Markdown 的 Last-Modified 与日期条件验证、十一端点 HEAD 等价门禁、Sitemap、robots、NFKC/AND 本地全文搜索；九个结构化端点与源文均有 SHA-256 ETag/条件读取 |
...
| 结构化发现传输预算 | done | 清单、Schema、JSON Feed、根 RSS、代表标签 RSS、代表专题 RSS、Sitemap、robots、OpenSearch 的稳定生产 raw/gzip 基线、50% + raw 4 KiB/gzip 1 KiB 余量、逐端点报告与恰好一次覆盖门 |
...
- 发现：`/content.json` 从公开 getter 稳定生成 4 条机器清单，逐项提供同 origin HTML/Markdown URL 与最终源文 SHA-256；`/content.schema.json` 使用 Draft 2020-12 固定 version 1 结构，并以 describedby/describes Link 与清单双向关联。根 RSS、`/tags/[slug]/rss.xml` 与 `/series/[slug]/rss.xml` 共用 item 序列化和 HTTP 响应边界：标签 Feed 投影 `getTagBySlug()` 的公开 items；专题 Feed 接收章节序列但由共享订阅生成器按首发时间倒序输出，专题 HTML 仍保持 `series.order` 升序。两个 scoped Feed 都拥有独立 channel title/home/self、正文 SHA-256 与页面可见/alternate 发现，未知标签/专题返回共享的无验证器 `no-store` 404。RSS 只在 `updatedAt > publishedAt` 时输出 `dcterms:modified`，同时保留原 `guid`、`pubDate`、首发排序与频道更新时间，并拒绝 item 级 `atom:updated`；逐 item 的 `<category>` 只序列化同一 `record.tags`，与 JSON Feed tags 保持数量、顺序和值一致。清单、Schema、JSON Feed、根/标签/专题 RSS、Sitemap、robots、OpenSearch 都以最终正文生成 SHA-256 ETag 并支持空 304。`lib/feed-http.ts` 把格式修订时间与上海日界线下的最新内容日期合成为 Feed Last-Modified；JSON Feed、RSS、清单与单篇 Markdown 均复用三种标准 HTTP-date、`If-Modified-Since` 与 `If-None-Match` 强制优先，Feed/RSS/Sitemap/OpenSearch 保留一小时 fresh/一天 SWR，robots 保留一天 fresh；
...
- 质量：ESLint、Node test、TypeScript、Next build、真实生产服务器 HTTP 测试、npm audit、YAML workflow 契约、官方 action 完整 SHA 共享门禁、十三路 HTML 与九端点结构化发现的 `Buffer.byteLength`/Node zlib raw-gzip 双层预算、源站/边缘 ETag 与 GET/HEAD 等价验证、线上实际 origin 冒烟。

## 当前运行状态

- 仓库：<https://github.com/Zach424/MyBlog>，生产分支 `main`；
- 生产站：<https://blog-iota-five-59.vercel.app>；
- 本轮功能提交：`cf3c631`（专题级 RSS），生产基线提交：`3982113`，均已推送 `main`；
- 自动交付：稳定生产专题页按第 1→2 章展示并提供可见与 alternate RSS 发现，Feed 按最新发布第 2→1 章输出；成功/未知专题的 GET/HEAD、Last-Modified、SHA-256 ETag 与条件响应全部通过。完整 smoke 为 27 routes、OAuth 302，十三条 HTML 与九个结构化发现端点全部 PASS；专题 RSS 为 2065/983 B（raw/gzip）；
- 最新完成迭代：0127 专题级 RSS 与排序语义；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md`、`docs/iterations/*.md` 与 `docs/knowledge/*.md` 是同一份本地文件，可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

每个公开专题现在都有独立 RSS。专题页继续把 `series.order` 当作教学目录，显示第 1→2 章；`createRssXml()` 则把同一专题记录当作订阅变化流，按首发时间倒序输出第 2→1 章，并保留内容 GUID、日期、修改时间和 categories。专题页同时输出可见订阅链接与 alternate，Feed 声明独立 home/self、文件名、Link 与 `noindex`。`createRssNotFoundResponse()` 统一标签/专题未知资源的无验证器 `no-store` 404；`next.config.ts` 将专题 HTML 缓存从 `/series/:path*` 收窄到 `/series` 与 `/series/:slug`，让协议子路由拥有自己的成功/错误缓存。完整验证为 532/532 单元测试、64 个生成页面、33/33 应用测试；稳定生产为 27 routes、OAuth 302。代表专题 Feed 有 2 条内容、2065/983 B，第九条发现预算绑定功能提交 `cf3c631`。根/标签 RSS 表示修订未变化，无客户端 JavaScript、依赖、账号、数据库、追踪或云配置变化；本轮中文状态、迭代归档和知识笔记都位于同一 Obsidian Vault。
...
13. JSON Feed 当前为 4 条、20697/9876 B raw/gzip，根 RSS 为 3400/1284 B，代表标签 RSS 为 2059/923 B，代表专题 RSS 为 2065/983 B；全文 `content_text` 与各 scoped 命中集合会随公开内容增长。九端点预算已把当前规模、推导上限和本地/生产覆盖闭环，OpenSearch 当前为 700/462 B。0127 已在专题功能上线后重测并把基线来源绑定 `cf3c631`；基线不会自动追随当前输出。达到阈值后再评估最近 N 条、分页或派生缓存，不能先抬线。Vercel 会消费 SWR、可为压缩表示弱化 ETag 并精简 304 representation metadata，生产验证必须比较 opaque SHA-256、等价缓存和零正文，而不是只比较源站字符串。
...
下一轮唯一主任务：提供 OPML 2.0 聚合订阅导出，让读者从一个只读端点一次导入全站、全部公开标签与全部公开专题 Feed。它应直接消费现有订阅索引，保持稳定分组/顺序、绝对 XML URL、SHA-256 条件响应、`noindex`、页面可见入口与生产预算；不新增账号、数据库、追踪、邮件服务或第三方依赖。
