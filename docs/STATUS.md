...
| Markdown | done | GFM、代码高亮、语言标签、渐进增强的一键复制、与实际渲染一致的 H1–H6 heading id、H2/H3 目录与原生永久链接、Obsidian 兼容脚注/尾注、行内/块级数学公式、Callout、受限 Mermaid 服务端 SVG、受限本地静音 MP4、受限多图画廊、受限技术表格与受限只读任务清单、A4 打印/PDF 版式、阅读时间、相邻文章与响应式媒体 |
...
| 网页写作 | done | `/studio`、GitHub OAuth、Decap workflow、PR、按 slug 归档媒体、稳定 slug 锁定、双层 SHA-256 冲突预检、快速重选 latest-wins、共享生产管线的公式/Callout/Mermaid/视频/画廊/技术表格/只读任务清单预览、自定义本地静音视频组件、可排序画廊、结构化技术表格/任务清单组件与全字段只读发布清单 |
| Obsidian 写作 | done | Vault、三类受信模板、桌面插件 1.45.0、文件名唯一草稿身份、图片/画廊/MP4 原子归档、画廊/技术表格/任务清单模板插入、source-scoped 作者意图与来源行导航、发布/复核 single-flight、三方版本联锁、bundle SHA-256 完整性、四路径 Git HEAD/index/worktree provenance、sealed Git 交付恢复、维护台账、全库生产同步、手动单篇收敛与正常/恢复交付自动接力 |
...
作者现在可以在 Studio 或 Obsidian 使用 `[!tasks]` 加标准 GFM `[x]` / `[ ]` 项发布有标题的项目进度。每组 2–20 项、每篇最多 4 组/40 项，标题和单项分别限制 120/240 字符；裸、折叠、松散、嵌套、重复或复杂块任务失败关闭。Studio 提供可增删/重排/勾选的结构化组件，Obsidian 1.45.0 提供三项模板。读者页使用无脚本 Task Ledger：原生 progress、DONE/OPEN 证据和 disabled checkbox，搜索保留正文，390 px 与打印均有专门投影。完整证据为 585/585 单测、5/5 图表测试、70 个生成页面/资源、35/35 应用测试、生产依赖审计 0 漏洞、插件 3/3 SHA-256，以及真实浏览器桌面/390 px 语义与视觉检查。
