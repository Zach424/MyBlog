...
| Obsidian 写作 | done | Vault、三类受信模板、桌面插件 1.20.0、文件名唯一草稿身份、本地原子新建与安全改名、四个新发布/复核事务的 single-flight lease、阶段/输出活动脉冲、会话内最近终态回执与自动 doctor 联锁、13 项本机前置电路、统一只读 Git 交付分诊、版本化维护台账/Author Proof v3、两类安全重送/可信回执、deferred 并行草稿和新稿 `--check-only`/`--push` |
...
- 本轮实现提交：`00a487f2f7364ea3136dffa6437e7d20670fce06`（Obsidian 文件名唯一草稿身份与安全改名）；
- 自动交付：实现提交已推送；两条 GitHub Actions check 与 Vercel commit status 共 3/3 success，均绑定实现提交 SHA；
- 最新完成迭代：0069 文件名唯一的未发布草稿安全改名；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md` 与 `docs/iterations/*.md` 可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

MyBlog Publisher 1.20.0 把 inbox 草稿的 slug 收敛为安全文件名这一份事实：三个受信模板和新建向导不再写重复 frontmatter `slug`，发布转换继续从文件名派生正式身份。活动文件精确匹配 `content/inbox/<lowercase-ascii>.md` 时，“重命名当前草稿”Modal 以 `CURRENT → TARGET` 展示一次身份变化；输入、80 字符上限和 inbox/posts/projects 三命名空间在任何读取前验证，Vault 直接正文必须可解析、为 `draft: true` 且没有旧式顶层 `slug`，执行前再检查一次来源与碰撞。插件只调用一次 `FileManager.renameFile`，正文保持不变，内部链接更新服从 Obsidian 设置；多个 Modal 由独立 lease 串行化，同一 Modal 双击被同步 guard 拦截。API 拒绝或后置路径证据不足统一返回不确定结果，显示旧/新路径且不自动重试或回滚。全链路零子进程、零发布、零 Git、零网络。失败优先基线为 85 pass / 23 fail；实现后定向两文件 108/108、三文件 111/111，真实 doctor 13/13、固定依赖 32/32；两遍成功完整门均通过 277/277 单元与集成、45 条构建页面、19/19 生产应用测试和依赖审计 0，用时 131.1 秒与 118.3 秒。归档初稿曾触发项目页 100 KB HTML 预算，细节移回迭代档案后恢复。功能提交的两条 GitHub Actions check 与 Vercel commit status 共 3/3 success。
...
4. Current record 已有 Studio、Obsidian 与每周 Actions 三个只读视图；Obsidian 1.20.0 已闭合本地草稿创建与文件名改名，继续保留四事务 owner-checked lease、活动脉冲、会话内终态回执、Author Proof v3、统一 Git 交付 switchyard、两类独立交付状态、精确重送与可信回执。创建/改名已有 DOM/CSS、输入、模板漂移、碰撞、竞态、宿主拒绝和不确定后置状态自动测试，但没有真实 Obsidian 宿主像素快照；首次日常使用需观察 Modal 尺寸、FileManager 错误与链接自动更新设置。旧式带 frontmatter `slug` 的草稿有意拒绝自动双字段迁移，需作者核对后删除冗余字段；tracking ref 只是最后本地观察，inspect 不会自动修复；
...
下一轮唯一主任务：评估旧式 inbox 草稿的只读身份迁移诊断。先报告安全文件名、顶层 frontmatter `slug`、`draft` 与 inbox/posts/projects 命名空间事实；只有能够用 Obsidian Vault 的单文件原子修改、并发所有权和可验证恢复证明安全时，才提供一次性删除冗余 slug 的入口。不得批量改名、猜测身份、自动发布、提交、推送或访问网络；证明不足时只交付诊断。
