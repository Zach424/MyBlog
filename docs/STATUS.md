...
| Obsidian 写作 | done | Vault、三类受信模板、桌面插件 1.29.0、文件名唯一草稿身份、本地原子新建/安全改名/旧身份取证与严格清理、source-scoped 当前草稿作者意图、媒体 COVER/BODY 来源/逐次替代文本/作者或文件名回退取证/精确变换、ALT 精确源码行导航及链接 trace、四个新发布/复核事务的 single-flight lease、阶段/输出活动脉冲、会话内最近终态回执与自动 doctor 联锁、13 项本机前置电路、统一只读 Git 交付分诊、版本化维护台账/Author Proof v3、两类安全重送/可信回执、deferred 并行草稿和新稿 `--check-only`/`--push` |
...
- 本轮实现提交：`99ea0ed0699da3f07fa5691e871de38742772dee`（从 ALT 证据安全导航到精确源码行）；
- 自动交付：实现提交已推送；[Quality Gate #139](https://github.com/Zach424/MyBlog/actions/runs/31057335870) 与 [Verify Vercel production #132](https://github.com/Zach424/MyBlog/actions/runs/31057368905) 均成功，归档提交继续按同一链路独立验证；
- 最新完成迭代：0078 当前草稿媒体 ALT 精确源码行导航；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md` 与 `docs/iterations/*.md` 可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

MyBlog Publisher 1.29.0 让每个 `ALT · L<n> · AUTHORED/FILENAME FALLBACK` 标签成为原生键盘按钮。点击先重新核对安全且仍相同的冻结/活动 inbox 路径、原 `TFile` 身份与 Vault 映射，再用只读磁盘内容检查一基行号；打开后只接受同文件的活动 `MarkdownView`，重新检查编辑器行界，再设置零基 cursor、滚动并聚焦。Modal 只在成功后关闭；重复点击单航班，漂移、越界、错误视图或宿主拒绝都失败关闭。实现不写内容、不启动检查/发布/Git、不联网，readiness 仍为 version 5。定向回归 183/183、完整 352/352、45 页构建、19/19 应用测试和生产依赖审计 0 均通过；真实 doctor 13/13 ready，真实 inbox 为空且全部安全声明为 false。
...
4. Current record 已有 Studio、Obsidian 与每周 Actions 三个只读视图；Obsidian 1.29.0 已闭合本地草稿创建、旧身份取证/严格清理、文件名改名、source-scoped 当前草稿作者意图、媒体用途/来源/逐次替代文本/作者来源、精确媒体变换、ALT 源码行导航和链接 trace，并继续保留四事务 owner-checked lease、活动脉冲、会话内终态回执、Author Proof v3、统一 Git 交付 switchyard、两类独立交付状态、精确重送与可信回执。作者意图页已有 DOM/CSS、键盘焦点、重复点击单航班、ready/scheduled/blocked、COVER/BODY、出现次数/源码行/alt 文本与来源、空值/回退 blocker、optimized/preserved/unproven 媒体包络、文章/项目/自链接，以及路径、`TFile`、异步读取、磁盘/编辑器行界、视图和打开失败测试，但没有真实 Obsidian 宿主像素/交互快照；`LINK TRACE` 仍需手动在编辑器寻找位置，version 5 也没有内容指纹，因此同一路径文件在目标行仍有效时的语义变化无法自动识别。聚焦模式仍轻量解析全部草稿并读取全部已发布内容作为链接目标，这是保留共享附件和正式链接正确性的明确成本，不再对无关附件生成媒体候选；带注释、引号、anchor/tag、缩进、重复键或不匹配值的旧 slug 故意保持只读；
...
下一轮唯一主任务：让当前草稿 `LINK TRACE` 的每次引用证据复用既有导航守卫，安全打开冻结来源文件并定位到精确源码行；继续验证活动/冻结路径、Vault `TFile` 与磁盘/编辑器行界，只做本地导航，不改写正文、不自动修复、不执行发布事务、不联网或接入云服务/API。
