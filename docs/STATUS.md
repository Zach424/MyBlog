...
| 内容发现 | done | SEO、内容级 OG/Twitter 封面、首页唯一 `WebSite`、文章/项目稳定 `@id` 与 `isPartOf` 站点引用、四类详情可见路径与 `BreadcrumbList` JSON-LD、OpenSearch 1.1、version 1 公开内容清单及 Draft 2020-12 JSON Schema、JSON Feed 1.1、RSS、文章/项目可移植 Markdown 源文、Sitemap、robots、NFKC/AND 本地全文搜索；七个结构化端点与源文均有 SHA-256 ETag/条件读取 |
...
| HTML 传输预算 | done | 九条关键路由的稳定生产 raw/Node gzip 基线、160 KiB 紧急上限、20%/2 KiB gzip 余量公式、本地稳定 host 与部署后实际 origin 双验证、逐路由余量报告与覆盖失败关闭；0107 基线来自 `668d26fb` 稳定生产响应 |
...
- 结构化身份：`SITE_TITLE`、`SITE_DESCRIPTION`、`SITE_LANGUAGE` 与可信请求 origin 生成仅首页存在的唯一 `WebSite`，根 URL 与 `#website` 身份稳定；文章 `BlogPosting` 和项目 `SoftwareSourceCode` 使用同 origin canonical 加 `#content` 的稳定身份，并以最小 `isPartOf` 引用站点节点；四类详情面包屑继续由可见路径单一来源生成。全部使用原生服务端 JSON-LD script，不增加客户端代码、数据库、虚构别名、SearchAction 或未经确认的人物档案；
...
- 质量：ESLint、Node test、TypeScript、Next build、真实生产服务器 HTTP 测试、npm audit、YAML workflow 契约、官方 action 完整 SHA 共享门禁、九路 HTML 与七端点结构化发现的 `Buffer.byteLength`/Node zlib raw-gzip 双层预算、源站/边缘 ETag 等价验证与线上实际 origin 冒烟。

## 当前运行状态

- 仓库：<https://github.com/Zach424/MyBlog>，生产分支 `main`；
- 生产站：<https://blog-iota-five-59.vercel.app>；
- 本轮实现提交：`668d26fb`（文章/项目连接主页站点身份）；
- 自动交付：[Quality Gate #201](https://github.com/Zach424/MyBlog/actions/runs/31351146646) 与 [Production Smoke #194](https://github.com/Zach424/MyBlog/actions/runs/31351175522) 均成功；
- 最新完成迭代：0107 内容—站点身份图；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md` 与 `docs/iterations/*.md` 可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

文章 `BlogPosting` 与项目 `SoftwareSourceCode` 现在都从共享纯生成器取得稳定内容身份：`@id` 为各自同 origin canonical 加 `#content`，`isPartOf` 只以 `{ "@id": "<root>#website" }` 引用主页站点节点。生成器复用 `createWebsiteId()`，拒绝跨 origin、凭据、查询/fragment 和首页路径，不修改输入 URL；两类文档的 `inLanguage` 同时改为复用 `SITE_LANGUAGE`，已有标题、描述、日期、URL、图片、作者、仓库与技术栈字段保持不变。内部页仍不复制完整 `WebSite`，未知文章/项目 404 不输出内容节点，也没有新增可见 UI、客户端代码、数据库或内容字段。功能门为 497/497 单元测试、49 个构建页面、23/23 应用测试、生产依赖审计 0；真实浏览器确认两个标题正常、身份/引用准确、`WebSite=0`、console 无警告或错误；真实生产 24 路由与 OAuth 302 全部通过。九路 HTML 已用 `668d26fb` 稳定生产重新冻结，代表文章为 51784/12216 B、代表项目为 108029/24464 B（raw/gzip），gzip 余量分别为 +3144 B 与 +5232 B。
...
20. 当前内容身份生成器有意要求 canonical 与站点同 origin，避免把外部页面错误声明为本博客内部节点；内容契约仍允许 HTTPS canonical，因此未来若要转载或迁移到外部 canonical，必须先明确“本地页面身份、原始作品身份与 `isPartOf`”的语义，再调整契约和生成器，不能绕过同源门。现有文章/项目完整 JSON-LD 字段仍在两个页面内联构造，下一轮先提取为纯生成器并锁定可选字段语义，避免作者、日期、图片与代码仓库在后续迭代分叉。

下一轮唯一主任务：把文章 `BlogPosting` 与项目 `SoftwareSourceCode` 的完整 JSON-LD 对象从页面内联代码提取为两个纯生成器，继续复用共享内容/站点身份与 `SITE_LANGUAGE`，用精确对象测试锁定标题、日期、图片、作者、仓库、技术栈和 `undefined` 可选字段的边界。保持实际 HTML、可见 UI、公开事实与生产预算不变，不新增 Person/ProfilePage、客户端代码、数据库或内容字段。
