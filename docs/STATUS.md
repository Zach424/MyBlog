...
| Obsidian 写作 | done | Vault、三类受信模板、桌面插件 1.26.0、文件名唯一草稿身份、本地原子新建/安全改名/旧身份取证与严格清理、source-scoped 当前草稿作者意图、媒体 COVER/BODY 来源与精确变换及链接 trace、四个新发布/复核事务的 single-flight lease、阶段/输出活动脉冲、会话内最近终态回执与自动 doctor 联锁、13 项本机前置电路、统一只读 Git 交付分诊、版本化维护台账/Author Proof v3、两类安全重送/可信回执、deferred 并行草稿和新稿 `--check-only`/`--push` |
| Inbox 发布就绪 | done | version 3/read-only 全草稿 ready/scheduled/blocked、Article/TIL/Project、精确站内目标/源码行/重复次数、媒体 COVER/BODY 用途/出现次数/源码行、真实媒体候选、目标/共享附件诊断、CLI 全库或 `--source` 聚焦 JSON 与 Obsidian 当前草稿原生摘要 |
...
- 本轮实现提交：`890db7f46e681474120c6d0b4b7d61a5beda528b`（当前草稿媒体 COVER/BODY 来源证据）；
- 自动交付：实现提交已推送；Quality Gate run 31051421580 与 Verify Vercel production run 31051465868 均成功；归档提交仍按同一链路独立验证；
- 最新完成迭代：0075 当前草稿精确媒体来源用途；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md` 与 `docs/iterations/*.md` 可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

MyBlog Publisher 1.26.0 把每个 preparation 与原始草稿用途绑定：现有 `MEDIA TRACE` 在变换轨迹前先显示 `COVER / BODY`、出现次数与全部源码行。version 3 readiness 直接在既有 cover/正文附件归一化过程中记录来源证据；插件只严格验证角色唯一顺序、cover 单次约束、次数/行号一致和正序行号，不读图、不解析第二遍媒体语法，也不改变发布边界。167/167 定向回归、完整 328/328、45 页构建、19/19 应用测试与生产依赖审计 0 均通过；真实 inbox 为空且 version 3 安全声明保持全 false。
...
4. Current record 已有 Studio、Obsidian 与每周 Actions 三个只读视图；Obsidian 1.26.0 已闭合本地草稿创建、旧身份取证/严格清理、文件名改名、source-scoped 当前草稿作者意图、媒体用途/来源、精确媒体变换和链接 trace，并继续保留四事务 owner-checked lease、活动脉冲、会话内终态回执、Author Proof v3、统一 Git 交付 switchyard、两类独立交付状态、精确重送与可信回执。作者意图页已有 DOM/CSS、ready/scheduled/blocked、COVER/BODY、出现次数/源码行、optimized/preserved/unproven 媒体包络、文章/项目/自链接、schema/safety/计数漂移、单 entry 和活动文件漂移测试，但没有真实 Obsidian 宿主像素快照，也尚未逐项展示发布后的替代文本。聚焦模式仍轻量解析全部草稿并读取全部已发布内容作为链接目标，这是保留共享附件和正式链接正确性的明确成本，不再对无关附件生成媒体候选；带注释、引号、anchor/tag、缩进、重复键或不匹配值的旧 slug 故意保持只读；
...
下一轮唯一主任务：为 `MEDIA TRACE` 补充每个 COVER/BODY 使用点最终发布的替代文本证据，让作者在发布前核对可访问性措辞；必须复用现有 cover/frontmatter 与正文图片归一化结果，不得重新读取图片、在插件中实现第二套 Markdown/媒体解析、改变发布边界、引入云服务/真实 API、自动发布或自动修复。
