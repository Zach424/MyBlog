...
| Obsidian 写作 | done | Vault、模板、桌面插件 1.16.0、四个新发布/复核事务的 single-flight lease、active/idle 活动快照与自动 doctor 联锁、13 项本机前置电路、统一只读 Git 交付分诊、版本化维护台账/Author Proof v3、两类安全重送/可信回执、deferred 并行草稿和新稿 `--check-only`/`--push` |
...
- 发布：Decap CMS 3.14.1、GitHub OAuth、stable slug 自定义控件、同源媒体清单、内存会话账本、per-input generation 与 SHA-256 冲突确认、官方 preview template、同源只读公式/全字段预检端点、public-only 内容维护快照与严格浏览器契约、内容字段 allowlist、Obsidian 自有插件 1.16.0、四事务 single-flight lease/active-idle 快照/自动 author-doctor interlock/version 1 的 13 项 preflight circuit、单快照 Git 交付 switchyard、版本化维护 deadline ledger/复核 Author Proof v3/两类本地交付 rail 与 sealed receipt/发布 Commit Envelope/inbox 只读 Modal、候选 SHA-256 与 Git-clean blob 绑定、两类待交付提交识别和独立安全重送、精确 OID refspec、防重复发布、manifest 稳定性、共享 worktree impact classifier、deferred 路径证据、review-note/publish-note 领域与 Git 交付门、六条纯文本降级、统一子进程生命周期与 Node 发布脚本；
...
- 本轮实现提交：`fbc6c88d86ed0a4e4fcf3e2e9a153bbdecfe6466`（Obsidian 作者事务只读活动快照）；
- 自动交付：实现提交已推送；两条 GitHub Actions check 与 Vercel commit status 共 3/3 success，均绑定实现提交 SHA；
- 最新完成迭代：0065 作者事务只读活动快照；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md` 与 `docs/iterations/*.md` 可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

MyBlog Publisher 1.16.0 为 single-flight lease 增加 `preflight / domain / diagnostic` phase 和 startedAt，并提供“查看当前作者事务”。ACTIVE/IDLE 状态与并发调用的 BUSY 都从 `getAuthorTransactionSnapshot` 生成同一个冻结事实集：operation、sourcePath、phase、UTC ISO 开始时间与按查询时钟派生的非负 elapsed；查询不进入活动进程账本、不启动子进程。ready 和两类结构化证据降级只在 lease identity 仍有效时转换阶段；原有 owner child 结算、诊断/恢复 bypass 与全部释放路径不变。界面继续只用原生 Notice，不增加 CSS、Modal、按钮、进度条或历史。真实仓库为 13/13 ready、32/32 依赖匹配；定向测试 70/70，完整门为 236/236 单元与集成、45 条构建页面、19/19 生产应用测试、依赖审计 0。
...
4. Current record 已有 Studio、Obsidian 与每周 Actions 三个只读视图；Obsidian 1.16.0 已为四个新发布/复核事务增加 owner-checked single-flight lease 与活动快照，并保留严格 Author Proof v3、统一 Git 交付 switchyard、两类独立交付状态、精确重送与可信回执。快照现在能区分 phase 并显示总 elapsed，但还没有 phaseEnteredAt、阶段用时和最近 stdout/stderr 活动，因此长任务无法区分“刚进入该阶段”与“该阶段已静默很久”；时间证据也不能直接等价为进程健康。tracking ref 只是最后本地观察，inspect 不会自动修复；真实主题组合与大媒体清单仍由 DOM/CSS 契约而非宿主像素验收覆盖；
...
下一轮唯一主任务：为 Obsidian 作者事务增加只读活动脉冲。租约记录 phaseEnteredAt 与最近一次 owning child stdout/stderr 的时间；快照增加阶段用时、最近输出时间和静默时长，并在 child ownership 转交时原子重置阶段证据。它不把静默解释成故障，不提供取消、重试或 watchdog，不保存输出正文或持久历史；显式 doctor、报告、分诊、状态与 deliver 继续绕过，不接云 API。
