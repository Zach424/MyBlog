...
> 更新时间：2026-08-13 · 每轮迭代更新 · 本文件位于仓库根 Obsidian Vault 中
...
| Markdown | done | GFM、代码高亮、语言标签、渐进增强的一键复制、与实际渲染一致的 H1–H6 heading id、H2/H3 目录与原生永久链接、Obsidian 兼容脚注/尾注、行内/块级数学公式、Callout、受限 Mermaid 服务端 SVG、受限本地静音 MP4、受限本地 MP3 + 完整文字稿、受限多图画廊、受限技术表格、受限只读任务清单、受限参考资料清单、受限步骤流程、受限术语定义表、受限 FAQ、受限项目文件树、受限项目时间线、受限技术决策记录、受限技术实验记录、受限代码变更证据与受限 HTTP 请求/响应证据、A4 打印/PDF 版式、阅读时间、相邻文章与响应式媒体 |
...
| 网页写作 | done | `/studio`、GitHub OAuth、Decap workflow、PR、按 slug 归档媒体、稳定 slug 锁定、双层 SHA-256 冲突预检、快速重选 latest-wins、共享生产管线的公式/Callout/Mermaid/视频/音频/画廊/技术表格/只读任务/参考资料/步骤流程/术语表/FAQ/项目文件树/项目时间线/技术决策/技术实验/代码变更/HTTP 交换预览、自定义本地 MP3/静音 MP4 组件、可排序画廊与结构化富内容组件、UNIFIED/BEFORE_AFTER 代码变更编辑器、静态脱敏 HTTP 交换编辑器及全字段只读发布清单 |
| Obsidian 写作 | done | Vault、三类受信模板、桌面插件 1.56.0、文件名唯一草稿身份、图片/画廊/MP3/MP4 原子归档、画廊/技术表格/任务清单/音频/参考资料/步骤流程/术语表/FAQ/项目文件树/项目时间线/技术决策/技术实验/代码变更/HTTP 交换模板插入、source-scoped 作者意图与来源行导航、发布/复核 single-flight、三方版本联锁、bundle SHA-256 完整性、四路径 Git HEAD/index/worktree provenance、sealed Git 交付恢复、维护台账、全库生产同步、手动单篇收敛与正常/恢复交付自动接力 |
...
| 生产内容同步 | done | `content:production` 输出全库 deployed/pending/missing/unexpected；`content:production:wait` 冻结单篇来源 SHA-256/ETag，以条件 GET 有界等待 deployed；Obsidian 1.45.0 提供手动入口，并从正常或 recovery publication/review 的可信 Git 成功结果在写事务释放、Vault reconcile 后自动接力同一 latest-wins 等待器 |
...
| 媒体门禁 | done | 图片真实格式解码、3 MiB/2560 px/像素与动图预算；MP3 真实编码/时长/码率/采样率/声道、8 MiB/15 分钟预算；MP4 真实轨道/编码/fast-start、12 MiB/90 秒/1080p 预算；Studio 上传前诊断、Obsidian 处理与构建扫描 |
| 媒体引用完整性 | done | Markdown AST 图片/画廊/音频/视频抽取、精确路径存在性、根暂存拒绝、slug 所有权与已归档孤儿附件门禁 |
| 媒体展示 | done | 封面、正文图和画廊共享固有尺寸/`next/image` 链路；画廊使用无脚本有序接触表、桌面双栏/窄屏单栏与打印布局；HTTPS 外图有明确降级边界；本地 MP3 与静音 MP4 使用无 autoplay/iframe/追踪的原生播放器，音频完整文字稿可直接阅读/搜索，打印降级为文字稿和本地路径 |
...
- 发布：Decap CMS 3.14.1、GitHub OAuth、stable slug/媒体/公式/条目预检、本地 MP3/静音 MP4、可排序画廊、结构化富内容、UNIFIED/BEFORE_AFTER 代码变更与静态脱敏 HTTP 交换组件，Obsidian Publisher 1.56.0 的受信模板与代码变更/HTTP 交换插入、文件名身份、source-scoped 作者意图、媒体/链接来源行、四事务联锁、所有 Git writer 三方版本、bundle 摘要与 Git provenance 握手、sealed Git 交付恢复、全库生产同步、单篇收敛等待与正常/恢复 version 1 post-delivery handoff；`content:production`/`content:production:wait` 复用公开清单生成器，使用受限流式 GET、严格 version 1 协议、来源字节冻结、条件请求与零写入报告；
- 媒体：Sharp 0.35.3、MP4Box 2.4.1、music-metadata 11.14.0、浏览器图片/音频/视频元数据解析、`createImageBitmap`/`HTMLAudioElement`/`HTMLVideoElement` 与 Web Crypto、构建期确定性摘要清单、mdast-util-from-markdown 2.0.3、`next/image`、原生 `<audio>`/`<video>`、固有尺寸、WebP 优化、引用所有权与 Git 附件跟踪；
...
- Iteration 0146 发布候选已通过本地总门：639/639 全量单测、10/10 HTTP 专项、81 个生成页面/资源、35/35 应用测试、153.2 秒完整 `release:check`、生产依赖审计 0 漏洞与插件 3/3 SHA-256；
- 真实 Chromium 已覆盖 1,200 px、320 px 与两页打印 PDF：公开台账无按钮，宽屏请求/响应并排，窄屏纵向且根页面无横向溢出；打印首轮发现并修复 VERIFICATION 孤标题，复验后整个验证区在第二页保持完整。Vercel 收敛与稳定生产 smoke 待本轮功能提交后补写；
- 最新完成迭代：0146 受约束 HTTP 请求/响应证据、静态 Exchange Ledger、Studio/Obsidian 作者入口、搜索降噪与安全发布门；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md`、`docs/iterations/*.md` 与 `docs/knowledge/*.md` 是同一份本地文件，可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

作者现在可以在 Studio 或 Obsidian 发布受约束 HTTP 请求/响应证据。每条固定保存标题、方法、状态、真实日期、目的、脱敏目标、安全头字段、可选文本正文与 1–6 条验证；每篇最多 2 条，头字段合计最多 30 个，正文合计最多 160 行。共享 AST 同时校验固定区段、HTTPS/loopback、URL 凭据/片段/敏感查询名、头字段唯一性、正文类型与 Content-Type、无正文语义、未来日期和疑似凭据。Studio 提供结构化表单与共享生产预览，Obsidian Publisher 1.56.0 提供一键模板；阅读端以无交互 Exchange Ledger 展示，搜索保留安全事实并删除机械标签，320 px 与打印保留全部证据。
...
35. 受限只读任务清单已闭环可移植语法、完成/待完成状态、单组/单篇预算、Studio/Obsidian、搜索、桌面/390 px/打印和共享生产预览，但当前公开内容还没有真实任务清单样本。现有证据证明原生 progress、三个有名称的 disabled checkbox、行内 Markdown 和无客户端交互，不证明第一次在真实 Decap workflow 中编辑最大清单的效率，也不覆盖任务历史。读者勾选、截止日期、负责人、提醒、评论、筛选、全站聚合和 Obsidian Tasks 双向同步继续关闭；出现持久协作任务时应使用独立任务系统。

36. 受限本地 MP3 已闭环可移植语法、真实编码/时长/码率/采样率/声道预算、Studio/Obsidian、附件事务、搜索、原生播放器、桌面/390 px/打印和共享生产预览，但当前公开内容没有真实音频样本。现有浏览器夹具引用刻意不存在的 MP3，因此 404 只说明播放器请求了该地址，不证明真实解码、Range、Content-Type 或 CDN 缓存。WAV/M4A/OGG、外部托管、自动转写、RSS enclosure、章节、波形脚本和播放统计继续关闭；真实内容上线后再补文件传输证据。

37. 受限参考资料清单已闭环可移植语法、标题/条目/URL/短注预算、Studio/Obsidian、搜索、站内关系、HTTPS 库存、桌面/390 px/打印和共享生产预览，但当前公开内容没有真实清单样本。现有证据证明作者契约和 Source Index 投影，不证明外站长期可用，也不覆盖第一次真实 Decap workflow 编辑 12 条资料的效率。远程标题/摘要/favicon 抓取、BibTeX/DOI/ISBN 元数据、网页快照、健康徽章、分类嵌套和账号书签继续关闭；实时网络状态仍只由作者显式报告观察。

38. 受限步骤流程已闭环可移植语法、顺序/标题/说明/验证预算、Studio/Obsidian、搜索、桌面/390 px/打印和共享生产预览，但当前公开内容没有真实流程样本。现有证据证明静态 Runbook Path 与有序语义，不证明第一次真实 Decap workflow 编辑 10 步流程的效率，也不覆盖读者完成状态。分支、循环、并行泳道、交互勾选、负责人、期限、提醒、自动执行和外部任务同步继续关闭；需要状态或协作时应使用独立工作流系统。

39. 受限术语定义表已闭环可移植语法、标题/术语/定义/别名/上下文预算、同组名称唯一性、Studio/Obsidian、搜索、桌面/390 px/打印和共享生产预览，但当前公开内容没有真实术语表样本。现有证据证明静态 Definition Ledger 与定义列表语义，不证明第一次真实 Decap workflow 编辑 12 个术语的效率，也不证明定义本身正确。跨文章术语合并、自动反向链接、远程词典抓取、自动翻译、同义词推断、知识数据库、客户端筛选和协同审校继续关闭；知识内容仍由作者核实。

40. 受限 FAQ 已闭环可移植语法、标题/问题/答案预算、同组问题唯一性、Studio/Obsidian、搜索、点击/键盘、320 px、打印和共享生产预览，但当前公开内容没有真实 FAQ 样本。现有证据证明静态 Answer Cabinet 与原生 details/summary 披露语义，不证明第一次真实 Decap workflow 编辑 10 个问题的效率，也不证明答案本身正确。投票、评论、远程问答库、自动生成答案、客户端搜索、跨访问展开状态和协同审校继续关闭；问答内容仍由作者核实。

41. 受限项目时间线已闭环可移植语法、真实/未来日期、事件类型/排序/唯一性、Studio/Obsidian、搜索、桌面/320 px/打印和共享生产预览，但当前公开内容没有真实时间线样本。现有证据证明静态 Release Tape、原生 `<time datetime>` 和未来日期失败关闭，不证明作者记录的历史本身正确，也不覆盖第一次真实 Decap workflow 编辑 16 个事件的效率。时分秒、跨时区、未来计划、提醒、进度、负责人、日历与外部项目管理同步继续关闭；需要计划或协作时应使用独立系统。

42. 受限技术决策记录已闭环可移植语法、状态/真实日期、固定叙述区段、备选与影响台账、唯一性/总量预算、Studio/Obsidian、搜索、桌面/320 px/打印和共享生产预览，但当前公开内容没有真实决策样本。现有证据证明静态 Decision Brief、原生 `<time datetime>` 和未来日期失败关闭，不证明作者判断本身正确，也不覆盖第一次真实 Decap workflow 编辑 6 个方案的效率。审批、投票、评论、读者编辑、远程工单同步、自动建议和替代关系自动维护继续关闭；需要协作决策流程时应使用独立系统。

43. 受限技术实验记录已闭环可移植语法、结果状态/真实日期、假设/环境/方法/样本/测量/结论/局限、唯一性与总量预算、Studio/Obsidian、搜索、桌面/320 px/打印和共享生产预览，但当前公开内容没有真实实验样本。现有证据证明静态 Bench Sheet、原生 `<time datetime>`、测量台账和未来日期失败关闭，不证明作者假设、读数或结论本身正确，也不覆盖第一次真实 Decap workflow 编辑 8 个指标的效率。在线执行、遥测、运行数据库、原始数据、自动结论、显著性推断、跨运行比较和客户端状态继续关闭；需要实验管理时应使用独立系统。

44. 受限代码变更证据已闭环可移植语法、UNIFIED/BEFORE_AFTER 双模式、真实日期、目的/文件/代码/验证/风险、路径安全、diff 一致性、敏感凭据与总量预算、Studio/Obsidian、搜索、桌面/320 px/打印和共享生产预览，但当前公开内容没有真实代码变更样本。现有证据证明静态 Review Docket 和文件台账，不证明 patch 来自某个提交、hunk 行号完整或验证结论正确，也不覆盖第一次真实 Decap workflow 编辑多文件 diff 的效率。实时 Git、patch 应用、仓库编辑、签名、commit/PR/CI 状态、行级评论、折叠和读者审批继续关闭；需要协作审查时应使用代码托管平台。

45. 受限 HTTP 请求/响应证据已闭环可移植语法、常用方法、100–599 状态、真实日期、脱敏 URL、安全头字段、可选文本正文、Content-Type/无正文语义、验证与总量预算、Studio/Obsidian、搜索、桌面/320 px/打印和共享生产预览。当前公开内容没有真实 `[!http]` 样本；现有证据证明静态 Exchange Ledger、失败关闭与作者入口，不证明远端请求真实发生、响应未被篡改或验证结论正确。网络发送、重放、Cookie/Authorization/API key、变量替换、请求链、HAR 导入、客户端生成、二进制/multipart、计时与读者状态继续关闭；真实凭据永远不进入博客源文。

下一轮唯一主任务：建立受约束的终端命令运行证据块。用可迁移 Markdown 保存已执行命令、操作系统/运行时、仓库相对工作目录、退出码、受限 stdout/stderr、说明与验证，并接入 Studio/Obsidian、搜索、窄屏和打印；不执行命令、不提供终端模拟器、不保存环境变量或凭据、不重放进程、不生成脚本，也不连接远程 shell。
