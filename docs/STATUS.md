...
| Obsidian 写作 | done | Vault、三类受信模板、桌面插件 1.27.0、文件名唯一草稿身份、本地原子新建/安全改名/旧身份取证与严格清理、source-scoped 当前草稿作者意图、媒体 COVER/BODY 来源/逐次替代文本/精确变换及链接 trace、四个新发布/复核事务的 single-flight lease、阶段/输出活动脉冲、会话内最近终态回执与自动 doctor 联锁、13 项本机前置电路、统一只读 Git 交付分诊、版本化维护台账/Author Proof v3、两类安全重送/可信回执、deferred 并行草稿和新稿 `--check-only`/`--push` |
| Inbox 发布就绪 | done | version 4/read-only 全草稿 ready/scheduled/blocked、Article/TIL/Project、精确站内目标/源码行/重复次数、媒体 COVER/BODY 用途/出现次数/源码行/最终替代文本、空替代文本阻塞、真实媒体候选、目标/共享附件诊断、CLI 全库或 `--source` 聚焦 JSON 与 Obsidian 当前草稿原生摘要 |
...
- 本轮实现提交：`4ff7acc6d3cbbb71f47d25ebde60b754653cc68c`（当前草稿媒体逐次替代文本证据）；
- 自动交付：实现提交已推送；Quality Gate run 31052849364 与 Verify Vercel production run 31052890256 均成功；归档提交仍按同一链路独立验证；
- 最新完成迭代：0076 当前草稿媒体替代文本轨迹；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md` 与 `docs/iterations/*.md` 可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

MyBlog Publisher 1.27.0 把每个媒体使用点的最终替代文本与既有来源行一一绑定：version 4 `altTexts` 直接在正文 Wiki/Markdown 图片转换时记录，cover 文本则取自已经通过正式 Zod 契约的 `coverAlt`。`MEDIA TRACE` 用 `ALT · L<n>` 显示原生文本；空值保留证据、标为 `EMPTY · WILL FAIL`，readiness 同时产生 `attachment-alt-empty` 并阻塞草稿。插件严格验证次数/行号/文本长度和 blocker 双向对应，不解析 Markdown、不读图、不自动改写。174/174 定向回归、完整 335/335、45 页构建、19/19 应用测试与生产依赖审计 0 均通过；真实 inbox 为空且 version 4 安全声明保持全 false。
...
4. Current record 已有 Studio、Obsidian 与每周 Actions 三个只读视图；Obsidian 1.27.0 已闭合本地草稿创建、旧身份取证/严格清理、文件名改名、source-scoped 当前草稿作者意图、媒体用途/来源/逐次替代文本、精确媒体变换和链接 trace，并继续保留四事务 owner-checked lease、活动脉冲、会话内终态回执、Author Proof v3、统一 Git 交付 switchyard、两类独立交付状态、精确重送与可信回执。作者意图页已有 DOM/CSS、ready/scheduled/blocked、COVER/BODY、出现次数/源码行/alt 文本、optimized/preserved/unproven 媒体包络、文章/项目/自链接、schema/safety/计数漂移、单 entry 和活动文件漂移测试，但没有真实 Obsidian 宿主像素快照；无 display 的 Wiki 图片仍会以文件名生成非空 alt，当前可见但尚未区分“作者描述”与“文件名回退”。聚焦模式仍轻量解析全部草稿并读取全部已发布内容作为链接目标，这是保留共享附件和正式链接正确性的明确成本，不再对无关附件生成媒体候选；带注释、引号、anchor/tag、缩进、重复键或不匹配值的旧 slug 故意保持只读；
...
下一轮唯一主任务：为每个 BODY 替代文本增加 `AUTHORED / FILENAME FALLBACK` 来源证据，并让文件名回退进入明确 blocker，要求作者在 Markdown alt 或 Wiki display 中填写真实描述；必须在现有正文图片归一化回调中产出，不得另写 Markdown 解析、自动生成/改写文案、改变发布边界或接入云服务/真实 API。
