...
| Markdown | done | GFM、代码高亮、语言标签、渐进增强的一键复制、与实际渲染一致的 H1–H6 heading id、H2/H3 目录与原生永久链接、Obsidian 兼容脚注/尾注、行内/块级数学公式、Callout、受限 Mermaid 服务端 SVG、受限本地静音 MP4、受限本地 MP3 + 完整文字稿、受限多图画廊、受限技术表格、受限只读任务清单与受限参考资料清单、A4 打印/PDF 版式、阅读时间、相邻文章与响应式媒体 |
...
| 网页写作 | done | `/studio`、GitHub OAuth、Decap workflow、PR、按 slug 归档媒体、稳定 slug 锁定、双层 SHA-256 冲突预检、快速重选 latest-wins、共享生产管线的公式/Callout/Mermaid/视频/音频/画廊/技术表格/只读任务/参考资料预览、自定义本地 MP3/静音 MP4 组件、可排序画廊、结构化技术表格/任务清单/参考资料组件与全字段只读发布清单 |
| Obsidian 写作 | done | Vault、三类受信模板、桌面插件 1.47.0、文件名唯一草稿身份、图片/画廊/MP3/MP4 原子归档、画廊/技术表格/任务清单/音频/参考资料模板插入、source-scoped 作者意图与来源行导航、发布/复核 single-flight、三方版本联锁、bundle SHA-256 完整性、四路径 Git HEAD/index/worktree provenance、sealed Git 交付恢复、维护台账、全库生产同步、手动单篇收敛与正常/恢复交付自动接力 |
...
作者现在可以在 Studio 或 Obsidian 发布受约束的参考资料清单。每组有明确标题和 2–12 条资料，每条必须包含可独立理解的名称、完整 HTTPS 或站内绝对路径，并可补一行短注；每篇最多 3 组/24 条。Studio 提供可排序结构化组件与共享生产预览，Obsidian 1.47.0 提供一键模板。读者页显示稳定序号、站外域名或“本站”，搜索保留标题/名称/短注，站内目标和 HTTPS 目标分别复用知识关系与外链库存，打印保留显式 URL；桌面与 390 px 均无横向溢出。
...
下一轮唯一主任务：建立受约束的步骤流程块。用可迁移 Markdown 表达有顺序的操作步骤、步骤标题、说明与可选验证结果，冻结单组/单篇预算、Studio/Obsidian 作者入口、搜索、窄屏和打印；不加入读者交互状态、流程数据库、提醒或外部任务系统。
