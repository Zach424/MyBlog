...
| Obsidian 写作 | done | Vault、三类受信模板、桌面插件 1.24.0、文件名唯一草稿身份、本地原子新建/安全改名/旧身份取证与严格清理、source-scoped 当前草稿作者意图与精确链接 trace、四个新发布/复核事务的 single-flight lease、阶段/输出活动脉冲、会话内最近终态回执与自动 doctor 联锁、13 项本机前置电路、统一只读 Git 交付分诊、版本化维护台账/Author Proof v3、两类安全重送/可信回执、deferred 并行草稿和新稿 `--check-only`/`--push` |
...
- 本轮实现提交：`57163eaf8974b84df728fb1fce6a0122e58f0dfd`（当前草稿精确站内链接 trace）；
- 自动交付：实现提交已推送；远端 Quality、Vercel 验证与生产状态仍由 GitHub/Vercel 按提交独立报告；
- 最新完成迭代：0073 当前草稿精确站内链接目标；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md` 与 `docs/iterations/*.md` 可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

MyBlog Publisher 1.24.0 把 `LINKS` 计数背后的正式验证结果提升为 version 2 `internalLinks`：每个唯一精确目标保留 `post/project/self`、最终公开路径与 fragment、全部源码行和重复次数。证据来自同一次 GFM AST 引用遍历与原有目标/标题验证循环，公开 outgoing/backlink 仍按原规则去重。插件严格交叉检查数量、路径类型、自链接身份、顺序和重复目标，再用只读 `LINK TRACE` 展示，不增加跳转、修复或发布动作。失败优先合同、相关回归、完整 311/311、45 页构建、19/19 应用测试与生产依赖审计 0 均通过；真实 inbox 为空且 version 2 安全声明保持全 false。
...
4. Current record 已有 Studio、Obsidian 与每周 Actions 三个只读视图；Obsidian 1.24.0 已闭合本地草稿创建、旧身份取证/严格清理、文件名改名、source-scoped 当前草稿作者意图和精确链接 trace，并继续保留四事务 owner-checked lease、活动脉冲、会话内终态回执、Author Proof v3、统一 Git 交付 switchyard、两类独立交付状态、精确重送与可信回执。作者意图页已有 DOM/CSS、ready/scheduled/blocked、媒体包络、文章/项目/自链接、重复次数、源码行、schema/safety/计数漂移、单 entry 和活动文件漂移测试，但没有真实 Obsidian 宿主像素快照；聚焦模式仍轻量解析全部草稿并读取全部已发布内容作为链接目标，这是保留共享附件和正式链接正确性的明确成本，不再对无关附件生成媒体候选；带注释、引号、anchor/tag、缩进、重复键或不匹配值的旧 slug 故意保持只读；
...
下一轮唯一主任务：把当前草稿的 `MEDIA` 从单一附件数提升为精确媒体变换清单，直接消费 readiness 已验证的 source/target/public URL、格式、宽高、帧数、字节与优化状态；不得重新读取图片、在插件中实现媒体规则、改变发布边界、引入云服务/真实 API、自动发布或自动修复。
