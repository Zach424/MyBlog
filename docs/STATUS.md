...
| Obsidian 写作 | done | Vault、三类受信模板、桌面插件 1.40.0、文件名唯一草稿身份、原子新建/改名/旧身份清理、source-scoped 作者意图与来源行导航、发布/复核 single-flight、运行代码/runtime manifest/磁盘 manifest 三方版本联锁、manifest/main/styles 的 bundle SHA-256 完整性、sealed Git 交付恢复、维护台账、全库生产同步、手动单篇收敛与正常/恢复交付自动接力 |
...
| 生产内容同步 | done | `content:production` 输出全库 deployed/pending/missing/unexpected；`content:production:wait` 冻结单篇来源 SHA-256/ETag，以条件 GET 有界等待 deployed；Obsidian 1.40.0 提供手动入口，并从正常或 recovery publication/review 的可信 Git 成功结果在写事务释放、Vault reconcile 后自动接力同一 latest-wins 等待器 |
...
- 发布：Decap CMS 3.14.1、GitHub OAuth、stable slug/媒体/公式/条目预检，Obsidian Publisher 1.40.0 的受信模板、文件名身份、source-scoped 作者意图、媒体/链接来源行、四事务联锁、所有 Git writer 三方版本与 bundle 摘要握手、sealed Git 交付恢复、全库生产同步、单篇收敛等待与正常/恢复 version 1 post-delivery handoff；`content:production`/`content:production:wait` 复用公开清单生成器，使用受限流式 GET、严格 version 1 协议、来源字节冻结、条件请求与零写入报告；
...
- 本轮实现提交：`5890e5a`（MyBlog Publisher 磁盘 bundle 完整性契约）；
- 自动交付：[Quality Gate #180](https://github.com/Zach424/MyBlog/actions/runs/31336409712) 与 [Production Smoke #173](https://github.com/Zach424/MyBlog/actions/runs/31336436650) 均成功；
- 最新完成迭代：0097 Obsidian 插件 bundle 完整性；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md` 与 `docs/iterations/*.md` 可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

MyBlog Publisher 1.40.0 新增不自我哈希的 `bundle.json`，按固定顺序绑定 `main.js`、`manifest.json`、`styles.css` 的插件 id/version 与 SHA-256。显式生成器只在 `--write` 时改 descriptor，默认复算；Author Doctor legacy JSON 保持 13 项 version 1 供 1.39 旧 runtime 识别未来磁盘版本，当前插件和默认 text 使用增加规范化摘要证据与 `publisher-bundle` 的 14 项 version 2。六个 Git writer 只在三方版本和 bundle 同时 compatible/verified 时继续；缺失、符号链接、未知字段、局部更新或摘要漂移显示无按钮 `PLUGIN BUNDLE INVALID`，不自动生成、覆盖、重载或 Git。完整门为 465/465、47 路由、20/20、生产依赖审计 0；真实生产以冻结参数在 1 次、1263 ms 内确认 deployed。
...
4. Obsidian 1.40.0 已闭合草稿身份、source-scoped 作者意图、媒体/链接源码行、发布/复核事务、sealed Git 交付恢复、内容维护、全库生产同步和活动正式笔记收敛等待；正常与恢复 publication/review 成功后均自动接力。三方版本、bundle 完整性、未来 patch/minor、伪造/局部摘要失败关闭、receipt/handoff、事务先释放、reconcile 后启动、latest-wins、卸载取消和零重复 Git 动作均由宿主/真实临时仓库覆盖，但仍需首次真实 Obsidian 主题下观察 reload/bundle interlock、两个连续 Modal、长 ETag、commit 和持续 Notice 的视觉密度；
...
15. Git/Obsidian sealed receipt、version 1 handoff、生产收敛、三方版本与磁盘 bundle 摘要已覆盖正常和恢复交付。descriptor 仍证明“这些工作区文件彼此一致”，不证明它们由 Git 跟踪或等于受信 HEAD/index；受控开发者显式运行 `--write` 可以为任意当前字节生成新摘要。内容发布门会拒绝夹带代码，但 recovery 的最窄前置证据仍应明确绑定插件文件的 Git provenance。

下一轮唯一主任务：为插件 bundle 增加只读 Git provenance。新增协商式下一版 doctor 证据，证明 `bundle.json` 与三份插件文件均被 Git 跟踪、工作区/索引未修改，并绑定同一 HEAD tree/blob；1.40 的 version 2 请求必须继续可用，以免旧 runtime 再次退化。正常与恢复 Git writer 在 provenance 不可信时显示专用 hold，不运行 add/commit/push/fetch/reset，不把本地 `--write` 当作发布来源签名，也不扩展为远程签名服务。
