...
| Obsidian 写作 | done | Vault、模板、桌面插件 1.4.0、版本化维护台账、正式内容检查/精确同步、收件箱总览与带目标标题校验的新稿 `--check-only`/`--push` |
...
- 发布：Decap CMS 3.14.1、GitHub OAuth、stable slug 自定义控件、同源媒体清单、内存会话账本、per-input generation 与 SHA-256 冲突确认、官方 preview template、同源只读公式/全字段预检端点、public-only 内容维护快照与严格浏览器契约、内容字段 allowlist、Obsidian 自有插件 1.4.0、版本化维护 deadline ledger/inbox 只读 Modal、精确 Vault 笔记打开、正式内容 review-note 领域/Git 双门、纯文本降级、统一子进程生命周期与 Node 发布脚本；
...
- 本轮实现提交：`2e6f05f62ed8543a15cca593ba82cfa5d460c319`（正式内容人工复核契约、完整检查、精确单文件提交/推送和 Obsidian 双命令）；
- 自动交付：本轮实现 Quality Gate `30986953764`（#93）与 Vercel Production 验证 `30986989441`（#86）均 completed/success；
- 最新完成迭代：0053 Obsidian 正式内容自助复核闭环；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md` 与 `docs/iterations/*.md` 可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

MyBlog Publisher 1.4.0 把维护台账推进为完整人工复核闭环。作者必须先手工核对并编辑正式 Current 笔记；`content:review` 再比较 HEAD 与当前语义，要求 reviewedAt 严格推进到上海当天，事实变化时 updatedAt 同步当天，并拒绝 publishedAt/freshness 身份漂移。Git 门只接受 main、已跟踪目标、空暂存区、零未跟踪文件和唯一目标修改；完整 `npm run check` 前后复查状态。check-only 不暂存、不提交、不推送；push 使用 pathspec 与 `git commit --only`，提交后复核 tree 只含该 Markdown 再推送。临时真实仓库验证门失败保持 HEAD/暂存区不变，本地裸远端验证成功提交的唯一文件。最终门为 171/171 单元、45 条构建页面、19/19 生产应用测试与 audit 0；公开静态资源仍为 1,818,133 B，插件 main/manifest/styles 合计 29,770 B且不进入公开客户端。
...
4. Current record 已有 Studio、Obsidian 与每周 Actions 三个只读视图；Obsidian 1.4.0 已能定位、检查并精确同步正式笔记，但检查成功目前只有 Notice，作者仍需自己确认日期和语义差异；同日不能因日期精度重复声明复核，push 失败会保留合法本地提交。真实主题组合与大队列仍由 DOM/CSS 契约而非宿主像素验收覆盖，插件更新后必须重启或重新启用；
...
下一轮唯一主任务：给“检查当前正式内容复核”增加结构化 Author Proof。CLI 在完整门通过后输出版本化 JSON，列出 sourcePath、HEAD/当前 reviewedAt、是否有事实变化、updatedAt 证据、唯一可提交路径与质量门结果；插件严格验证后用原生只读 Modal 展示，同步命令仍保持独立显式动作。JSON/schema/UI 异常时退回纯文本检查证据，不显示误导性的可提交状态；继续零自动改日期、零 check-only Git 副作用、零外部服务。
