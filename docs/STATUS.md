...
| 内容发现 | done | SEO、内容级 OG/Twitter 封面、首页唯一 `WebSite`、文章/项目纯生成器维护的稳定身份与站点引用、文章 `wordCount`/`timeRequired`、四类详情可见路径与 `BreadcrumbList` JSON-LD、统一年月时间档案、可见订阅目录、标签级 RSS 与页面自动发现、首页/Sitemap 共享公开路由事实、OpenSearch 1.1、version 1 公开内容清单及 Draft 2020-12 JSON Schema、JSON Feed 1.1、保留首发 `pubDate` 且以 `dcterms:modified` 表达更新的 RSS、Feed/清单/单篇 Markdown 的 Last-Modified 与日期条件验证、十端点 HEAD 等价门禁、Sitemap、robots、NFKC/AND 本地全文搜索；八个结构化端点与源文均有 SHA-256 ETag/条件读取 |
...
| 结构化发现传输预算 | done | 清单、Schema、JSON Feed、根 RSS、代表标签 RSS、Sitemap、robots、OpenSearch 的稳定生产 raw/gzip 基线、50% + raw 4 KiB/gzip 1 KiB 余量、逐端点报告与恰好一次覆盖门 |
...
- 发现：`/content.json` 从公开 getter 稳定生成 4 条机器清单，逐项提供同 origin HTML/Markdown URL 与最终源文 SHA-256；`/content.schema.json` 使用 Draft 2020-12 固定 version 1 结构，并以 describedby/describes Link 与清单双向关联。根 RSS 与 `/tags/[slug]/rss.xml` 共用 item 序列化和 HTTP 响应边界：标签 Feed 只投影 `getTagBySlug()` 的公开 items，拥有独立 channel title/home/self、正文 SHA-256 与页面可见/alternate 发现，未知标签返回无验证器 `no-store` 404。RSS 只在 `updatedAt > publishedAt` 时输出 `dcterms:modified`，同时保留原 `guid`、`pubDate`、首发排序与频道更新时间，并拒绝 item 级 `atom:updated`；逐 item 的 `<category>` 只序列化同一 `record.tags`，与 JSON Feed tags 保持数量、顺序和值一致。清单、Schema、JSON Feed、根/标签 RSS、Sitemap、robots、OpenSearch 都以最终正文生成 SHA-256 ETag 并支持空 304。`lib/feed-http.ts` 把格式修订时间与上海日界线下的最新内容日期合成为 Feed Last-Modified；JSON Feed、RSS、清单与单篇 Markdown 均复用三种标准 HTTP-date、`If-Modified-Since` 与 `If-None-Match` 强制优先，Feed/RSS/Sitemap/OpenSearch 保留一小时 fresh/一天 SWR，robots 保留一天 fresh；
...
- 质量：ESLint、Node test、TypeScript、Next build、真实生产服务器 HTTP 测试、npm audit、YAML workflow 契约、官方 action 完整 SHA 共享门禁、十三路 HTML 与八端点结构化发现的 `Buffer.byteLength`/Node zlib raw-gzip 双层预算、源站/边缘 ETag 与 GET/HEAD 等价验证、线上实际 origin 冒烟。

## 当前运行状态

- 仓库：<https://github.com/Zach424/MyBlog>，生产分支 `main`；
- 生产站：<https://blog-iota-five-59.vercel.app>；
- 本轮功能提交：`d4e26b8`（标签级 RSS），生产基线提交：`4bda63a`，均已推送 `main`；
- 自动交付：稳定生产 TypeScript 标签页已提供可见与 alternate RSS 发现，Feed 只含 2 条匹配内容；成功/未知标签的 GET/HEAD、Last-Modified、SHA-256 ETag 与条件响应全部通过。完整 smoke 为 27 routes、OAuth 302，十三条 HTML 与八个结构化发现端点全部 PASS；标签 RSS 为 2059/923 B（raw/gzip）；
- 最新完成迭代：0126 标签级 RSS 订阅；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md`、`docs/iterations/*.md` 与 `docs/knowledge/*.md` 是同一份本地文件，可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

每个公开标签现在都有独立 RSS。`createRssXml()` 接受 scoped channel 元数据，`createRssResponse()` 集中根/标签 RSS 的 MIME、缓存、Last-Modified 和条件验证器；标签路由只传入公开 `tag.items`，保留内容 GUID、时间、排序和 categories。标签页同时输出可见订阅链接与自动发现，未知标签 GET/HEAD 为无验证器 `no-store` 404，Feed 不进入 Sitemap。真实应用测试两次捕获 next.config `/tags/:path*` 覆盖成功与错误缓存，最终将页面规则收窄到 `/tags` 和 `/tags/:slug`。完整验证为 531/531 单元测试、63 个生成页面、32/32 应用测试；稳定生产为 27 routes、OAuth 302。TypeScript Feed 有 2 条内容、2059/923 B，新八端点基线绑定功能提交 `d4e26b8`。无客户端 JavaScript、依赖、账号、数据库、追踪或云配置变化；本轮中文状态、迭代归档和知识笔记都位于同一 Obsidian Vault。
...
13. JSON Feed 当前为 4 条、20697/9876 B raw/gzip，根 RSS 为 3400/1284 B，代表标签 RSS 为 2059/923 B；全文 `content_text` 和各标签命中集合会随公开内容增长。八端点预算已把当前规模、推导上限和本地/生产覆盖闭环，OpenSearch 当前为 700/462 B。0126 已在功能上线后重测并把基线来源绑定 `d4e26b8`；基线不会自动追随当前输出。达到阈值后再评估最近 N 条、分页或派生缓存，不能先抬线。Vercel 会消费 SWR、可为压缩表示弱化 ETag 并精简 304 representation metadata，生产验证必须比较 opaque SHA-256、等价缓存和零正文，而不是只比较源站字符串。
...
25. `/activity` 已提供按事件查看“何时发布、何时真正更新”的统一活动流，archive 继续只保留首次发布日期。首页摘要已直接复用同一模型并限制三项，没有复制排序或日期规则；根/标签 RSS 以 `dcterms:modified` 表达严格更新语义，以 category 表达与 JSON Feed 相同的作者标签，同时保留首发 `pubDate`。JSON Feed/RSS 的 HTTP Last-Modified 另行表达整个表示何时发生变化，并同时考虑内容日期与格式修订。阅读器可以合法忽略扩展、分类或 HTML 自动发现，因此本站保证标准化 XML、解析器可读和跨格式对齐，不保证所有客户端一定展示或通知这些字段。
...
27. 标签 RSS 依赖精确路由缓存所有权。`next.config.ts` 只给 `/tags` 与 `/tags/:slug` HTML 页面设置 CDN 缓存，不能恢复成 `/tags/:path*`，否则会覆盖 RSS 成功响应和未知标签 `no-store` 404。新增标签子协议时必须同时验证已知/未知 GET 与 HEAD 的最终生产响应。

下一轮唯一主任务：为专题详情增加 `/series/[slug]/rss.xml`。专题是有顺序的学习路径，Feed 应只包含该专题公开记录，并复用标签 RSS 的共享响应、页面可见/自动发现、未知专题 `no-store` 404、条件 GET/HEAD 和生产预算边界；实现前先决定 Feed 采用全站时间倒序还是专题章节顺序，并用产品语义测试锁定，不新增账号、数据库、追踪、第三方服务或云配置。
