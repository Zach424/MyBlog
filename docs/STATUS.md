...
| Markdown | done | GFM、代码高亮、语言标签、渐进增强的一键复制、与实际渲染一致的 H1–H6 heading id、H2/H3 目录与原生永久链接、Obsidian 兼容脚注/尾注、行内/块级数学公式、Callout、受限 Mermaid 服务端 SVG、受限本地静音 MP4、受限本地 MP3 + 完整文字稿、受限多图画廊、受限技术表格、受限只读任务清单、受限参考资料清单、受限步骤流程、受限术语定义表、受限 FAQ、受限项目文件树、受限项目时间线与受限技术决策记录、A4 打印/PDF 版式、阅读时间、相邻文章与响应式媒体 |
...
| 网页写作 | done | `/studio`、GitHub OAuth、Decap workflow、PR、按 slug 归档媒体、稳定 slug 锁定、双层 SHA-256 冲突预检、快速重选 latest-wins、共享生产管线的公式/Callout/Mermaid/视频/音频/画廊/技术表格/只读任务/参考资料/步骤流程/术语表/FAQ/项目文件树/项目时间线/技术决策预览、自定义本地 MP3/静音 MP4 组件、可排序画廊、结构化技术表格/任务清单/参考资料/步骤流程/术语表/FAQ/项目文件树/项目时间线/技术决策组件与全字段只读发布清单 |
| Obsidian 写作 | done | Vault、三类受信模板、桌面插件 1.53.0、文件名唯一草稿身份、图片/画廊/MP3/MP4 原子归档、画廊/技术表格/任务清单/音频/参考资料/步骤流程/术语表/FAQ/项目文件树/项目时间线/技术决策模板插入、source-scoped 作者意图与来源行导航、发布/复核 single-flight、三方版本联锁、bundle SHA-256 完整性、四路径 Git HEAD/index/worktree provenance、sealed Git 交付恢复、维护台账、全库生产同步、手动单篇收敛与正常/恢复交付自动接力 |
...
作者现在可以在 Studio 或 Obsidian 发布受约束技术决策记录。每条记录固定保存状态、真实日期、上下文、最终决定、理由、1–6 个备选方案与 1–6 个正/负/中性影响；每篇最多 3 条且备选与影响合计不超过 24 项。状态、区段顺序、段落预算、NFKC 唯一性、安全行内内容和未来日期由同一 AST 契约验证。Studio 提供状态/日期/台账表单与共享生产预览，Obsidian 1.53.0 提供一键模板；阅读端以真实 `<time datetime>` 和 Decision Brief 展示，搜索保留判断证据并删除机械标签，320 px 与打印保留完整取舍。
...
下一轮唯一主任务：建立受约束的技术实验记录块。用可迁移 Markdown 保存假设、环境、方法、测量结果、结论与局限，冻结指标/样本/长度预算、Studio/Obsidian 作者入口、搜索、窄屏和打印；不加入在线执行器、遥测采集、自动结论、统计显著性冒充或客户端状态。
