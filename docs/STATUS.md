...
| Obsidian 写作 | done | Vault、三类受信模板、桌面插件 1.23.0、文件名唯一草稿身份、本地原子新建/安全改名/旧身份取证与严格清理、source-scoped 当前草稿作者意图摘要、四个新发布/复核事务的 single-flight lease、阶段/输出活动脉冲、会话内最近终态回执与自动 doctor 联锁、13 项本机前置电路、统一只读 Git 交付分诊、版本化维护台账/Author Proof v3、两类安全重送/可信回执、deferred 并行草稿和新稿 `--check-only`/`--push` |
| Inbox 发布就绪 | done | version 1/read-only 全草稿 ready/scheduled/blocked、Article/TIL/Project、站内引用计数、真实媒体候选、目标/共享附件诊断、CLI 全库或 `--source` 聚焦 JSON 与 Obsidian 当前草稿原生摘要 |
...
- 本轮实现提交：`ee8b4e6e9917d17b5dfd5b432853b1c429dddbee`（source-scoped 当前草稿媒体证据）；
- 自动交付：实现提交已推送；远端 Quality、Vercel 验证与生产状态仍由 GitHub/Vercel 按提交独立报告；
- 最新完成迭代：0072 当前草稿 source-scoped 媒体证据；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md` 与 `docs/iterations/*.md` 可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

MyBlog Publisher 1.23.0 将“查看当前草稿发布意图”切换为 `content:inbox -- --format json --source <精确路径>`。readiness 仍逐篇执行正式 Markdown/frontmatter/链接/附件目标轻量解析，继续观察正式目标、附件缺失/目标/Git 跟踪和跨草稿共享源；只有被请求的 entry 调用真实 `prepareMediaForPublishing`，应用共享问题后再把报告裁成一条。默认未传 `--source` 时仍派生全部媒体并返回完整 version 1 报告。插件额外要求 JSON 恰好只有一个 entry，防止全库报告伪装成当前摘要；不存在或不安全来源失败关闭。失败优先选择性合同出现 4 个预期失败后达到 11/11，相关四文件回归 145/145；真实 doctor 13/13、插件 1.23.0。两遍成功完整门均通过 306/306、45 个构建页面、19/19 生产应用测试和生产依赖审计 0；归档门曾捕获公开项目 HTML 超过 100 KB，压缩公开摘要且保留 Vault 细节后恢复。
...
4. Current record 已有 Studio、Obsidian 与每周 Actions 三个只读视图；Obsidian 1.23.0 已闭合本地草稿创建、旧身份取证/严格清理、文件名改名和 source-scoped 当前草稿作者意图摘要，并继续保留四事务 owner-checked lease、活动脉冲、会话内终态回执、Author Proof v3、统一 Git 交付 switchyard、两类独立交付状态、精确重送与可信回执。作者意图页已有 DOM/CSS、ready/scheduled/blocked、完整媒体包络、schema/safety/计数漂移、单 entry 和活动文件漂移测试，但没有真实 Obsidian 宿主像素快照；聚焦模式仍轻量解析全部草稿并读取全部已发布内容作为链接目标，这是保留共享附件和正式链接正确性的明确成本，不再对无关附件生成媒体候选；带注释、引号、anchor/tag、缩进、重复键或不匹配值的旧 slug 故意保持只读；
...
下一轮唯一主任务：把当前草稿的 `LINKS` 从单一计数提升为可核对的精确站内目标清单，直接复用正式链接验证循环已经解析并验证的目标/锚点事实；必须覆盖文章、项目、自引用标题、重复出现与严格失败路径，不得二次扫描 Markdown、在插件中解析链接、改变公开关系语义、引入云服务/真实 API、自动发布或自动修复。
