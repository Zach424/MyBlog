...
| Obsidian 写作 | done | Vault、三类受信模板、桌面插件 1.31.0、文件名唯一草稿身份、本地原子新建/安全改名/旧身份取证与严格清理、source-scoped 当前草稿作者意图、原始来源字节 SHA-256 摘要/导航双重绑定、媒体 COVER/BODY 来源/逐次替代文本/作者或文件名回退取证/精确变换、ALT 与 LINK occurrence 精确源码行导航、四个新发布/复核事务的 single-flight lease、阶段/输出活动脉冲、会话内最近终态回执与自动 doctor 联锁、13 项本机前置电路、统一只读 Git 交付分诊、版本化维护台账/Author Proof v3、两类安全重送/可信回执、deferred 并行草稿和新稿 `--check-only`/`--push` |
...
- 本轮实现提交：`0b88a46adc1c92ab92ed9144d4847027c597c56a`（将当前草稿摘要和 ALT/REF 导航绑定到原始来源字节 SHA-256）；
- 自动交付：实现提交已推送；[Quality Gate #143](https://github.com/Zach424/MyBlog/actions/runs/31061516375) 与 [Verify Vercel production #136](https://github.com/Zach424/MyBlog/actions/runs/31061547281) 均成功，归档提交继续按同一链路独立验证；
- 最新完成迭代：0080 当前草稿来源字节 SHA-256 证据绑定；
- Obsidian 状态：仓库根目录就是 Vault，`docs/STATUS.md` 与 `docs/iterations/*.md` 可直接阅读和维护；
- 手动外部接入：自定义域名、统计、评论、公开邮箱均暂缓，不阻塞当前开发。

## 本轮新增能力

Inbox readiness 升至 version 6，并为每个成功读取的草稿记录原始文件字节 `sourceSha256`；不可读或符号链接条目保留 `null` 诊断，但 source-scoped Obsidian 消费者严格要求 64 位小写摘要。MyBlog Publisher 1.31.0 在摘要打开前重读当前 `TFile`，核对活动身份、Vault 映射和完整 SHA-256；每次 ALT/REF 导航再独立复核同一摘要，哪怕路径、对象与行号均未变化，只要正文任意字节变化就保留 Modal 并要求重跑。证据表只新增 `SOURCE / SHA-256 · <12 chars>`，比较仍使用完整 64 位值。实现不写内容、不自动重跑、不启动发布/Git、不联网。定向回归 199/199、完整 360/360、45 页构建、19/19 应用测试和生产依赖审计 0 均通过；真实 doctor 13/13 ready，真实 inbox v6 为空且全部安全声明为 false。
...
4. Current record 已有 Studio、Obsidian 与每周 Actions 三个只读视图；Obsidian 1.31.0 已闭合本地草稿创建、旧身份取证/严格清理、文件名改名、source-scoped 当前草稿作者意图、媒体用途/逐次 alt/来源/变换、ALT/LINK occurrence 源码行导航和两阶段来源 SHA-256 绑定，并继续保留四事务 owner-checked lease、活动脉冲、会话内终态回执、Author Proof v3、统一 Git 交付 switchyard、两类独立交付状态、精确重送与可信回执。作者意图页已有 DOM/CSS、键盘焦点、跨 ALT/REF 的重复点击单航班、ready/scheduled/blocked、媒体与链接证据，以及路径、`TFile`、摘要、异步读取、磁盘/Editor 行界、视图和打开失败测试，但没有真实 Obsidian 宿主像素/交互快照。摘要只是本地新鲜度证据而非签名；当前异步摘要验证也没有命令级 generation，作者连续运行两次检查或插件在读取期间卸载时，较旧完成项仍可能在较新结果之后打开 Modal，这是下一轮的生命周期主线。聚焦模式轻量解析全部草稿并读取全部已发布内容作为链接目标，是保留共享附件和正式链接正确性的明确成本；带注释、引号、anchor/tag、缩进、重复键或不匹配值的旧 slug 故意保持只读；
...
下一轮唯一主任务：给“查看当前草稿发布意图”的命令、报告回调和异步 SHA-256 复核增加插件级 latest-wins generation，并在 `onunload` 时失效全部未完成 generation；较旧或已卸载的完成项不得打开 Modal、发送成功 Notice 或触发重试。保持纯只读，不写正文、不取消或重启发布事务、不访问网络或云 API。
