...
`/subscribe` 把已经存在的五类读取能力变成普通读者和工具都能发现的服务端目录：RSS 面向阅读器，JSON Feed 面向结构化订阅器，OpenSearch 面向支持该协议的浏览器/搜索工具，内容清单与 Schema 面向同步器，单篇 Markdown 面向 Obsidian、离线阅读和引用工具。页面显示准确 MIME、端点模式和一小时 fresh/一天 SWR 更新语义，并用原生链接直达真实端点。

目录由 `createSubscriptionCatalog()` 生成，不维护第二份内容数据。前四条是受测试锁定的协议事实；Markdown 通道从公开记录复制排序后稳定选择最新内容作为可用示例，空集合则保留明确状态而不制造链接。页脚只链接 `/subscribe`，根布局仍保留 RSS、JSON Feed、清单和 OpenSearch 的机器发现声明；页面明确所有通道公开、只读、无账号，发布仍由 Studio/Obsidian 进入 Git 工作流。
...
`/content.json`、`/content.schema.json`、`/feed.json`、`/rss.xml`、`/sitemap.xml`、`/robots.txt` 与 `/opensearch.xml` 共用独立的确定性传输预算。Iteration 0111 以稳定生产功能提交 `5ab34a7` 在 2026-08-11 的完整响应重新确认基线来源；按固定顺序记录 raw UTF-8 与 Node zlib gzip 字节。raw 上限为 `baseline + max(50%, 4096 B)` 后按 1 KiB 取整，gzip 上限为 `baseline + max(50%, 1024 B)` 后按 512 B 取整。

预算检查不依赖 CDN 是否实际协商 gzip/Brotli；它用同一响应正文生成可复现的传输代理。本地真实 Next 测试使用较短的固定测试 origin，生产冒烟使用实际传入 origin，因此两者共享上限但各自报告真实字节。七个端点必须恰好覆盖一次，漏测、重复、意外端点、raw 超限或 gzip 超限都会失败。稳定生产基线依次为 3009/921、3278/755、20697/9876、3238/1241、4882/524、155/127、700/462 B（raw/gzip）；Sitemap 因新增 `/subscribe` 从 4703/512 B 增长到 4882/524 B。基线只能在确认增长属于有价值的产品变化、重新测量真实生产并记录来源提交后更新；不能为了让门变绿自动读取当前输出重置自己。
...
- URL：`/sitemap.xml`
- 包含：首页、文章、项目、时间档案、订阅目录、专题、标签、搜索、关于、全部详情与派生索引页
- `lastmod`：内容使用 `updatedAt` 或 `publishedAt`；集合使用其最新公开内容日期
- 缓存：1 小时 fresh，24 小时 stale-while-revalidate
- 验证器：最终 XML 字节的 SHA-256 ETag；条件命中返回空 304

当前公开内容生成 26 个 URL。Sitemap 不包含草稿、未来日期、查询参数或 JSON Feed/RSS/robots 端点本身。
