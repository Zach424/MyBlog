...
lib/
  content/                          内容契约、维护报告、文件读取、派生索引与引用关系
    author-doctor.ts                本机作者环境 legacy version 1 与含 bundle 摘要的 14 项 version 2 报告
...
收敛报告是 version 1、`mode: read-only` 的结构化证据，包含冻结目标、时限、每次观测、最终生产快照与固定安全声明。Obsidian 1.40.0 既允许活动正式笔记手动启动，也能从正常新内容发布/正式复核或两条 sealed recovery delivery 的可信 Git 成功结果自动接力；同一 scope 使用 generation 与子进程取消实现 latest-wins，插件卸载也终止在途等待。stderr 只接收带固定前缀的逐行进度，stdout 只接收最终 JSON；插件独立验证键集合、时间、状态、差异、ETag、来源路径和零写入声明后才显示无按钮 Modal。
...
Obsidian 1.40.0 在所有 Git writer 之前建立独立的插件版本握手：运行 bundle 的内嵌常量、Obsidian 实例上的 runtime manifest、Author Doctor 读取的磁盘 manifest 三者必须使用完整数字语义版本且完全相等。doctor 的插件检查由磁盘观察动态派生 expected/observed/resolution，因此未来 patch/minor 版本仍可被旧运行时代码结构化验证；报告字段或派生检查被伪造则继续失败关闭。版本漂移进入无动作的 `PLUGIN RELOAD REQUIRED` interlock，不自动 reload、不进入领域 Git；两条 recovery delivery 在 author transaction lease 外只执行这一版本预检，允许非版本 attention 继续恢复，但缺失或无法验证的磁盘版本身份同样阻止 Git 写入。

1.40.0 再用一个不自我哈希的 `bundle.json` 固定 `main.js`、`manifest.json`、`styles.css` 的顺序、插件身份和 SHA-256。生成器只在显式 `npm run plugin:bundle -- --write` 时重写 descriptor；默认命令只复算验证，构建测试也逐字节重算。Author Doctor 不带参数的 JSON 保留 13 项 version 1，确保仍运行的 1.39 可以识别未来磁盘版本并要求 reload；1.40 Git writer 固定追加 `--plugin-bundle`，消费含规范化 descriptor 状态、三文件 expected/observed 摘要与第 14 项 check 的 version 2。人类 text 默认使用 version 2。descriptor 缺失、未知字段、符号链接、局部更新或摘要漂移显示无按钮 `PLUGIN BUNDLE INVALID`；恢复路径只允许 bundle verified 的非版本 attention 旁路，不自动生成、覆盖或重载。
...
`lib/content/author-doctor.ts` 把本机基础发布前置条件冻结为 Runtime 3、Git 4、Workspace 4、Vault 2 共 13 项；bundle version 2 再增加一个 Vault check。`author-doctor-environment.mjs` 只读 `process.version`、受信 npm CLI、Git version/root/branch/upstream/local tracking、Git identity 是否配置、package scripts、全部声明依赖的已安装精确版本、五个固定作者路径，以及普通文件形式的插件 manifest/main/styles/bundle；姓名与邮箱值不会进入 observation。Windows 不通过 shell 执行 `npm.cmd`，而是用当前 Node 直接运行受信 `npm-cli.js`。同步基线复用本地 delivery snapshot，不访问远端，也不要求工作区为空。`report-author-doctor.mjs` 不带 bundle 参数的 JSON 保留 version 1；显式 `--plugin-bundle` 与默认 text 使用 version 2，ready 返回 0、attention 返回 1；safety 固定证明零安装、零配置/文件变更、零凭据读取与零网络。1.40.0 parser 从 observation/pluginBundle 独立重算 14 项状态、summary 与 repair，再用 preflight circuit 展示；四个作者事务和两条 recovery preflight 固定请求 version 2。结构异常只降级为纯文本并失败关闭；版本漂移与 bundle 失败分别显示无动作的 reload/bundle interlock，doctor 致命退出同样不进入领域命令。
