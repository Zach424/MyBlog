...
发布前，作者可以从 Obsidian 命令面板运行“查看全部草稿发布就绪状态”；维护既有内容时运行“查看已发布内容复核台账”。插件 1.4.0 以固定参数数组和 `shell: false` 启动本地 `content:inbox` 或版本化 `content:status --format json`，Windows 通过隐藏 `cmd.exe` 执行 npm，POSIX 直接执行 npm。维护 JSON 在 CommonJS 插件边界严格验证精确字段、ISO 日期及日差、四档计数/阈值、确定性顺序、公开 URL 和 `content/posts|projects/<slug>.md`；仅当 Vault 返回同路径 Markdown 文件时才交给原生 leaf 打开。逾期状态允许 CLI 退出码 1；无效 JSON/schema/path 或 Modal 渲染异常会重新运行纯文本报告，并继续用 `setText` 隔离诊断 HTML。输出最多捕获 200,000 字符。所有命令共用活动进程账本：成功、非零退出、spawn error 都隐藏持续 Notice 并只结算一次，插件卸载会隐藏 Notice、忽略迟到事件，并在 Windows 以固定参数、无 shell 的 `taskkill.exe /T /F` 终止命令进程树（启动失败回退直接 kill），POSIX 直接终止子进程。两种报告都只读且零网络；单篇“检查当前草稿”与“发布当前草稿并同步 GitHub”保持原语义，完整 `npm run check` 仍是最终权威门禁。

`lib/content/review-note.ts` 把正式复核定义为 HEAD 快照与当前文件之间的领域转换：路径必须是稳定正式文件，前后都已公开且为 Current，`publishedAt` 不变，`reviewedAt` 必须严格推进到上海当天；去除 reviewedAt/updatedAt 和派生字数后若语义快照变化，当前 `updatedAt` 必须是当天，否则只能保持旧 updatedAt。`scripts/review-note.mjs` 在领域检查外增加 Git 事务边界：只允许 `main`、已跟踪目标、空暂存区、零未跟踪文件和唯一目标修改；完整 `npm run check` 前后各复查一次状态。`--check-only` 零 Git 副作用；`--push` 用 pathspec 暂存并以 `git commit --only` 提交单文件，提交后再核对 tree 路径后推送。门或 commit 失败不改 HEAD并取消暂存；push 失败保留可恢复的本地合法提交。插件 1.4.0 只在活动正式 Markdown 上暴露对应两条命令，并复用既有进程/Notice 生命周期。
