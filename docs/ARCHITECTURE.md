...
发布前，作者可以从 Obsidian 命令面板运行“查看全部草稿发布就绪状态”；维护既有内容时运行“查看已发布内容复核台账”。插件 1.11.0 以固定参数数组和 `shell: false` 启动本地 `content:inbox`、版本化 `content:status --format json`、`content:review:status --format json`、`content:publish:status --format json`，或两个独立的 `content:review:deliver --format json` / `content:publish:deliver --format json`，Windows 通过隐藏 `cmd.exe` 执行 npm，POSIX 直接执行 npm。维护、状态与回执 JSON 在 CommonJS 插件边界严格验证精确字段、日期或 Git object identity、确定性顺序和安全来源路径；仅当 Vault 返回同路径 Markdown 文件时才交给原生 leaf 打开。逾期/待交付状态允许 CLI 退出码 1；无效状态 JSON/schema/path 或 Modal 渲染异常会重新运行对应纯文本报告；不可信的成功回执则失败关闭，绝不自动再次 push。输出最多捕获 200,000 字符。所有命令共用活动进程账本：成功、非零退出、spawn error 都隐藏持续 Notice 并只结算一次，插件卸载会隐藏 Notice、忽略迟到事件，并在 Windows 以固定参数、无 shell 的 `taskkill.exe /T /F` 终止命令进程树（启动失败回退直接 kill），POSIX 直接终止子进程。状态报告都只读；deliver 是明确命名的 Git 写动作，单篇“检查当前草稿”与“发布当前草稿并同步 GitHub”保持原语义，完整 `npm run check` 仍是最终权威门禁。
