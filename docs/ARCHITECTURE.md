...
`lib/content/inbox-readiness.ts` 在作者工作区逐篇隔离检查直接位于 `content/inbox` 的 Markdown：复用真实发布器完成类型/slug/frontmatter/站内链接与附件目标派生，再按报告 scope 决定哪些附件交给同一媒体发布策略，在系统临时目录生成并校验实际候选产物。默认 scope 对全部草稿派生，保持既有全库报告；`sourcePath` scope 仍轻量解析全部草稿并交叉检查正式目标、附件目标、Git 跟踪附件和多草稿共享源，但只有精确目标执行真实媒体派生，应用共享问题后才裁成单 entry 报告。一个坏草稿不会阻止其他草稿产生证据。状态分为现在可发布的 `ready`、可以提交但尚未到公开日的 `scheduled` 和需要处理诊断的 `blocked`。version 2 JSON 固定 `mode: read-only`，并证明作者文件、commit、push 与网络均未变化；entry 同时保留 Article/TIL/Project、精确站内目标与出现证据、正式目标和媒体候选。临时目录在成功和失败路径都删除，作者 Markdown 与附件不写入；`scripts/report-inbox-readiness.mjs` 提供文本/JSON 和安全的 `--source content/inbox/<slug>.md`，Obsidian 桌面插件保留全库文本 Modal，并用聚焦 JSON 生成当前草稿作者意图摘要。它不接入 Actions，因为未跟踪本地草稿对 CI 天然不可见。

`lib/content/markdown.ts` 的一次 GFM + math AST 遍历按既有精确引用 key 聚合 `occurrences` 与 `sourceLines`；兼容入口再投影回原 `InternalContentReference`，因此公开关系派生没有第二次解析或语义变化。`validatePreparedContentLinks` 在同一目标/标题锚点验证循环中把自链接解析为自己的最终公开路径，并返回 `post/project/self + target + sourceLines + occurrences`。readiness 只透传这份结果；Obsidian 1.24.0 只验证 version 2 schema、计数、路径类型、自链接身份、顺序与唯一目标，然后渲染文本 trace，不解析 Markdown。
