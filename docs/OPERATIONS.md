...
必须验证：首页、集合、文章、项目、时间档案、内容活动、订阅目录、知识地图、搜索、OpenSearch、公开内容清单及 Schema、JSON Feed、根/标签/专题 RSS、Atom、OPML、robots、Sitemap 全部 URL、文章/项目 Markdown 源文、Studio HTML/配置/媒体清单/媒体预检/稳定 slug 控件/预览/固定版本运行时、OAuth 跳转、安全头、缓存和真实 404；清单、JSON Feed 与根 RSS 要保持同一公开 URL 顺序，根页面必须包含实际生产 origin 的清单/Feed/Atom/OpenSearch 发现链接，代表标签/专题页必须包含同 origin alternate 和可见 RSS 链接。OpenSearch 必须验证 1.1 namespace、唯一必填元素、同源 results/self URL、安全 `{searchTerms}` 模板、示例查询、语言/编码、MIME、文件名、`noindex`、SHA-256 ETag、等价缓存与空 304，且不进入 Sitemap。清单必须验证 version 1、字段 allowlist、绝对 HTML/Markdown URL、每项强 SHA-256 `markdown_etag`、全部真实源文强弱等价、MIME、文件名、self/describedby/up Link、`noindex`、Last-Modified、Vercel 等价缓存、ETag/日期条件空 304、旧/坏日期 200 与 ETag 优先；Schema 必须验证 Draft 2020-12、同源 `$id`、结构关键字、MIME、文件名、self/describes/up Link、`noindex`、SHA-256 ETag、等价缓存与空 304。RSS 还必须声明准确的 Dublin Core Terms 命名空间；每个 item 的 `pubDate` 必须继续等于 JSON Feed `date_published`，只有 `date_modified > date_published` 时才允许恰好一个同值的 `dcterms:modified`，不得出现 RSS item 级 `atom:updated`；逐 item 的 category 必须在 XML 解码后与对应 JSON Feed tags 的数量、顺序和值完全一致，内容类型不得混入无 domain 分类。标签 RSS 还必须只含同名 tag 的 JSON Feed 子集；专题 RSS 必须只含对应章节集合，HTML 保持章节升序而 Feed 保持首发时间倒序。两类 scoped Feed 的 channel home/self、文件名、self/up Link 与 `noindex` 都必须正确，且不进入 Sitemap；未知标签/专题 GET/HEAD 必须是无 ETag/Last-Modified 的 `no-store` 404。Atom 必须使用 `http://www.w3.org/2005/Atom` namespace，feed 只有一个 title/id/updated 和 author，每个 entry 只有一个 title/id/published/updated，id/alternate 指向同一规范内容 URL；entry 顺序必须等于 JSON Feed 按 `date_modified ?? date_published`、首发日、中文标题与 URL 决胜后的结果，summary/content/category 必须与 description、纯文本正文和 tags 对齐。首页 alternate、MIME、内联文件名、self/alternate Link、`noindex`、正文 SHA-256 ETag、Last-Modified、条件 GET/HEAD 与 Sitemap 排除都必须成立。OPML 必须使用 version 2.0，根/标签/专题分组顺序稳定，所有 leaf 具备必填 `text` 和订阅 `type`/`xmlUrl`，绝对 URL 集合精确等于根 RSS 加 Sitemap 推导的全部公开标签/专题 Feed，并明确不含 Atom；MIME、下载文件名、self/up Link、`noindex`、正文 SHA-256 ETag、条件 GET/HEAD 与 Sitemap 排除都必须成立，并且不得出现无法证明的 OPML 日期或 Last-Modified。JSON Feed、RSS 与 Atom 必须分别返回 `Thu, 06 Aug 2026 10:09:53 GMT`、`Mon, 10 Aug 2026 22:25:11 GMT`、`Tue, 11 Aug 2026 00:13:39 GMT` 的 Last-Modified；同值日期条件返回空 304，旧日期、非 HTTP-date 返回完整 200，陈旧或畸形 `If-None-Match` 存在时不得再用日期命中。JSON Feed、根/标签/专题 RSS、Atom、OPML、Sitemap、robots 与 OpenSearch 还必须验证最终正文 SHA-256 ETag，以及携带源响应标签时同 digest、同缓存策略、空正文的 304；除 robots 外均保留一小时 fresh，robots 保留一天 fresh。源文必须验证详情页 alternate/可见入口、公开 frontmatter、绝对链接/媒体、MIME、文件名、canonical、`noindex`、Vercel 等价缓存、SHA-256 ETag、Last-Modified、ETag/日期条件空 304、旧/坏日期 200、ETag 优先，以及未知 slug 的 `no-store` 404。生产响应允许 Brotli CDN 把强标签弱化并精简 304 metadata，但强/弱 opaque digest 必须相同，Cache-Control 必须安全，仍存在的日期/链接/robots 不能漂移。命令会为十三条关键 HTML 路由和十一个结构化发现端点输出实际生产域名下的 raw/gzip、阈值、基线与余量，任何漏测、重复或超限都会失败。首次上线或域名切换还需用未登录浏览器覆盖桌面、390px、深色和键盘路径；域名变化后必须重新核对 manifest/schema/Feed/Atom/OPML/OpenSearch/source URL、验证器、预算 origin 与真实生产基线，不能沿用替代主机的假绿。
...
Iteration 0121 起，RSS 根元素必须同时声明 `xmlns:atom` 与 `xmlns:dcterms="http://purl.org/dc/terms/"`。逐 item 核对 `guid`/顺序与 JSON Feed `id` 一致、`pubDate` 等于原始 `date_published`；只有严格更晚的 `date_modified` 才输出一个 `dcterms:modified`，同日或缺失更新不得输出，且任何 item 都不得出现 `atom:updated`。功能提交 `97eabce` 的稳定生产 XML 可被标准 XML 解析器读取，共 4 个 item、4 个修改时间、0 个 Atom updated；RSS 为 3536/1298 B（raw/gzip）。预算提交 `52d6a18` 以同一部署统一重测七个发现端点；完整 smoke 为 27 routes、OAuth 302。

Iteration 0122 起，JSON Feed 与 RSS 必须声明由各自表示修订时间和最新公开内容共同派生的 Last-Modified，并支持 `If-Modified-Since`。接收端兼容三种标准 HTTP-date，拒绝日历/星期错误、重复值和非 HTTP 日期；任何 `If-None-Match` 存在时日期条件必须完全失效。功能提交 `237fd8d` 的稳定生产验证了精确日期、日期命中空 304、旧/非法日期完整 200、陈旧 ETag 优先返回 200，以及 Vercel 可选精简 304 元数据；完整 smoke 为 27 routes、OAuth 302。Feed 正文与 ETag 均未改变，因此七端点仍使用 `97eabce` 生产 body 基线，不创建响应头专用重基线。
...
Iteration 0125 起，生产 smoke 必须把每个 RSS item 与同位置 JSON Feed item 配对，在解码 XML 实体后要求 categories 与 tags 数量、顺序和值相同；`article`、`til` 与 `Project` 不得作为无 domain category。RSS Last-Modified 固定为 `Mon, 10 Aug 2026 22:25:11 GMT`，既有 ETag、日期、GET/HEAD 和 304 矩阵继续执行。功能提交 `f9bd0d0` 的稳定生产 RSS 为 3400/1284 B（raw/gzip），完整 smoke 为 27 routes、OAuth 302；预算提交 `0b1f81d` 以同一生产版本更新七端点来源与 RSS 实测值，其余六端点不变。

Iteration 0126 起，生产 smoke 还要验证 `/tags/typescript` 的可见/alternate 发现与 `/tags/typescript/rss.xml` 的精确 JSON Feed 标签子集、channel home/self、MIME、文件名、self/up Link、`noindex`、Last-Modified、ETag、日期/ETag GET 和完整 HEAD 矩阵；未知标签 GET/HEAD 必须 `no-store` 且无验证器。Feed 明确不进入 Sitemap。功能提交 `d4e26b8` 的稳定生产标签 Feed 有 2 个 item、2059/923 B（raw/gzip），完整 smoke 为 27 routes、OAuth 302；预算提交 `4bda63a` 将它作为第八条端点并把来源绑定已部署功能提交。

Iteration 0127 起，同一 smoke 还要验证 `/series/build-my-blog` 的可见章节严格为第 1→2 章，同时 `/series/build-my-blog/rss.xml` 的 GUID 严格为最新发布第 2→1 章。专题 Feed 必须只包含对应 JSON Feed 记录，并核对 channel home/self、MIME、文件名、self/up Link、`noindex`、Last-Modified、正文 SHA-256 ETag、日期/ETag GET 与完整 HEAD 矩阵；未知专题 GET/HEAD 必须 `no-store` 且无验证器，Feed 不进入 Sitemap。功能提交 `cf3c631` 的稳定生产专题 Feed 有 2 个 item、2065/983 B（raw/gzip），完整 smoke 为 27 routes、OAuth 302；预算提交 `3982113` 将它作为第九条端点并把来源绑定已部署功能提交。`/series` HTML 缓存规则只能精确匹配索引/详情，不能恢复为 `/series/:path*`。

Iteration 0128 起，同一 smoke 还要验证 `/subscribe` 可见六条只读通道，并解析 `/feeds.opml`：根组必须先于标签和专题组，当前精确包含 13 个唯一订阅 leaf；每项都要有标准必填属性、同源绝对 `xmlUrl` 与对应 `htmlUrl`，标签/专题集合从 Sitemap 公开页面反推，不能维护第二份固定数量清单。响应必须是 `text/x-opml; charset=utf-8`、安全附件名、self/up Link、`noindex`、一小时缓存、最终正文 SHA-256 ETag 与空 304；普通/条件 HEAD 必须零正文；不得返回 Last-Modified 或 OPML 日期，也不得进入 Sitemap。功能提交 `d0f2165` 的稳定生产 OPML 为 5193/962 B、SHA-256 `6344a8a82167b1d35b3d55e9f3679dc43cca9b72c31823d74ac8d1e72b7c7ce8`，完整 smoke 为 27 routes、OAuth 302；预算提交 `e33f395` 将它作为第十条端点并把来源绑定已部署功能提交。

Iteration 0129 起，同一 smoke 还要验证 `/subscribe` 可见七条只读通道，并解析 `/updates.atom`：Atom namespace、feed/entry 必填元素、唯一 id、self/alternate、首次发布与真实更新时间、按变更排序、纯文本 summary/content、与 JSON Feed 一致的 category 都必须成立。首页必须自动发现 Atom；OPML 和 Sitemap 必须排除 Atom，避免默认双订阅。响应必须是 `application/atom+xml; charset=utf-8`、安全内联文件名、self/alternate Link、`noindex`、一小时缓存、最终正文 SHA-256 ETag、`Tue, 11 Aug 2026 00:13:39 GMT` Last-Modified、日期/摘要条件 304 与普通/条件 HEAD 零正文。功能提交 `7e5909f` 的稳定生产 Atom 为 21338/10063 B、SHA-256 `a9564d43d1b2fc04cf77812431837c3a1820d2ccdf37269f6bf9278da823671f`，完整 smoke 为 27 routes、OAuth 302；预算提交 `fd359c1` 将它作为第十一个端点并把来源绑定已部署功能提交。
