...
- 目标托管：Vercel Git 自动部署；
- 当前回退站：`https://zach424-engineering-notes.zhiqingchen792.chatgpt.site`，迁移验收前保持在线；
...
1. 用 GitHub 登录 Vercel，导入 `Zach424/MyBlog`；
2. Framework Preset 选择 Next.js，Production Branch 选择 `main`，其余构建设置使用仓库默认值；
3. 在 Vercel Production 环境保存 `GITHUB_OAUTH_ID` 与 `GITHUB_OAUTH_SECRET`；
4. GitHub OAuth App 的 Homepage URL 使用稳定生产 origin，Authorization callback URL 使用 `<origin>/api/cms/callback?provider=github`；
5. 在 GitHub Actions variable 保存 `VERCEL_PRODUCTION_URL`；如启用仓库回滚工作流，再保存 `VERCEL_TOKEN`、`VERCEL_ORG_ID`、`VERCEL_PROJECT_ID` secrets；
6. 生产冒烟、未登录浏览器、Studio 登录/草稿/发布和 Obsidian 发布全部通过后，才切换 README、自定义域名或书签。
...
3. 草稿阶段保持 `draft: true`；
4. 本地方式运行 `npm run content:publish -- <note>`；网页方式使用 editorial workflow；
...
必须验证：首页、集合、文章、项目、搜索、RSS、robots、Sitemap 全部 URL、Studio 三个资源、OAuth 跳转、安全头、缓存和真实 404。首次上线或域名切换还需用未登录浏览器覆盖桌面、320px、深色和键盘路径。
