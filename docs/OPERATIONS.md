...
在本机 Vault 中也可从命令面板运行“查看已发布内容复核台账”。插件 1.5.0 先启动本地 `content:status --format json`，严格核对版本、计数、日期计算、状态阈值、排序、公开路由和精确来源路径，再显示四档期限轨迹与逐条记录；“打开笔记”只接受 Vault 中真实存在的 `content/posts|projects/<slug>.md`。报告命令因已过期内容返回退出码 1 时仍会解析并展示有效 JSON；JSON、schema、路径或 Modal 渲染异常时，插件会再运行纯文本 `content:status` 并以安全文本节点显示证据。两条路径都零网络、零日期/文件写入。Windows 报告子进程隐藏运行且不插入动态 shell 字符串，插件卸载时以固定 `taskkill.exe /T /F` 参数清理整棵命令进程树，POSIX 直接终止子进程。持续 Notice 若没有在命令终态消失，或插件关闭后仍存在 npm 进程，视为插件生命周期故障；笔记不存在时先确认文件已同步且路径大小写正确，再在终端运行 `npm run content:status` 取证。插件文件更新后，已打开的 Obsidian 需要重启或重新启用插件才能加载新版本。

正式内容复核使用 `npm run content:review -- content/posts|projects/<slug>.md --check-only`，确认后把最后一个参数改为 `--push`。命令只接受 `main` 上已经公开的 Current 文件：`publishedAt` 不得改变，`reviewedAt` 必须从 HEAD 中的旧日期推进到 `Asia/Shanghai` 当天；仅推进复核日时 `updatedAt` 保持原值，正文或元数据有事实变化时 `updatedAt` 必须也是当天。执行前暂存区必须为空，且工作区除当前文件外不能有已修改或未跟踪文件。两种模式都运行完整 `npm run check`；check-only 不触碰 Git，push 只暂存/提交当前文件并推送 `origin main`。MyBlog Publisher 1.5.0 的 check-only 会请求 `--format json`：CLI 最多内部捕获 5,000,000 字符质量日志，成功 stdout 只保留版本化 Proof；插件严格复核日期、事实变化、Git 数组与 passed 状态后展示只读 Modal。结构或 UI 异常会重新执行普通文本 check-only；命令本身失败则直接显示诊断，不伪造 Proof。门失败时文件保持未暂存；commit 前失败会自动取消暂存；push 失败时合法本地提交会保留，修复连接后运行 `git push origin main`，不要重做日期或强制重置。
