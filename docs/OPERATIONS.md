...
9. 打开首页、文章、`/subscribe`、代表标签/专题页、公开内容清单、清单 Schema、JSON Feed、根/标签/专题 RSS、OPML、Sitemap 和 OpenSearch，确认新内容与公开读取通道可见且绝对 URL 指向当前生产域名；首页 `Guest · <n> public URLs · Sitemap synced` 的数字必须等于 Sitemap `<loc>` 数量，首页 `LATEST` 必须等于 Sitemap 根 URL 的 `lastmod`，且不能再出现手写 `REV. <数字>`；清单中的 `markdown_etag` 应与对应源文响应一致，`/content.json` 与 `/content.schema.json` 应通过 describedby/describes Link 双向关联，十个结构化端点应带最终正文 SHA-256 ETag 并支持空正文 304，首页应声明绝对 OpenSearch `rel="search"`，标签/专题页应声明并显示自己的 RSS，OPML 应包含当前全部公开 RSS 地址且不进入 Sitemap。
...
必须验证：首页、集合、文章、项目、时间档案、内容活动、订阅目录、知识地图、搜索、OpenSearch、公开内容清单及 Schema、JSON Feed、根/标签/专题 RSS、OPML、robots、Sitemap 全部 URL、文章/项目 Markdown 源文、Studio HTML/配置/媒体清单/媒体预检/稳定 slug 控件/预览/固定版本运行时、OAuth 跳转、安全头、缓存和真实 404；清单、JSON Feed 与根 RSS 要保持同一公开 URL 顺序，根页面必须包含实际生产 origin 的清单/Feed/OpenSearch 发现链接，代表标签/专题页必须包含同 origin alternate 和可见 RSS 链接。OpenSearch 必须验证 1.1 namespace、唯一必填元素、同源 results/self URL、安全 `{searchTerms}` 模板、示例查询、语言/编码、MIME、文件名、`noindex`、SHA-256 ETag、等价缓存与空 304，且不进入 Sitemap。清单必须验证 version 1、字段 allowlist、绝对 HTML/Markdown URL、每项强 SHA-256 `markdown_etag`、全部真实源文强弱等价、MIME、文件名、self/describedby/up Link、`noindex`、Last-Modified、Vercel 等价缓存、ETag/日期条件空 304、旧/坏日期 200 与 ETag 优先；Schema 必须验证 Draft 2020-12、同源 `$id`、结构关键字、MIME、文件名、self/describes/up Link、`noindex`、SHA-256 ETag、等价缓存与空 304。RSS 还必须声明准确的 Dublin Core Terms 命名空间；每个 item 的 `pubDate` 必须继续等于 JSON Feed `date_published`，只有 `date_modified > date_published` 时才允许恰好一个同值的 `dcterms:modified`，不得出现 RSS item 级 `atom:updated`；逐 item 的 category 必须在 XML 解码后与对应 JSON Feed tags 的数量、顺序和值完全一致，内容类型不得混入无 domain 分类。标签 RSS 还必须只含同名 tag 的 JSON Feed 子集；专题 RSS 必须只含对应章节集合，HTML 保持章节升序而 Feed 保持首发时间倒序。两类 scoped Feed 的 channel home/self、文件名、self/up Link 与 `noindex` 都必须正确，且不进入 Sitemap；未知标签/专题 GET/HEAD 必须是无 ETag/Last-Modified 的 `no-store` 404。OPML 必须使用 version 2.0，根/标签/专题分组顺序稳定，所有 leaf 具备必填 `text` 和订阅 `type`/`xmlUrl`，绝对 URL 集合精确等于根 RSS 加 Sitemap 推导的全部公开标签/专题 Feed；MIME、下载文件名、self/up Link、`noindex`、正文 SHA-256 ETag、条件 GET/HEAD 与 Sitemap 排除都必须成立，并且不得出现无法证明的 OPML 日期或 Last-Modified。JSON Feed 与 RSS 必须分别返回 `Thu, 06 Aug 2026 10:09:53 GMT`、`Mon, 10 Aug 2026 22:25:11 GMT` 的 Last-Modified；同值日期条件返回空 304，旧日期、非 HTTP-date 返回完整 200，陈旧或畸形 `If-None-Match` 存在时不得再用日期命中。JSON Feed、根/标签/专题 RSS、OPML、Sitemap、robots 与 OpenSearch 还必须验证最终正文 SHA-256 ETag，以及携带源响应标签时同 digest、同缓存策略、空正文的 304；除 robots 外均保留一小时 fresh，robots 保留一天 fresh。源文必须验证详情页 alternate/可见入口、公开 frontmatter、绝对链接/媒体、MIME、文件名、canonical、`noindex`、Vercel 等价缓存、SHA-256 ETag、Last-Modified、ETag/日期条件空 304、旧/坏日期 200、ETag 优先，以及未知 slug 的 `no-store` 404。生产响应允许 Brotli CDN 把强标签弱化并精简 304 metadata，但强/弱 opaque digest 必须相同，Cache-Control 必须安全，仍存在的日期/链接/robots 不能漂移。命令会为十三条关键 HTML 路由和十个结构化发现端点输出实际生产域名下的 raw/gzip、阈值、基线与余量，任何漏测、重复或超限都会失败。首次上线或域名切换还需用未登录浏览器覆盖桌面、390px、深色和键盘路径；域名变化后必须重新核对 manifest/schema/Feed/OPML/OpenSearch/source URL、验证器、预算 origin 与真实生产基线，不能沿用替代主机的假绿。
...
同一命令还会为清单、Schema、JSON Feed、根 RSS、代表标签 RSS、代表专题 RSS、OPML、Sitemap、robots 与 OpenSearch 输出 `[discovery-budget]` 十行 raw/gzip 证据。任一路由缺失、重复、超出自己的冻结上限都会阻止冒烟；不要通过删除检查或自动采用当前输出解决失败。若增长是一次有意的内容/协议变化，先核对实际正文与公开集合，再在稳定生产重新测量十端点，同时更新基线数值、日期、来源提交、测试和迭代归档。域名变化会改变绝对 URL 和正文大小，必须以新稳定 origin 重新建立有来源基线。
