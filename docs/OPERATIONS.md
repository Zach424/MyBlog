...
新内容发布的 push 失败与正式复核分开处理。运行 `npm run content:publish:status`，或在 Obsidian 运行“查看待同步新内容发布”；报告只读本地 refs 与 HEAD commit，不 fetch、不 push、不写文件。只有 ahead 1 / behind 0，subject 为 `content: publish <slug>`，父级等于 tracking head，且 raw diff 精确等于一个新增正式 Markdown、可选同 slug inbox 删除和零到多个同 slug 归档媒体新增，才显示 `PUBLICATION HOLD / ATOMIC BUNDLE`、完整 Commit Envelope 与 `git push origin <verified-oid>:refs/heads/main`。普通 ahead、复核提交、额外路径、修改旧媒体和多提交堆叠都不给恢复命令。当前 1.10.0 只展示证据，不代为执行；不要手工恢复草稿或再次运行发布器。任何非 synchronized 状态也会在发布器读取源草稿前阻止第二次 `--push`。
