...
| 7. 持续内容与作者体验 | in progress | Iteration 0127 已让每个公开专题拥有可发现、可条件读取且排序语义明确的独立 RSS |

## 当前唯一主线

进入持续内容与作者体验阶段。Iteration 0127 把专题级订阅落到 `/series/[slug]/rss.xml`：专题 HTML 保持 `series.order` 章节升序，Feed 复用共享 RSS 边界并按首发时间倒序，让新增或插入章节优先进入阅读器。专题页提供可见和 alternate 发现，未知专题保持无验证器 `no-store` 404，Feed 不进入 Sitemap；`/series/:path*` 同样收窄为精确 HTML 页面规则。功能提交 `cf3c631` 已进入稳定生产；完整证据为 532 项单元测试、64 个生成页面、33 项应用测试、27 routes 与 OAuth 302。代表专题 Feed 为 2 items、2065/983 B（raw/gzip），预算提交 `3982113` 已把第九条发现端点绑定真实部署。下一主线是 OPML 2.0 聚合导出，把已经分散的全站、标签和专题 Feed 变成一次导入的可移植订阅包。首次真实 Obsidian 人机验收保留为所有者可执行事项，需要品牌域名时再绑定自定义域名。
...
- 当前结构化生产 raw/gzip 基线为：清单 3009/921 B、Schema 3278/755 B、JSON Feed 20697/9876 B、根 RSS 3400/1284 B、代表标签 RSS 2059/923 B、代表专题 RSS 2065/983 B、Sitemap 5059/532 B、robots 155/127 B、OpenSearch 700/462 B；逐端点推导上限和覆盖门已闭环。0127 的新专题投影已在稳定生产重测，并把九端点基线来源绑定到功能提交 `cf3c631`；后续基线仍必须伴随产品价值复核、真实生产重测、来源提交和归档，不能自动跟随输出自我放行；
...
- 九个结构化端点与单篇 Markdown 的 GET/HEAD 条件语义已经闭环；所有 HEAD 200/304 零正文，验证器、缓存及存在的表示元数据与 GET 等价，未知源文、标签 RSS 和专题 RSS 保持无验证器 `no-store` 404。该能力依赖 Next 发送边界而非十一套显式 HEAD 导出，框架升级必须重跑真实构建与生产门；Vercel 对部分压缩 200 响应使用弱标签、对 304 精简表示元数据，生产门按 HTTP 等价语义验收；
