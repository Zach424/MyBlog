...
| 7. 持续内容与作者体验 | in progress | Iteration 0111 发布订阅与开放接口目录，并把生产预算扩展到十一条 HTML 路由 |

## 当前唯一主线

进入持续内容与作者体验阶段。Iteration 0111 已发布服务端 `/subscribe`：RSS、JSON Feed、OpenSearch、公开清单/Schema 与单篇 Markdown 从同一 Markdown/Git 来源形成五通道路由表，页脚、Sitemap、真实 SSR、390px 深色、打印契约和生产预算均已闭环；功能提交 `5ab34a7` 与基线提交 `8c39241` 已通过本地/线上门。下一主线是设计自定义 `not-found` 恢复页，把未知 URL 明确导回搜索、时间档案、文章和项目，同时保持 no-store、单个 H1、无客户端请求、响应式/深浅色/打印与真实 404 状态；不引入推荐算法、数据库或外部服务。首次真实 Obsidian 人机验收继续保留为所有者可执行事项，需要品牌域名时再绑定自定义域名。
...
- `/archive` 当前只有一个年份和月份，但契约已经覆盖跨年、跨月、同日决胜、空集合和输入不变。不要为了当前四条内容提前增加客户端筛选、分页或年份导航；先让内容增长和十一路 HTML 预算提供真实压力，再决定增强方式；
