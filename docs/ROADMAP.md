...
| 7. 持续内容与作者体验 | in progress | Iteration 0125 已让 RSS category 与 JSON Feed tags 共用同一标签事实源 |

## 当前唯一主线

进入持续内容与作者体验阶段。Iteration 0125 审计发现 RSS 已有 category，但旧实现把 `article`/`Project` 与作者 tags 混在同一个无 domain 分类轴；失败优先测试以 7/8 证明漂移。功能提交 `f9bd0d0` 改为逐 item 只序列化 `record.tags`，并在单元、真实 Next 和稳定生产中与 JSON Feed tags 对齐、覆盖 XML 转义和既有 GET/HEAD 条件语义；完整证据为 530 项单元测试、52 页构建、31 项应用测试、27 routes 与 OAuth 302。RSS 生产体积从 3536/1298 降到 3400/1284 B（raw/gzip），预算提交 `0b1f81d` 已绑定真实部署。下一主线转向主题级订阅：为标签页提供只含该标签公开记录的 RSS，并复用全站 Feed 的排序、标签、验证器和缓存边界。首次真实 Obsidian 人机验收保留为所有者可执行事项，需要品牌域名时再绑定自定义域名。
...
- 当前结构化生产 raw/gzip 基线为：清单 3009/921 B、Schema 3278/755 B、JSON Feed 20697/9876 B、RSS 3400/1284 B、Sitemap 5059/532 B、robots 155/127 B、OpenSearch 700/462 B；逐端点推导上限和覆盖门已闭环。0125 的 RSS 正文变化已在稳定生产重测，并把基线来源绑定到功能提交 `f9bd0d0`；后续基线仍必须伴随产品价值复核、真实生产重测、来源提交和归档，不能自动跟随输出自我放行；
