...
| 7. 持续内容与作者体验 | in progress | Iteration 0126 已让每个公开标签拥有可发现、可条件读取的独立 RSS |

## 当前唯一主线

进入持续内容与作者体验阶段。Iteration 0126 把主题级订阅落到 `/tags/[slug]/rss.xml`：根/标签 RSS 共享 item 序列化与 HTTP 条件响应，标签页提供可见和 alternate 发现，未知标签保持无验证器 `no-store` 404，Feed 不进入 Sitemap。两次真实应用失败证明 next.config 的 `/tags/:path*` 会分别污染成功与错误缓存，最终通过收窄 HTML 路径所有权解决。功能提交 `d4e26b8` 已进入稳定生产；完整证据为 531 项单元测试、63 个生成页面、32 项应用测试、27 routes 与 OAuth 302。代表 TypeScript Feed 为 2 items、2059/923 B（raw/gzip），预算提交 `4bda63a` 已把第八条发现端点绑定真实部署。下一主线转向有顺序的专题订阅，先明确时间倒序与章节顺序的产品语义。首次真实 Obsidian 人机验收保留为所有者可执行事项，需要品牌域名时再绑定自定义域名。
...
- 当前结构化生产 raw/gzip 基线为：清单 3009/921 B、Schema 3278/755 B、JSON Feed 20697/9876 B、根 RSS 3400/1284 B、代表标签 RSS 2059/923 B、Sitemap 5059/532 B、robots 155/127 B、OpenSearch 700/462 B；逐端点推导上限和覆盖门已闭环。0126 的新标签投影已在稳定生产重测，并把八端点基线来源绑定到功能提交 `d4e26b8`；后续基线仍必须伴随产品价值复核、真实生产重测、来源提交和归档，不能自动跟随输出自我放行；
...
- 八个结构化端点与单篇 Markdown 的 GET/HEAD 条件语义已经闭环；所有 HEAD 200/304 零正文，验证器、缓存及存在的表示元数据与 GET 等价，未知源文和未知标签 RSS 保持无验证器 `no-store` 404。该能力依赖 Next 发送边界而非十套显式 HEAD 导出，框架升级必须重跑真实构建与生产门；Vercel 对部分压缩 200 响应使用弱标签、对 304 精简表示元数据，生产门按 HTTP 等价语义验收；
