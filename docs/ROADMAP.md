...
| 7. 持续内容与作者体验 | in progress | Iteration 0027 完成 cover 契约、响应式详情页、分享元数据与双作者入口归档闭环 |

## 当前唯一主线

进入持续内容与作者体验阶段。Iteration 0027 已把 `cover` 从编辑字段推进为完整公开资产：仓库路径和 `coverAlt` 受契约约束，固有尺寸由服务端读取，文章/项目共享 `next/image` 组件，内容级图片进入 OG/Twitter/JSON-LD，Obsidian 封面与正文附件共用归档事务。下一主线是把同样的无布局跳动和响应式策略扩展到 Markdown 正文图片。需要品牌域名时再绑定自定义域名，旧公开站继续只作为迁移历史证据。
...
- 附件仍依赖 Git 仓库存储；封面引用、展示和分享已闭环，但 Studio/普通 Git 入口没有自动优化，Markdown 正文图片仍未读取固有尺寸或使用响应式候选；
