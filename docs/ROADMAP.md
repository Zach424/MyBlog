...
| 7. 持续内容与作者体验 | in progress | Iteration 0118 让搜索与知识图谱共享首发/更新日期语义 |

## 当前唯一主线

进入持续内容与作者体验阶段。Iteration 0118 已让 `SearchDocument` 与 `KnowledgeGraphNode` 保留可选 `updatedAt`，搜索结果、桌面 SVG 节点和移动孤立记录共同消费 `PUBLISHED/UPDATED` presenter；空搜索明确写作“按首发时间/首发顺序”，搜索相关性决胜、知识图次序与 `/archive` 首发时间线均未改变。功能提交 `a0ddd5b` 已通过 521 项单元测试、51 页构建、29 项应用测试、390px/1280px 深色浏览器验收与稳定生产 smoke；线上仍为 26 routes、OAuth 302，十二条 HTML 和七个发现端点预算全部通过。下一主线是新增纯服务端“内容活动”时间线，同时表达首次发布和真实后续更新，补上 archive 有意不记录维护事件的发现入口。首次真实 Obsidian 人机验收保留为所有者可执行事项，需要品牌域名时再绑定自定义域名。
...
- 共享列表、搜索与知识图已经区分首发和更新，但 `/archive` 有意只回答“何时首次形成”，目前没有一个公开入口按事件展示后续维护。下一轮应从同一内容记录纯派生 Published/Updated 活动流，保留 archive 的首发职责，拒绝把 `reviewedAt` 伪装成内容变化；
...
- `/archive` 当前只有一个年份和月份，但契约已经覆盖跨年、跨月、同日决胜、空集合和输入不变。不要为了当前四条内容提前增加客户端筛选、分页或年份导航；先让内容增长和十二路 HTML 预算提供真实压力，再决定增强方式；
