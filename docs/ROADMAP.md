...
| 7. 持续内容与作者体验 | in progress | Iteration 0129 已提供按真实内容变更排序的 Atom 1.0，并转回富文本式 Markdown 作者体验 |

## 当前唯一主线

进入持续内容与作者体验阶段。Iteration 0129 把更新优先订阅落到 `/updates.atom`：它直接消费公开内容集合，同时输出标准 `published`/`updated`，按 `updatedAt ?? publishedAt` 倒序，并保持根/标签/专题 RSS 的首发时间语义完全不变。首页自动发现与 `/subscribe` 第七条通道让读者主动选择；OPML 继续只导入 13 个 RSS leaf，避免默认订阅两个内容重叠的全站频道。Atom 1.0 必填元素、namespace、稳定 ID、日期、纯文本全文、分类、SHA-256 ETag、Last-Modified、条件 GET/HEAD、`noindex` 与 Sitemap 排除均已闭环。功能提交 `7e5909f` 已进入稳定生产；完整证据为 536 项单元测试、66 个生成页面、35 项应用测试、27 routes 与 OAuth 302。Atom 为 21338/10063 B（raw/gzip），预算提交 `fd359c1` 已把第十一个发现端点绑定真实部署。下一主线回到用户要求的富内容写作：先实现 Obsidian 兼容 Callout，让网页、Studio、搜索、源文与打印在不开放任意 HTML 的前提下共享信息块语义。首次真实 Obsidian 人机验收保留为所有者可执行事项，需要品牌域名时再绑定自定义域名。
...
- 当前结构化生产 raw/gzip 基线为：清单 3009/921 B、Schema 3278/755 B、JSON Feed 20697/9876 B、根 RSS 3400/1284 B、代表标签 RSS 2059/923 B、代表专题 RSS 2065/983 B、Atom 21338/10063 B、OPML 5193/962 B、Sitemap 5059/532 B、robots 155/127 B、OpenSearch 700/462 B；逐端点推导上限和覆盖门已闭环。0129 的更新投影已在稳定生产重测，并把十一端点基线来源绑定到功能提交 `7e5909f`；后续基线仍必须伴随产品价值复核、真实生产重测、来源提交和归档，不能自动跟随输出自我放行；
- `/subscribe` 只说明并直达现有公开读取协议：RSS 表达首次发布顺序，Atom 表达真实变更顺序，OPML 只打包全部 RSS 地址；它们都不提供邮件订阅、推送通知、写入 API、账号系统或第三方阅读器状态。若未来需要邮件列表，必须先明确供应商、隐私、退订、发送身份与成本，不能把当前只读目录当成已具备投递能力；
...
- 十一个结构化端点与单篇 Markdown 的 GET/HEAD 条件语义已经闭环；所有 HEAD 200/304 零正文，验证器、缓存及存在的表示元数据与 GET 等价，未知源文、标签 RSS 和专题 RSS 保持无验证器 `no-store` 404。该能力依赖 Next 发送边界而非十三套显式 HEAD 导出，框架升级必须重跑真实构建与生产门；Vercel 对部分压缩 200 响应使用弱标签、对 304 精简表示元数据，生产门按 HTTP 等价语义验收；
...
- 富内容写作仍缺少类似飞书文档的信息块和图表语义。下一步先做受限、服务端、Obsidian 兼容的 Callout；任意 HTML、iframe 与第三方脚本继续不进入内容契约，视频/图表后续也必须先明确安全、降级、打印和 Studio 预览边界；
