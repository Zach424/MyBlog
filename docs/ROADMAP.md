...
| 7. 持续内容与作者体验 | in progress | Iteration 0124 已把九个公开条件端点的 HEAD 语义纳入本地与生产回归门 |

## 当前唯一主线

进入持续内容与作者体验阶段。Iteration 0124 已审计 Next.js 16.3 本地文档、发送层源码和稳定生产，确认七个结构化端点与两篇代表 `source.md` 在仅导出 GET 时也能提供 HEAD。失败优先部署契约先因 smoke 没有 `method: "HEAD"` 覆盖而失败；提交 `64ab9dd` 随后为真实生产构建增加第 31 项应用测试，并在线验证普通 HEAD、ETag 304、日期 304、旧/坏日期 200、陈旧 ETag 优先和未知源文 404 全部零正文，验证器与缓存保持 GET 等价。完整证据为 530 项单元测试、52 页构建、31 项应用测试、27 routes 与 OAuth 302；运行时、正文、ETag 和所有预算未改变。下一主线转向读者可直接消费的 RSS 标签分类：把内容契约已有 tags 稳定映射为逐 item `<category>`，与 JSON Feed tags 对齐并经过 XML 转义、生产预算和订阅兼容验证。首次真实 Obsidian 人机验收保留为所有者可执行事项，需要品牌域名时再绑定自定义域名。
...
- 当前结构化生产 raw/gzip 基线为：清单 3009/921 B、Schema 3278/755 B、JSON Feed 20697/9876 B、RSS 3536/1298 B、Sitemap 5059/532 B、robots 155/127 B、OpenSearch 700/462 B；逐端点推导上限和覆盖门已闭环。0124 只增加 HEAD 测试与生产验证，运行时正文和 ETag 均未变化，因此没有重写基线；后续基线仍必须伴随产品价值复核、真实生产重测、来源提交和归档，不能自动跟随输出自我放行；
...
- 七个结构化端点与单篇 Markdown 的 GET/HEAD 条件语义已经闭环；所有 HEAD 200/304 零正文，验证器、缓存及存在的表示元数据与 GET 等价，未知源文保持无验证器 `no-store` 404。该能力依赖 Next 发送边界而非九套显式 HEAD 导出，框架升级必须重跑真实构建与生产门；Vercel 对部分压缩 200 响应使用弱标签、对 304 精简表示元数据，生产门按 HTTP 等价语义验收；
