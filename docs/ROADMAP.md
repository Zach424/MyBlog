...
| 7. 持续内容与作者体验 | in progress | Iteration 0063 完成四个 Obsidian 作者事务的自动 doctor 前置联锁 |

## 当前唯一主线

进入持续内容与作者体验阶段。Iteration 0063 已把 MyBlog Publisher 升到 1.14.0：检查/发布当前草稿、检查/提交当前正式内容复核四个事务都会先运行同一 version 1 doctor，ready 无弹窗继续原命令，attention 以 `TRANSACTION INTERLOCK / HELD` 展示被冻结的操作、来源路径与同一 preflight circuit；不可信 JSON 和致命退出均失败关闭。所有只读报告、统一分诊、领域状态与两类 deliver 继续绕过联锁，避免 synchronized 要求阻断正确恢复。下一主线为这四条链路增加跨 preflight 与领域命令的 single-flight lease；需要品牌域名时再绑定自定义域名，旧公开站继续只作为迁移历史证据。
...
- Obsidian 维护/Author Proof/交付状态与回执 1.14.0 已为四个新发布/复核事务增加自动 doctor 联锁，并保留统一分诊及原有六套版本化 JSON、活动 sourcePath、候选 SHA-256、deferred 差集、复核与发布两种待交付关系、精确 OID refspec、各自安全重送和不自动重试的回执；当前联锁按单次调用结算，两个几乎同时触发的作者事务仍可能各自通过 doctor 并并行启动重型质量门或写入链路。新内容 Commit Envelope 的真实主题组合、超长 object id/path 和大量媒体仍需随使用观察；tracking ref 明确只是最后本地观察，inspect 路由不会猜测修复；
