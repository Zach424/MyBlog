...
| 7. 公开发布与自定义域名 | partial | 私有站点的桌面、390px、320px、深色、Reduced Motion、搜索与键盘验收已完成；等待用户授权公开访问并完成未登录会话复核 |
...
4. 公开后用未登录会话回归桌面、窄屏、键盘、RSS 与分享元数据。

## 已知风险

- Vinext 仍需在项目范围内验证 Next.js 兼容边界。
- 当前浏览器验收版的 Wrangler 干跑总上传为 `2625.52 KiB`，gzip 后 `624.86 KiB`；已低于项目预算，仍需在内容增长后持续观察体积与冷启动表现。
...
- Windows Chromium 的系统字体折行已经过 1440px、390px 与 320px 验收；macOS、Safari 和 Firefox 的字宽差异仍需公开后回归。
