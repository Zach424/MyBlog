...
| 7. 持续内容与作者体验 | in progress | Iteration 0114 让首页项目、学习与当前焦点全部由公开内容事实驱动 |

## 当前唯一主线

进入持续内容与作者体验阶段。Iteration 0114 已让首页 Evidence Rail 的 Building、Learned 与 Current focus 从精选项目、最新公开文章和共享最新日期派生；项目状态、stack/tag 摘要、空集合与非法计数由 `lib/homepage-evidence.ts` 集中处理，组件不再维护内容状态副本。功能提交 `047ef40` 已通过 513 项单元测试、51 页构建、26 项应用测试、390px 深色长标题验收与稳定生产 smoke；线上仍为 26 routes、OAuth 302，十二条 HTML 和七个发现端点预算全部通过。下一主线是把 `/about` 的硬编码技术栈和静态规模说明改为公开内容/路由事实生成的系统档案。首次真实 Obsidian 人机验收保留为所有者可执行事项，需要品牌域名时再绑定自定义域名。
...
- 首页 URL 数量、日期、项目、学习与焦点均已数据驱动；`/about` 仍手写“TypeScript、React、Next.js 与 Vercel”且不显示真实内容/路由规模，下一轮应复用公开集合与精选项目生成可验证系统档案；
