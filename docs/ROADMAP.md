...
| 7. 持续内容与作者体验 | in progress | Iteration 0071 完成当前 Obsidian 草稿作者意图摘要 |

## 当前唯一主线

进入持续内容与作者体验阶段。Iteration 0071 已把 MyBlog Publisher 升到 1.22.0：“查看当前草稿发布意图”冻结活动安全 inbox `TFile`，运行版本化只读 inbox report，再严格选择同一路径证据。摘要复用正式 `prepareObsidianNote`、内容契约、媒体派生和共享附件判断，显示 Article/TIL/Project、公开目标、NOW/SCHEDULED 日期语义、附件数、站内链接数与阻塞原因；不解析 Markdown/YAML、不写文件、不进入 author lease、不发布或联网。JSON、安全声明、计数、路径或活动对象漂移时失败关闭且不回退、不重试。下一主线评估 source-scoped inbox 证据能否在保留跨草稿共享附件判断的同时跳过无关媒体派生；需要品牌域名时再绑定自定义域名，旧公开站继续只作为迁移历史证据。
...
- Obsidian 维护/Author Proof/交付状态与回执 1.22.0 保留四事务 owner-checked single-flight lease、phase/output activity pulse、会话内 terminal receipt、统一分诊及版本化 JSON；当前草稿作者意图也改读 version 1 inbox JSON，但现阶段仍为生成摘要而检查整个 inbox，草稿和大图很多时延迟可能高于单篇需要。模板驱动向导、文件名唯一身份、FileManager 改名与 `Vault.process` 严格旧字段清理已闭合新旧草稿身份。带注释、引号、anchor/tag、缩进、重复键或不匹配值的旧 slug 保持只读，真实 Obsidian 主题下的 Modal 版式仍需首次使用观察。新内容 Commit Envelope 的真实主题组合、超长 object id/path 和大量媒体仍需随使用观察；tracking ref 明确只是最后本地观察，inspect 路由不会猜测修复；
