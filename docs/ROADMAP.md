...
| 7. 持续内容与作者体验 | in progress | Iteration 0130 已提供 Obsidian Callout，继续推进受限、可移植的富内容 Markdown |

## 当前唯一主线

进入持续内容与作者体验阶段。Iteration 0131 已把受限 Mermaid fenced code 接入生产 Markdown HAST：流程、状态、时序、类、ER 与 XY 六类图表由服务端生成 SVG，移除上游样式/外链后再经过独立 HAST 白名单；每篇数量、源码、行、SVG 字节、元素、视窗与总输出都有硬上限。读者页面、Studio、搜索、公开 Markdown、深浅色、390px、键盘、源码折叠、打印与生产 smoke 共用一份契约，不向读者发送 Mermaid 客户端运行时。功能提交 `bbf1ec1`；本轮完整本地证据为 5/5 图表定向测试、541/541 既有单元测试、66 个生成页面与 35/35 应用测试，生产证据在本轮归档中固定。下一主线只处理受限视频内容决策：先确定本地文件/外部托管、隐私、字幕、体积、打印降级和 Obsidian/Studio 发布边界，不默认开放 iframe 或第三方播放器。首次真实 Obsidian 人机验收保留为所有者可执行事项，需要品牌域名时再绑定自定义域名。
...
- 视频仍没有内容契约。下一步必须先证明来源托管、隐私、字幕/替代说明、体积与带宽、移动端、打印、Studio/Obsidian 和生产 smoke，再决定是否实现原生本地 `<video>`；任意 iframe、追踪播放器和自动播放继续排除；
