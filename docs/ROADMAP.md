...
| 7. 持续内容与作者体验 | in progress | Iteration 0044 完成文章/项目 A4 打印版、可信来源、分页契约与生产 PDF 全页复核 |

## 当前唯一主线

进入持续内容与作者体验阶段。Iteration 0044 已把文章与项目详情页变成可打印、可导出 PDF 的 A4 技术记录：服务端来源、标题/事实/正文/代码/图片/表格/必要引用得到保留，网页导航与交互被隐藏，分页、长 URL、浅色代码块和关系账本均经生产 PDF 全页复核；屏幕样式和客户端 bundle 不变。下一主线补齐 Obsidian 作者常用的 Markdown 脚注/尾注，让技术判断可在正文标注证据且同时满足网页可访问性、反向跳转、打印和关系抽取边界。需要品牌域名时再绑定自定义域名，旧公开站继续只作为迁移历史证据。
