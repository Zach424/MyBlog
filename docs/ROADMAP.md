...
- 当前候选版的 Wrangler 干跑总上传为 `2633.66 KiB`，gzip 后 `627.21 KiB`；已低于项目预算，仍需在内容增长后持续观察体积与冷启动表现。
...
- Sites 第 7 个生产版本证明其静态资产层仍可绕过 Worker-first 配置；`public/_headers` 已加入专用安全兜底，修复版完成公网冒烟前保持为待验收风险。
