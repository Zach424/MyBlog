...
进入持续内容与作者体验阶段。Iteration 0130 已把 Obsidian Callout 语法接入生产 Markdown HAST：13 种官方类型及别名、自定义标题、静态/展开/收起、嵌套与正文 Markdown 共用服务端渲染；普通引用和代码示例不变。读者页面、Studio、搜索、深浅色、390px、打印与生产 smoke 共用一份契约，不增加依赖或客户端脚本。功能提交 `3c6dfbf` 已进入稳定生产；完整证据为 540 项单元测试、66 个生成页面、35 项应用测试、27 routes 与 OAuth 302。下一主线实现受限的 Mermaid 图表：优先服务端生成和安全清理 SVG，保留源码与文本降级，并先证明资源上限、打印、窄屏、搜索和 Studio 一致性。首次真实 Obsidian 人机验收保留为所有者可执行事项，需要品牌域名时再绑定自定义域名。
...
- 信息块能力已经闭环，但图表仍缺少受限语义。下一步评估服务端 Mermaid；任意 HTML、iframe 与第三方脚本继续不进入内容契约，图表必须先明确输入/输出资源上限、SVG 清理、失败降级、打印和 Studio 预览边界；
