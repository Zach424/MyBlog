...
| 7. 持续内容与作者体验 | in progress | Iteration 0141 已提供受约束项目文件树，继续推进受限、可移植的富内容 Markdown |

## 当前唯一主线

进入持续内容与作者体验阶段。Iteration 0141 已把受约束项目文件树接入同一生产 Markdown：作者使用顶层 `[!filetree]` 和递归无序列表，以 `` `路径段` — 说明 `` 表达文件/文件夹；Studio 可编辑有序完整相对路径，Obsidian 1.51.0 可插入模板。共享 AST 同时约束每棵 2–32 节点/4 层、每篇 3 棵/64 节点、路径段安全、文件/文件夹父子关系、完整路径规范化唯一性和说明白名单，并把同一事实投影到搜索、原生嵌套列表 Repository Slice、320 px 与打印。下一主线只处理受约束项目里程碑时间线：冻结开放 Markdown、事件日期/类型/排序/唯一性、Studio/Obsidian、搜索、窄屏和打印；不加入日历提醒、未来任务调度、外部项目管理同步或客户端动画。首次真实 Obsidian 音频、参考资料、步骤流程、术语表、FAQ 与文件树人机验收保留为所有者可执行事项，需要品牌域名时再绑定自定义域名。
