...
| 7. 持续内容与作者体验 | in progress | Iteration 0077 完成当前草稿替代文本来源与文件名回退阻塞 |

## 当前唯一主线

进入持续内容与作者体验阶段。Iteration 0077 已把 MyBlog Publisher 升到 1.28.0：`MEDIA TRACE` 消费 version 5 inbox report，以 `ALT · L<n> · AUTHORED/FILENAME FALLBACK` 区分作者描述与 Wiki 文件名回退；空文本和回退都保留最终证据并阻塞草稿。生产者仍在现有正文转换和正式 coverAlt 路径产出来源，插件只严格验证和展示，不解析或生成文案。下一主线为从每条 ALT 证据安全定位到当前草稿精确源码行；需要品牌域名时再绑定自定义域名，旧公开站继续只作为迁移历史证据。
...
- Obsidian 维护/Author Proof/交付状态与回执 1.28.0 保留四事务 owner-checked single-flight lease、phase/output activity pulse、会话内 terminal receipt、统一分诊及版本化 JSON；当前草稿作者意图使用 source-scoped version 5 inbox JSON，显示媒体 COVER/BODY、出现次数/来源行/逐次 alt/作者来源、精确变换与链接 trace，并仍只为当前附件生成真实候选、轻量解析全草稿和读取已发布链接目标以保留全局正确性。模板驱动向导、文件名唯一身份、FileManager 改名与 `Vault.process` 严格旧字段清理已闭合新旧草稿身份。空替代文本和 Wiki 文件名回退都与 blocker 双向绑定；源码行依赖现有保持换行的归一化路径，当前还不能直接从证据跳到编辑器位置，真实 Obsidian 主题下的 Modal 版式仍需首次使用观察。带注释、引号、anchor/tag、缩进、重复键或不匹配值的旧 slug 保持只读；新内容 Commit Envelope 的真实主题组合、超长 object id/path 和大量媒体仍需随使用观察；tracking ref 明确只是最后本地观察，inspect 路由不会猜测修复；
