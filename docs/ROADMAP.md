...
| 7. 持续内容与作者体验 | in progress | Iteration 0097 用无自引用 descriptor 与三份 SHA-256 关闭同版本插件文件缺失、局部更新和摘要漂移，六个 Git writer 均保持零自动修复 |

## 当前唯一主线

进入持续内容与作者体验阶段。Iteration 0097 已让 MyBlog Publisher 1.40.0 用 `bundle.json` 固定 manifest/main/styles 的顺序、版本和 SHA-256；legacy doctor v1 保留旧 runtime reload 能力，当前六个 Git writer 使用 14 项 v2，并在 descriptor/文件异常时显示 `PLUGIN BUNDLE INVALID`。生成、校验、Modal 和 Git 都保持显式分离，真实 Vercel 以冻结摘要在 1 次、1263 ms 内收敛。下一主线是给 bundle 增加只读 Git HEAD/index provenance，防止显式 `--write` 被误当成可信发布来源。需要品牌域名时再绑定自定义域名，旧公开站继续只作为迁移历史证据。
...
- Obsidian 1.40.0 保留既有草稿身份、作者意图、四事务联锁、sealed Git 交付恢复和维护能力；正常与恢复 publication/review 的可信 handoff 均自动接力生产等待。所有 Git writer 的三方版本与 bundle 摘要已覆盖未来 patch/minor、伪造/缺失 descriptor、局部更新和恢复旁路；真实主题下 reload/bundle interlock、连续 receipt/production Modal、长 ETag、commit、尝试列表、持续 Notice 与本机代理继承仍需首次使用观察。bundle 目前绑定工作区字节但不绑定 Git HEAD/index，下一步增加只读 provenance；
...
- Git/Obsidian sealed receipt、version 1 handoff、生产清单四态、单篇有界收敛、runtime/disk 版本与 bundle 完整性已覆盖正常和恢复交付；网络、协议、版本或摘要错误不能冒充内容差异或触发二次 Git 动作。下一轮只处理插件文件的只读 Git provenance，不改变交付协议；
