...
Obsidian 的维护入口服从宿主应用而不是复制网页皮肤。1.2.0 的“已发布内容复核队列”沿用原生 Modal、标题、边界说明和可滚动 `pre`，直接显示同一 CLI 证据；诊断文本用 `setText` 写入，不允许内容路径或错误输出成为 HTML。进度、完成和失败使用原生 Notice，持续 Notice 必须在任一终态和插件卸载时消失。第一版刻意不把纯文本报告重画成第三套卡片系统，确保终端、Obsidian 和 Actions 的语义仍可逐行对照。
