...
正式内容复核采用显式的两步动词，不把“打开笔记”或“点击同步”伪装成事实确认。1.4.0 只在正式文章/项目处于活动视图时显示“检查当前正式内容复核”和“提交并同步当前正式内容复核”；作者必须先手工核对清单并编辑日期。检查命令的成功 Notice 明确说明没有暂存、提交或推送；同步命令直接说明会执行完整检查、提交和推送，不增加含混的 Save、Done 或自动确认按钮。失败 Notice 保留可执行边界，插件不自动修改 frontmatter、不弹出虚构完成率，也不在命令名之外再叠一层确认仪式。
