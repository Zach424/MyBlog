...
Evidence Rail 的运行事实必须可证明。Verified 项中的公开 URL 数量从与 Sitemap 相同的路由清单实时派生，元数据明确写作 `Sitemap synced`；首屏版本编号不再手写，改用 `LATEST · <日期>` 展示与 Sitemap 根 URL `lastmod` 相同的最新公开内容日期。空库显示 `NO PUBLIC CONTENT`，不为填满界面伪造日期。这个变化只提高证据可信度，不改变首页网格、颜色、排版或交互层级。
