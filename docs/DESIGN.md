...
正式内容复核采用显式的两步动词，不把“打开笔记”或“点击同步”伪装成事实确认。1.5.0 只在正式文章/项目处于活动视图时显示“检查当前正式内容复核”和“提交并同步当前正式内容复核”；作者必须先手工核对清单并编辑日期。检查成功后打开原生只读 Author Proof，而不是只闪过 Notice：`AUTHOR PROOF / CHECKED`、真实标题/路径、HEAD 日期到当前日期的 review transition rail，以及一张规则线账本列出事实变化、updatedAt、`npm run check · passed`、main/index/untracked 和唯一可提交路径。迁移轨是唯一视觉签名，使用宿主 text/interface/monospace 字体、Verified/Trace/Caution 颜色和文本双重语义，不使用卡片、渐变、动画或同步按钮。底部明确“仍需单独运行提交并同步”；结构异常则退回纯文本 Proof，绝不把未经验证的数据画成可提交状态。
