...
- `/subscribe` 把一份 Markdown/Git 内容源画成无动画 switchboard：左侧 Source rail 只说明单一来源，右侧六个 Port 用受众、标题、说明、Format、Endpoint、Freshness 与原生动作链接表达真实协议，不做通用卡片或虚构统计。OPML 的“一次导入全部订阅”使用与其他端点相同的轨道，只改变动作文字，不提升成营销 CTA；RSS 的逐条修改时间，以及 Feed、内容清单和单篇 Markdown 的 HTTP 日期/HEAD 验证都属于机器协议增强，不新增徽章、状态卡或重复的可见活动流。读者仍可在 `/activity` 查看完整可见变化语义。`≤55rem` 时 Source 转为顶部总线，`≤42rem` 时每个 Port 折为编号轨道、正文和动作三段；深色只替换 Token，打印保留协议与 URL、隐藏无必要继续操作；
...
当前 `/subscribe` 展示一个全站 RSS 与一个聚合 OPML 入口；主题订阅仍采用“在标签语境中发现”的渐进路径，不把所有标签复制成中央 HTML 清单。阅读器是否显示 category、自动发现或保留 OPML 分组属于第三方界面行为，本站只承诺合法 XML、跨格式一致、稳定顺序和原生链接可达。标签 RSS 与 OPML 都不增加账户、订阅状态、邮件表单、动画或客户端偏好存储，也不进入 Sitemap 冒充可见页面。
