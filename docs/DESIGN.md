...
RSS category 是机器可读的主题标签，不新增可见 badge、筛选控件或第二套导航。它与站内标签页、JSON Feed tags 共用同一作者事实源；文章类型和项目身份继续由各自页面与结构化字段表达，不借 category 伪装成主题。特殊字符只在 XML 传输层转义，阅读器看到的文本仍是作者原标签。

当前 `/subscribe` 仍展示一个全站 RSS 入口，不暗示已经支持主题订阅。阅读器是否显示 category 属于第三方界面行为，本站只承诺合法 XML、跨格式一致与稳定顺序。Iteration 0126 若增加标签 RSS，入口应出现在对应标签页并沿用现有 Commit Trace 文本链接，不增加账户、订阅状态、邮件表单或客户端偏好存储。
