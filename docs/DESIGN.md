...
- `/subscribe` 把一份 Markdown/Git 内容源画成无动画 switchboard：左侧 Source rail 只说明单一来源，右侧七个 Port 用受众、标题、说明、Format、Endpoint、Freshness 与原生动作链接表达真实协议，不做通用卡片或虚构统计。RSS 的“跟踪首次发布”、Atom 的“跟踪真实变更”和 OPML 的“一次导入全部 RSS”使用相同轨道，只改变任务与动作文字，不提升成营销 CTA；Feed、内容清单和单篇 Markdown 的 HTTP 日期/HEAD 验证都属于机器协议增强，不新增徽章、状态卡或重复的可见活动流。读者仍可在 `/activity` 查看完整可见变化语义。`≤55rem` 时 Source 转为顶部总线，`≤42rem` 时每个 Port 折为编号轨道、正文和动作三段；深色只替换 Token，打印保留协议与 URL、隐藏无必要继续操作；
...
`/activity` 把同一内容的首发和后续变化画成一条 diff rail，而不是第二个档案页或统计仪表盘。`PUBLISHED` 使用实心 Signal 节点，`UPDATED` 使用空心双环；两者共享按日分组的纵向轨迹。日期栏以超大 `MM.DD` 和较弱年份形成扫描锚点，事件栏依次给出模式、内容类型、标题、摘要与原生链接。

顶部 key 明确说明 `REVIEWED` 只代表复核，不形成事件。页面复用 Commit Trace 的 Canvas/Ink/Signal/Trace、display/mono 字体和直角规则线，不增加卡片、渐变、阴影、装饰图表或客户端动画。`≤42rem` 时日期从左栏折成横向标记，事件改为三列并保留轨迹；390px 与 1280px 深色模式都必须零横向溢出。活动页不改变 `/archive` 的首发职责，后者只增加一个可见入口。

## 首页最近活动

首页在 Current focus 与最近记录之间加入 `CHANGE SET / 03 LATEST`，只回答回访读者“同一知识库最近发生了哪三次真实变化”。它直接复用活动账本前三项，不复制日期或排序规则；标题、类型、模式、日期和目标链接均来自同一事件对象，右上角原生链接进入完整 `/activity`。

宽屏使用一条横向三节点 rail：实心节点仍代表 PUBLISHED，双环节点仍代表 UPDATED；模式与日期用 Canvas 底色在轨道上留出可读缺口，避免线条穿过文字。`≤42rem` 后轨道转为纵向，三项保持原顺序并自然换行。该段不使用卡片、动画、统计、客户端状态或新的色板；它延续 Commit Trace 的规则线、Signal 箭头、display/mono 字体，并保持 Building/Learned/Current focus 和最近记录的原层级。

首页摘要与完整活动页分别由独立 CSS Module 承担专属布局，基础 Token 仍来自全局设计系统。视觉测试同时覆盖 1280×900 与 390×844：根宽等于视口，三项模式/日期/标题顺序正确，横向/纵向轨道可读，控制台无错误。

## 订阅分类语义

RSS category 是机器可读的主题标签，不新增可见 badge、筛选控件或第二套导航。它与站内标签页、JSON Feed tags 共用同一作者事实源；文章类型和项目身份继续由各自页面与结构化字段表达，不借 category 伪装成主题。特殊字符只在 XML 传输层转义，阅读器看到的文本仍是作者原标签。标签详情在 Collection Intro 与内容列表之间复用现有 `.collection-links` 文本动作，显示“订阅此标签 RSS →”；箭头只作装饰，链接保持原生无 JavaScript 可用。页面 head 同时声明绝对 `application/rss+xml` alternate，视觉入口与机器发现指向同一 Feed。

当前 `/subscribe` 展示全站 RSS、更新优先 Atom 与聚合 OPML 入口；主题订阅仍采用“在标签语境中发现”的渐进路径，不把所有标签复制成中央 HTML 清单。阅读器是否显示 category、自动发现、更新时间或保留 OPML 分组属于第三方界面行为，本站只承诺合法 XML、跨格式一致、稳定顺序和原生链接可达。标签 RSS、Atom 与 OPML 都不增加账户、订阅状态、邮件表单、动画或客户端偏好存储，也不进入 Sitemap 冒充可见页面。

## 专题目录与订阅流

专题详情是学习路径，不是活动流。可见列表继续按作者声明的 `series.order` 从第 1 章排到最后一章，Collection Intro 保留 `PARTS / ORDERED`，让第一次进入的读者按教学顺序阅读。专题页在 Intro 与章节列表之间复用 `.collection-links`，显示“订阅此专题 RSS →”，head 同时声明同一绝对 `application/rss+xml` alternate；链接无客户端 JavaScript 也可使用。

专题 RSS 服务的是“什么内容刚刚出现”，因此按首发时间倒序，而不是复制课程目录。新章节或后来插入的章节会进入阅读器顶部；频道主页仍指向专题 HTML，读者回到页面后看到完整章节顺序。两种排序都来自同一公开专题集合，只在消费语境处投影，不增加第二套作者字段或隐藏排序开关。

```text
同一 series.posts
├─ 专题 HTML → series.order 升序 → 连续学习
└─ 专题 RSS  → publishedAt 倒序 → 订阅提醒
```

专题 Feed 的视觉处理与标签 Feed 一致：不增加 badge、卡片、动画或订阅状态，不进入 Sitemap；区别只在链接文案与频道语境。机器排序差异必须由端到端测试锁定，避免未来为了“统一”而破坏任一用户任务。

## OPML 聚合订阅

OPML 是迁移和首次设置工具，不是新的内容页面。`/subscribe` 只增加一个“下载 OPML”原生动作；下载后的文档以“全部更新 / 按标签 / 按专题”组织频道，当前 13 个 leaf 都保留 RSS 标题、说明和对应 HTML 页面，让读者即使在导入前查看文本也能理解语境。分组顺序从全局到局部，标签与专题内部使用标题优先、slug 决胜，避免文件随输入遍历顺序抖动。
...
端点不渲染站点壳、卡片、图标或动画，使用 `text/x-opml` 附件响应；可见设计责任停留在 `/subscribe`。OPML 2.0 允许嵌套，但第三方阅读器可以合法展平分组，因此用户价值定义为“全部频道可移植”，不把目录外观当作可控结果。可选日期与 Last-Modified 只有在能证明聚合表示的精确变化时才显示；当前选择省略，用 SHA-256 ETag 表达字节身份，避免把构建或部署时间伪装成内容事实。

## Atom 更新订阅（Iteration 0129）

Atom 是“最近真正改了什么”的机器时间线，不是第二个可见活动页面。根 RSS 继续按首次发布日期排序，`/updates.atom` 则把同一公开内容集合按 `updatedAt ?? publishedAt` 排序；每个 entry 并列输出首次发布与真实更新时间，避免把旧文章修订伪装成新文章。`/subscribe` 用“更新阅读器 / 订阅真实内容变更”明确区分任务，首页 head 同时提供自动发现；可见 `/activity` 继续承担人类阅读的变化账本。

```text
同一公开内容集合
├─ /rss.xml       → published 倒序 → 发现新内容
└─ /updates.atom  → updated 倒序   → 跟踪真实修订
```

Atom XML 不渲染站点壳、卡片、图标或动画，使用 `application/atom+xml` 内联响应。entry 的 summary 与纯文本全文让阅读器在不执行站点 JavaScript 的情况下完成阅读；category 延续作者标签，不增加“更新”伪标签。OPML 有意只包含 RSS，因为一次导入同时加入根 RSS 和 Atom 会造成重叠提醒；愿意追踪修订的读者可在 `/subscribe` 主动选择 Atom。该决定把“提供选择”和“默认去重”分开，不假设第三方阅读器会自动合并 GUID。

## Obsidian Callout（Iteration 0130）

Callout 延续 Commit Trace / Evidence Rail，而不是引入一组圆角彩色卡片。每个信息块由一条左侧类型轨道、上下规则线和 `TYPE / 标题` 组成；类型码使用等宽字，标题保持正文层级，正文与标题之间用虚线分隔。13 种语义被约束为五个可读色组：信息、成功、问题、警告和引用；类型文字、边框结构和标题始终同时存在，颜色不是唯一信号。

```text
TYPE / 标题                         + 或 −
----------------------------------------
正文 Markdown、列表、公式与嵌套信息块
```

无折叠符的 Callout 是静态 `<aside role="note">`；`+`/`-` 使用原生 `<details><summary>`，右侧 `−/+` 明确当前展开状态。键盘焦点使用全站 Signal outline，不增加 JavaScript 动画。嵌套信息块缩短外边距并保持独立轨道，普通引用仍使用既有 Signal 左线，避免二者视觉身份混淆。

浅色背景只把类型色以 5% 混入 Paper，深色提高轨道和类型码亮度；390px 不改变信息顺序，长标题可以安全换行。打印时所有默认收起内容强制展开，去掉折叠符并避免标题与第一段分离。Studio 使用同一结构和同一五色映射，作者看到的不是近似占位符。桌面 1280px、390px 深色、默认收起/展开、嵌套、普通引用和公式已经用真实浏览器验收，控制台无错误。

明确排除：装饰图标、emoji 类型映射、第三方 icon font、渐变、阴影、动画、任意 class、HTML 属性和客户端状态持久化。标题保持纯文本；复杂行内标题语法只有在无障碍名称与 HAST 契约能够一并验证时才考虑。

## Mermaid 图表（Iteration 0131）

图表延续 Evidence Rail，而不是嵌入第三方白色卡片。顶部固定显示 `DIAGRAM / TYPE` 与带 Signal 节点的 `SERVER SVG`；画布使用两方向轻规则网格、直角边界和细滚动条，节点、连线、箭头与文字从 Paper/Ink/Trace/Signal 映射。宽图保持自身固有宽度，只在画布内横向滚动；390px 不压缩到无法阅读，也不增加页面根宽。下方原生 `<details>` 以 `MERMAID SOURCE / 查看源码` 和顺序编号暴露可移植源码。

浅色用 Paper 背景、Trace 节点和 Signal 箭头；深色复用全站 token，不另造黑底代码主题。画布和源码摘要都有 Signal `:focus-visible`；SVG 拥有图表类型无障碍名称，画布声明可横向滚动。打印移除网格和源码重复项，把 SVG 收进页面宽度并尽量避免跨页。真实浏览器检查覆盖桌面、390px、暗色、横向滚动、源码展开和键盘反向 Tab；视觉检查曾捕获连线标签缺少上游 `--bg` 映射导致黑底，修复后标签与节点表面一致。

明确排除：远程字体、上游内嵌 style、渐变/阴影、客户端动画、点击节点、自动缩放脚本、iframe、任意 HTML、作者自定义颜色与第三方图标。图表的结构、类型文字和源代码是主证据，颜色只承担辅助分组。
...
## 本地静音视频（Iteration 0132）

视频使用 Evidence Rail，而不是模拟视频网站皮肤。顶部 `VIDEO / SILENT MP4` 与 `LOCAL · NO TRACKING` 直接表达媒体类型、静音和来源；播放器保持直角、深色画布和浏览器原生 controls，不加入圆角卡片、渐变、阴影、悬浮工具栏、自动播放或自定义时间轴。Signal 只用于焦点与关键标签，不把“能播放”渲染成成功徽章。

信息顺序固定为“来源事实 → 原生播放器 → 标题 → 画面说明 → 加载提示”。标题是短身份，说明是看不到画面时仍成立的操作/结果文本。两者都会进入 figcaption；`aria-label` 合并二者。320px 下播放器收进正文规则线且不扩大根宽，caption 从横向信息折成自然网格；深色继续复用全站 token。打印隐藏播放器和重复装饰，只留下标题、说明和直接文件 URL。

`preload="none"` 是设计决定：正文优先可读，媒体由读者主动请求。无 autoplay、无 loop、无第三方 iframe，也不为“更像视频平台”增加脚本。音频、字幕按钮、poster、多清晰度和灯箱继续不在 v1；未来开放时必须先证明内容与可访问性契约。

## 多图画廊（Iteration 0133）

画廊是“有序证据接触表”，不是照片应用。顶部以 `GALLERY / NN FRAMES`、`ORDERED · LOCAL` 和组标题建立来源与顺序；主体在桌面显示两列等宽帧，`32rem` 以下按作者顺序折为单列。帧舞台使用 4:3 深色网格和 `object-fit: contain`，宁可保留留白，也不裁掉技术截图边缘。

每帧下方固定显示 Signal 色 `FRAME NN`、短标题和详细说明。编号、结构和文字共同表达关系，颜色不是唯一信号。组标题说明共同结论，短标题标识阶段，alt 承担图像不可见时的内容；三者不自动互相复制。

画廊没有箭头、分页圆点、自动轮播、灯箱、手势或悬浮按钮。所有图片同时存在于文档流，便于比较、键盘/屏幕阅读器访问和打印。打印继续两列并避免单帧内部跨页，舞台改为浅色；窄屏不改变顺序、不扩大页面根宽。
...
## 代码变更审阅卷宗（Iteration 0145）

代码变更使用 Review Docket / 变更审阅卷宗，而不是伪造 GitHub、终端或 IDE。顶部先显示 `CHANGE / REVIEW`、模式、真实日期与标题；目的作为审阅前提，文件索引使用状态码、路径和说明建立可扫描边界。CHANGE 区保留原生等宽代码与语法高亮，VERIFICATION 和 RISKS 分居证据与剩余不确定性两侧，让颜色只辅助识别，不承担唯一语义。
