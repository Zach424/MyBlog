...
订阅目录必须返回 200、具有 `/subscribe` canonical，并在可见 HTML 中恰好渲染 RSS、Atom、OPML、JSON Feed、OpenSearch、清单/Schema 与 Markdown 七条只读通道；页脚、Sitemap 和全部原生端点链接必须可达。纯目录测试锁定通道顺序、MIME/路径、清单双链接、最新 Markdown 的日期/标题/URL 决胜、输入不变和空集合说明；真实 SSR、生产 smoke 与 390px 浏览器还要验证页内真实链接、只读边界、无横向溢出和零 console error。

检查代表内容、时间档案、内容活动、订阅目录、标签/专题订阅发现、搜索、Studio HTML/配置/媒体清单/媒体预检/稳定 slug 控件/公式预览模块/版本化 KaTeX CSS/同源 CMS 运行时、OAuth、OpenSearch、公开内容清单及 Schema、JSON Feed、根/标签/专题 RSS、Atom、OPML、robots、全 Sitemap、永久重定向、安全头、缓存与固定 404。OpenSearch 必须具有 1.1 namespace、唯一 ShortName/Description、同源 results/self URL、安全 `{searchTerms}` 模板、示例查询、语言/编码、准确 MIME、`noindex`、最终正文 SHA-256 ETag 与空 304；首页和搜索结果页必须声明绝对 `rel="search"`，端点不能进入 Sitemap。公开内容清单必须由根 HTML 发现，使用 version 1、同 origin 绝对 HTML/Markdown URL、字段白名单、稳定顺序和与全部真实源文相同的 SHA-256 digest；清单自身 ETag、Last-Modified、空 304、`noindex` 与 Vercel 等价缓存必须成立，并与 JSON Feed/根 RSS 保持同一公开 id 顺序。Schema 必须使用 Draft 2020-12 和 `application/schema+json`，具有同源 `$id`、严格字段结构、与清单双向 registered Link relation、SHA-256 ETag、空 304、`noindex` 与等价缓存；Ajv 2020 还要验证真实清单并拒绝结构反例。JSON Feed 必须使用 1.1 version 和 `application/feed+json`，顶层 origin、作者、语言、icon 正确，item 必须有唯一同值 id/url、纯文本正文、RFC 3339 发布日期且不泄漏内部字段。标签 RSS 必须由同名 tags 子集派生；专题 RSS 必须由对应章节集合派生，且专题 HTML 的章节升序与 Feed 的首发时间倒序都要锁定。两类 scoped Feed 的页面可见/alternate 发现一致，channel home/self、文件名、Link/noindex 正确，未知资源 GET/HEAD 为无验证器 `no-store` 404，且 Feed 不进入 Sitemap。Atom 必须是可解析的 1.0 文档，feed/entry 必填元素、永久 id、self/alternate、published/updated、变更排序、纯文本全文与 tags 分类精确来自同一公开集合；首页 alternate、MIME/文件名/Link/noindex、可审计 Last-Modified 和 Sitemap 排除都必须成立。OPML 必须是可解析的 version 2.0 文档，按根/标签/专题稳定分组，订阅 leaf 精确覆盖当前全部公开 RSS 并排除 Atom，具备规范属性、绝对地址、正确 MIME/文件名/Link/noindex，不含虚构日期且不进入 Sitemap。JSON Feed、根/标签/专题 RSS、Atom、OPML、Sitemap、robots 和 OpenSearch 还必须证明 ETag 等于各自最终正文 SHA-256，并用源响应标签完成同 digest、同缓存策略、零正文的 304；带日期的 Feed 另需证明规范 Last-Modified、日期命中 304、旧/非法日期 200 和 ETag 优先级，本地接受三种 HTTP-date，生产允许边缘 304 省略可选表示元数据。缓存验证接受源响应的一小时 fresh/一天 SWR，以及 Vercel CDN 消费 SWR 后的等价一小时策略，robots 则必须保持明确的一天 public fresh。错误 TTL、`private` 或成功响应 `no-store` 仍失败关闭。十三条关键 HTML 路由和十一个结构化发现端点还必须在实际传入的生产域名下逐条输出 raw/gzip 实测、阈值、基线和带正负号的余量；缺失、重复、意外端点或超限都失败关闭。线上公式 POST 必须返回两条公式、KaTeX、MathML 与生产管线标记，CSS 必须内联 WOFF2 且不残留 package 相对字体路径。媒体清单必须是 `version: 1`、根为 `public/uploads`，每项包含安全路径、正整数节数与 64 位 SHA-256，并保持 `no-store`。永久重定向检查要求 `/blog` 返回 308、同源 `Location` 直达 `/posts`，且目标只需一次请求即返回 200。`--expect-oauth` 只用于已配置 GitHub OAuth 的生产环境；本地和 Preview 允许 OAuth 以 503 安全关闭。
...
- JSON Feed、根/标签/专题 RSS、Sitemap 与 robots 的强 ETag 必须等于最终响应正文 SHA-256；`If-None-Match` 的精确/弱值、列表与 `*` 命中必须返回保留原 ETag/MIME/缓存策略的空 304，错值与畸形列表返回完整 200；JSON Feed/RSS 的 Last-Modified 必须取各自表示修订时间和最新公开内容的最大值，发送端只用 IMF-fixdate，接收端兼容三种 HTTP-date，日期相等/更晚返回空 304，旧值、重复值、日历或星期错误返回完整 200，任何存在的 `If-None-Match` 都优先；RSS 还必须声明准确的 Dublin Core Terms 命名空间，逐项保持 `guid`、首发排序与 `pubDate`，仅对严格更晚的更新输出一个与 JSON Feed `date_modified` 相同的 `dcterms:modified`，并拒绝 item 级 `atom:updated`；标签 RSS 只允许同名 tag 子集，专题 RSS 只允许对应章节集合且必须和专题 HTML 的章节升序形成明确的首发时间倒序，频道和 Link 指向对应集合语境；这些 Feed 与 Sitemap 保留一小时 fresh/一天 SWR，robots 保留一天 public fresh；
- OPML 必须使用 version 2.0，根/标签/专题 outline 稳定分组并只消费公开索引；每个订阅 leaf 必须有 `text`、`type="rss"`、绝对 `xmlUrl` 及对应 HTML 语境，特殊字符正确转义、空分组省略、调用方输入不变。响应必须使用准确 MIME、附件名、self/up Link、`noindex`、一小时 fresh/一天 SWR 和最终正文 SHA-256 ETag；条件 GET/HEAD 返回空 304，端点不进入 Sitemap，且在没有可证明表示时间时不得生成 OPML 日期或 Last-Modified；
- Atom 必须使用 1.0 namespace，feed 的 title/id/updated/author 唯一且 self/alternate 正确；entry 必须以规范内容 URL 作为稳定 id，保留 published、以 `updatedAt ?? publishedAt` 输出 updated，并按 updated、published、中文标题、URL 稳定倒序。summary、纯文本 content 和 category 必须与同一公开记录对齐且输入不变；首页 alternate 与 `/subscribe` 可见，OPML/Sitemap 排除。响应必须使用准确 MIME、内联文件名、self/alternate Link、`noindex`、一小时 fresh/一天 SWR、最终正文 SHA-256 ETag 和可审计 Last-Modified；条件 GET/HEAD 返回空 304；
...
- 十一个结构化端点与文章/项目 `source.md` 必须在真实 Next 生产构建中证明 HEAD 200/304 零正文，并保持 GET 的 ETag、Last-Modified、MIME、缓存、文件名、Link/noindex；所有端点覆盖 ETag HEAD，带日期资源还覆盖 IMS 命中、旧/坏日期和 ETag 优先。稳定生产允许 304 精简可选表示元数据，但同一 opaque digest、缓存和零正文不可改变；未知源文、未知标签 RSS 与未知专题 RSS HEAD 必须是 `no-store` 404 且不得生成验证器；
- 所有可见内部导航目标返回成功；
...
| 十一个结构化发现端点 raw | 各自 `生产基线 + max(50%, 4 KiB)`，再向上取整到 `1 KiB` |
| 十一个结构化发现端点 gzip | 各自 `生产基线 + max(50%, 1 KiB)`，再向上取整到 `512 B` |
...
`scripts/discovery-budget.mjs` 独立保存十一个结构化端点的 stable-origin raw/gzip 基线和逐端点推导上限。它同时约束 raw 与 gzip：可压缩的异常正文不能只靠 gzip 通过，高熵增长也不能只靠 raw 通过。本地应用测试与生产冒烟必须各自恰好覆盖清单、Schema、JSON Feed、根 RSS、代表 TypeScript 标签 RSS、代表 `build-my-blog` 专题 RSS、Atom、OPML、Sitemap、robots、OpenSearch 一次，并输出 `[discovery-budget]` 报告；Iteration 0129 以 Atom 功能提交 `7e5909f` 的稳定生产响应统一绑定来源，依次为 3009/921、3278/755、20697/9876、3400/1284、2059/923、2065/983、21338/10063、5193/962、5059/532、155/127、700/462 B（raw/gzip）。预算提交 `fd359c1` 只接入第十一条实测与来源；十一个推导上限依次为 7168/2048、8192/2048、31744/14848、8192/2560、7168/2048、7168/2048、32768/15360、10240/2048、9216/2048、5120/1536、5120/1536 B（raw/gzip）。
...
Iteration 0127 先以应用测试证明专题页缺少订阅发现和专题协议路由，并以单元测试锁定“章节输入升序、订阅输出最新优先”。实现后专题 HTML 仍为第 1→2 章，Feed 为第 2→1 章；标签/专题共享安全 404，`/series/:path*` 被收窄以避免配置层污染协议缓存。功能提交 `cf3c631` 后完整门为 ESLint、TypeScript、532 项单元、64 个生成页面、33 项应用测试，稳定生产为 27 routes/OAuth 302。代表专题 Feed 有 2 项、2065/983 B（raw/gzip）；预算提交 `3982113` 把它纳入第九条发现端点，推导上限 7168/2048 B。

Iteration 0128 先以单元、目录和真实应用测试让旧实现分别因缺少 `lib/opml.ts`、第六条入口和 `/feeds.opml` 以 7/10、31/34 失败，再实现共享 OPML 生成器、Route Handler 与可见入口。XML 解析器独立证明特殊字符夹具合法；完整门为 ESLint、TypeScript、534 项单元、65 个生成页面、34 项应用测试，稳定生产为 27 routes/OAuth 302。当前 OPML 有 13 个订阅 leaf、5193/962 B，SHA-256 为 `6344a8a82167b1d35b3d55e9f3679dc43cca9b72c31823d74ac8d1e72b7c7ce8`；预算提交 `e33f395` 把它纳入第十条发现端点，推导上限 10240/2048 B。

Iteration 0129 先以 Atom/订阅/OPML/部署定向测试让旧实现因缺少 `lib/atom.ts`、第七条入口和 smoke 契约以 9/12 失败，再让真实应用因 `/subscribe` 仍为六条、`/updates.atom` 为 404、HEAD 矩阵缺少 Atom 以 32/35 失败。实现共享 Atom 生成器、Route Handler、首页自动发现与生产协议验证后，完整门为 ESLint、TypeScript、536 项单元、66 个生成页面、35 项应用测试，稳定生产为 27 routes/OAuth 302。当前 Atom 有 4 个 entry、21338/10063 B，SHA-256 为 `a9564d43d1b2fc04cf77812431837c3a1820d2ccdf37269f6bf9278da823671f`；预算提交 `fd359c1` 把它纳入第十一个发现端点，推导上限 32768/15360 B。
