...
顺序为 ESLint → 508 项内容/时间档案/订阅目录/维护/inbox/暂存媒体/关系/推荐/站点身份/面包屑/标题锚点与永久链接/脚注/数学公式/打印版式/知识图/外链库存与检查/搜索/OpenSearch/公开清单/发现端点验证器与预算/公开 Markdown/OAuth/Studio/Obsidian/媒体/重定向/代码复制/交付单元测试 → Next 路由类型生成与 TypeScript → 原生 Next.js 生产构建（51 个页面，并包含动态 Route Handler）→ 25 项真实生产 HTTP 与质量审计。任何一步失败都阻止合并和生产部署。
...
| 十一条关键路由 gzip 传输模拟 | `生产基线 + max(20%, 2 KiB)`，再向上取整到 `1 KiB` |
| 七个结构化发现端点 raw | 各自 `生产基线 + max(50%, 4 KiB)`，再向上取整到 `1 KiB` |
| 七个结构化发现端点 gzip | 各自 `生产基线 + max(50%, 1 KiB)`，再向上取整到 `512 B` |

`scripts/html-budget.mjs` 保存稳定生产 origin、基线日期、来源提交及十一条路由的 raw/Node zlib gzip 基线。本地生产测试用该稳定 origin 作为 forwarded host，对完整响应执行 `Buffer.byteLength` 与 `gzipSync`；部署后的 `production:smoke` 再对实际输入域名执行同一模块，二者都输出实测、阈值、基线和余量。raw 160 KiB 只防止异常解压/文档膨胀；性能回归由按生产基线推导的 gzip 门判断，因此高度重复但可压缩的 100KB 以上页面不会被旧统一门误伤，高熵增长仍会失败。

Iteration 0111 以稳定生产功能提交 `5ab34a7` 在 2026-08-11 重新冻结十一条基线：`/` 27419/6016、`/posts` 17972/4265、代表文章 51975/12279、代表项目 108139/24490、`/archive` 20386/4742、`/subscribe` 29108/5727、专题 17621/4184、标签 17442/4154、搜索 36304/13847、知识地图 36018/7262、关于页 15022/3870 B（raw/gzip）。页脚从直接 RSS 改为订阅目录使共享框架只产生 12 B 左右的可解释 raw 变化；十一条 gzip 上限依次为 8192、7168、15360、29696、7168、8192、7168、7168、17408、10240、6144 B。来源提交、日期和完整路径清单由脚本与测试共同锁定；基线仍不能自动追随当前输出。
