...
打开命令面板并运行“查看已发布内容复核台账”。MyBlog Publisher 1.5.0 会在仓库根目录隐藏运行 `npm --silent run content:status -- --format json`，验证版本化报告后，用原生 deadline ledger 显示报告日期、Current/Historical/未公开数量、healthy/review-soon/due-soon/overdue 四档计数、源笔记路径、review-by、剩余天数和复核清单。每条记录的“打开笔记”只打开 Vault 中精确存在的 `content/posts|projects/<slug>.md`。该命令不读取网络、不修改 `reviewedAt`、不保存文件、不提交也不推送；它和 Studio 队列、每周 Actions 复用同一维护规则。
...
保存笔记后先运行“MyBlog Publisher: 检查当前正式内容复核”。MyBlog Publisher 1.5.0 会要求当前分支为 `main`、文件已被 Git 跟踪、暂存区为空、没有未跟踪文件，且唯一工作区修改就是当前 `content/posts|projects/<slug>.md`；随后复算 HEAD 与当前 frontmatter/正文差异并执行完整 `npm run check`。成功打开只读 Author Proof：日期迁移轨显示 HEAD/当前 reviewedAt，账本显示事实变化、updatedAt、质量门、干净 Git 边界和唯一可提交路径；Modal 没有同步按钮，并明确尚未暂存、提交或推送。确认后运行“提交并同步当前正式内容复核”，它再次执行同样的前置条件和门禁，只暂存该 Markdown，以 `content: review <slug>` 提交并推送 `origin main`。
...
`--format json` 只允许和 check-only 组合；它在完整门通过后输出 `version: 1` 的 review/git/qualityGate 证据，质量门日志不会混入 stdout。插件对字段、日期关系、kind/slug/path、main、changed/committable 唯一路径、空 staged/untracked 与 `npm run check: passed` 做 exact schema 验证；JSON、schema、活动路径或 Modal 异常会重新运行纯文本检查。由于日期精度为一天，已经在当天复核过的内容不能再次声明新的复核；同日后续事实修正仍可用普通 Git 流程提交，但不要伪造第二次复核。全量门失败时文件保持未暂存；commit 前失败会取消暂存。若 commit 成功而 push 失败，本地提交会保留，联网恢复后执行 `git push origin main`，不要重新运行复核命令制造重复提交。
