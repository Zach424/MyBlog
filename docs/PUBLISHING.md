...
Studio 在正文工具栏选择“参考资料清单”，填写清单标题，再增删或拖动资料条目。Obsidian 可从命令面板运行“插入参考资料清单模板”，也可以在任意 Markdown 编辑器中直接写：
...
每组必须有 1–120 字符标题和 2–12 条资料，每篇最多 3 组且总计不超过 24 条。每条必须先写一个 1–160 字符的可见 Markdown 链接；目标只接受完整 HTTPS URL 或 `/` 开头的站内绝对路径，不能含凭据、非默认端口、空白、协议相对形式或未编码圆括号。可选短注使用精确的空格、破折号、空格 ` — ` 分隔，最多 240 字符；同一组不能重复目标。

名称应让读者脱离上下文也能理解，不要写“点这里”。构建不会抓取远程标题、摘要或 favicon，也不会因外站瞬时超时改变内容；需要观察外链健康时单独运行 `npm run links:external -- --check`。站内路径继续参与目标/标题锚点校验和知识关系，HTTPS 目标继续进入离线外链库存。网页显示域名或“本站”，打印会保留显式 URL。

### 发布操作步骤流程

Studio 在正文工具栏选择“操作步骤流程”，填写流程标题，再增删或拖动步骤。Obsidian 可从命令面板运行“插入操作步骤流程模板”，也可以直接写开放 Markdown：
...
每组标题为 1–120 字符，包含 2–10 步；每篇最多 3 组且总计不超过 24 步。有序列表必须从 1 开始。每步第一段必须只含 1–100 字符的粗体步骤名，第二段为 1–600 字符的操作说明；可选第三段以精确 `**验证：**` 开头，正文为 1–240 字符。同一流程不能有重复步骤名。

说明和验证允许文本、行内代码/公式、简单强调和链接；不要放图片、HTML、脚注、硬换行、嵌套列表、复选框或额外段落。流程表达固定执行次序，不保存读者完成状态；需要展示项目当前完成/待完成时继续使用“项目任务清单”。网页用 Runbook Path 展示编号与 CHECK，打印按步骤自然分页，搜索会保留实际说明与验证正文。

### 发布术语定义表

Studio 在正文工具栏选择“术语定义表”，填写标题，再增删或拖动术语。Obsidian 可从命令面板运行“插入术语定义表模板”，也可以直接写开放 Markdown：
...
每组标题为 1–120 字符，包含 2–12 个术语；每篇最多 3 组且总计不超过 24 个。每条第一段必须只含 1–100 字符的粗体术语，第二段为 1–800 字符的定义。可选的 `**别名：**` 使用中文顿号分隔 1–5 个纯文本别名，每个 1–60 字符；可选的 `**上下文：**` 最多 400 字符，并位于别名之后。

定义和上下文允许文本、行内代码/公式、简单强调和链接；不要放图片、HTML、脚注、硬换行、嵌套列表、复选框或额外段落。同一组的术语和所有别名不能重复。网页使用语义 `<dl>` Definition Ledger，搜索保留术语、定义、别名和上下文。它是本文的静态知识，不会自动查词典、翻译或跨文章合并。

### 发布常见问题 FAQ

Studio 在正文工具栏选择“常见问题 FAQ”，填写标题，再增删或拖动问题与答案段落。Obsidian 可从命令面板运行“插入常见问题 FAQ 模板”，也可以直接写开放 Markdown：
...
每组标题为 1–120 字符，包含 2–10 个问题；每篇最多 3 组且总计不超过 24 个。每条第一段必须只含 1–160 字符的粗体问题，随后填写 1–3 个答案段落；每段 1–600 字符，每题全部答案合计不超过 1,200 字符。同一组的问题不能重复。

答案允许文本、行内代码/公式、简单强调和链接；不要放图片、HTML、脚注、硬换行、嵌套列表、复选框或额外段落。网页第一题默认展开，其余问题使用原生 `<details>/<summary>` 独立开合；展开状态不会写回 Git 或跨访问保存。搜索和打印始终保留全部答案。

### 发布项目文件树

Studio 在正文工具栏选择“项目文件树”，填写标题，再按文档顺序添加完整相对路径和节点说明。Obsidian 可从命令面板运行“插入项目文件树模板”，也可以直接写开放 Markdown：
...
每棵树包含 2–32 个节点、最多 4 层；每篇最多 3 棵且合计不超过 64 个节点。每行必须使用行内代码包住单个路径段，并用精确的 ` — ` 接 1–240 字符说明。文件夹必须以 `/` 结尾，只有文件夹可以拥有子节点；完整路径在同一棵树中不能重复。

说明允许文本、行内代码/公式、简单强调和链接；不要放图片、HTML、脚注、硬换行、任务 checkbox 或额外段落。文件树用于解释与正文相关的仓库切片，不会实时读取 GitHub、浏览文件内容、保存折叠状态或编辑文件。仓库结构变化后由作者更新正文并重新复核。

### 发布项目里程碑时间线

Studio 在正文工具栏选择“项目里程碑时间线”，填写标题，再按日期添加、选择类型或拖动事件。Obsidian 可从命令面板运行“插入项目里程碑时间线模板”，也可以直接写开放 Markdown：
...
每条时间线标题为 1–120 字符，包含 2–16 个事件；每篇最多 3 条且合计不超过 32 个事件。事件类型只允许 `START`、`DECISION`、`SHIP`、`CHANGE`、`VERIFY`、`RETIRE`；日期必须是真实 `YYYY-MM-DD`，从早到晚排列且不能晚于发布检查日。同日事件可以按作者顺序保留，但同日同标题不能重复。说明为 1–600 字符，允许文本、行内代码/公式、强调、删除与链接；不要放图片、HTML、脚注、硬换行、嵌套列表、任务状态或额外段落。时间线只记录已经发生的历史，不承担未来计划、提醒或项目管理同步。

### 发布技术决策记录

Studio 在正文工具栏选择“技术决策记录”，填写状态、日期、上下文、决定与理由，再添加备选方案和影响。Obsidian 可从命令面板运行“插入技术决策记录模板”，也可以直接写开放 Markdown：

```markdown
> [!decision] 选择 Vercel 作为部署平台
> **STATUS:** `ACCEPTED` · **DATE:** `2026-08-12`
>
> **CONTEXT**
>
> 博客使用 Next.js，需要零服务器维护并保持 Git 主分支自动部署。
>
> **DECISION**
>
> 使用 Vercel 承载生产站，不在当前链路中引入 Cloudflare。
>
> **RATIONALE**
>
> 原生 Next.js 运行时和现有 GitHub 流程能减少适配层与日常维护。
>
> **ALTERNATIVES**
>
> - **Cloudflare Pages** — 需要额外适配当前 Next.js 能力与发布配置。
>
> **CONSEQUENCES**
>
> - `POSITIVE` 作者发布流程保持为 GitHub main 到 Vercel。
> - `NEGATIVE` 平台能力与回滚范围继续受 Vercel 套餐约束。
```

每条记录标题为 1–120 字符，状态只允许 `ACCEPTED`、`SUPERSEDED`、`DEPRECATED`、`REJECTED`，日期必须是真实 `YYYY-MM-DD` 且不能晚于发布检查日。上下文、决定和理由各为一个 1–800 字符段落；备选方案 1–6 个，名称 1–120 字符、说明 1–400 字符且 NFKC 后不能重复；影响 1–6 个，类型只允许 `POSITIVE`、`NEGATIVE`、`NEUTRAL`，说明 1–400 字符。每篇最多 3 条记录，全部备选与影响合计不超过 24 项。

叙述与说明允许文本、行内代码/公式、简单强调、删除与链接；不要放图片、HTML、脚注、硬换行、嵌套列表、任务状态或额外段落。区段名称与顺序必须保持一致。状态表示已经发布的事实，不是审批按钮；第一版不提供投票、评论、读者编辑、自动建议、远程工单同步或“被哪条决定替代”的自动关系。

### 发布技术实验记录

Studio 在正文工具栏选择“技术实验记录”，填写结果状态、运行日期、假设、环境、方法、样本和结论，再添加测量与局限。Obsidian 可从命令面板运行“插入技术实验记录模板”，也可以直接写开放 Markdown：
...
> **METHOD**
>
> 运行一次 `npm run release:check` 并保存最终结果。
>
> **SAMPLE**
>
...
每条记录标题为 1–120 字符，状态只允许 `SUPPORTED`、`REFUTED`、`INCONCLUSIVE`、`FAILED`，日期必须是真实 `YYYY-MM-DD` 且不能晚于发布检查日。假设、环境、方法、样本和结论各为一个 1–800 字符段落；测量 1–8 个，指标名 1–120 字符、行内代码值 1–80 字符、说明 1–400 字符；局限 1–6 个，名称 1–120 字符、说明 1–400 字符。测量名和局限名分别在 NFKC 后不能重复；每篇最多 3 条实验，全部测量与局限合计不超过 30 项。
