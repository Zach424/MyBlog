...
> **日期：** 2026-08-12
>
> **上下文：** 博客使用 Next.js，需要零服务器维护并保持 Git 主分支自动部署。
>
> **决定：** 使用 Vercel 承载生产站，不在当前链路中引入 Cloudflare。
>
> **理由：** 原生 Next.js 运行时和现有 GitHub 流程能减少适配层与日常维护。
>
> **备选方案：**
...
12. MyBlog Publisher 1.52.0 会先确认运行代码、runtime manifest 和磁盘插件版本一致，验证三个插件文件的 bundle 摘要，并证明 bundle 与三个目标文件均由 Git HEAD 跟踪、index 相等且 worktree 未修改；推送成功后再校验 Git 交付证据、释放可能存在的写事务并完成 Vault reconcile，自动等待该篇正式内容上线。push 失败后使用两条“重新同步待交付…”恢复成功也会自动接力。只在使用网页或普通 Git 时，才手动打开正式笔记运行“等待当前正式内容上线”。需要核对全库时运行“检查生产内容同步状态”。
