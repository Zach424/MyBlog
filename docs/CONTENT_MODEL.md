...
受约束流程使用顶层块引用 marker 和从 1 开始的有序列表；每个 list item 使用两到三个段落：
...
流程标题为 1–120 字符；每组 2–10 步、每篇最多 3 组且总计不超过 24 步。步骤名必须是第一段唯一的粗体内容，限制 1–100 字符；第二段说明限制 1–600 字符；可选第三段必须精确以 `**验证：**` 开头，其余正文限制 1–240 字符。同一流程的步骤名经 NFKC 与中文小写规范化后不能重复。

说明和验证只接受文本、行内代码/公式、简单强调、链接和链接引用；图片、HTML、脚注、硬换行、嵌套列表、任务 checkbox 与额外段落失败关闭。流程必须位于正文顶层，有序列表必须从 1 开始。该结构表达静态执行顺序，不存储完成状态、负责人、期限或提醒；状态快照继续由独立 `[!tasks]` 任务账本表达。

阅读端将同一结构投影为带命名标题的语义 `<section>` 和 `<ol>`，视觉为 Runbook Path；搜索保留标题、步骤名、说明和验证正文，删除 marker 与验证标签噪声。内容构建、Studio 条目预检与共享生产预览复用同一权威校验器。

## 术语定义表

受约束术语表使用顶层块引用 marker 和无序列表；每个 list item 使用两到四个段落：
...
术语表标题为 1–120 字符；每组 2–12 条、每篇最多 3 组且总计不超过 24 条。术语必须是第一段唯一的粗体内容，限制 1–100 字符；定义限制 1–800 字符。可选别名段必须精确以 `**别名：**` 开头，使用中文顿号分隔 1–5 个 1–60 字符的纯文本别名；可选上下文段必须精确以 `**上下文：**` 开头，其余正文限制 1–400 字符。别名必须位于上下文之前。

术语与所有别名在同一组内共用 NFKC + `zh-CN` 小写规范化命名空间，不能重复。定义和上下文只接受文本、行内代码/公式、简单强调、链接和链接引用；图片、HTML、脚注、硬换行、嵌套列表、任务 checkbox 与额外段落失败关闭。术语表必须位于正文顶层。

阅读端将同一结构投影为带标题的语义 `<section>` 和 `<dl>/<dt>/<dd>`，视觉为 Definition Ledger；搜索保留标题、术语、定义、别名和上下文，删除 marker 与元数据标签噪声。内容构建、Studio 条目预检与共享生产预览复用同一权威校验器。该结构不抓取远程词典、不自动翻译、不建立跨文章知识数据库。

## 常见问题 FAQ

受约束 FAQ 使用顶层块引用 marker 和无序列表；每个 list item 的第一段是仅粗体问题，随后是一个到三个答案段落：
...
FAQ 标题为 1–120 字符；每组 2–10 问、每篇最多 3 组且合计不超过 24 问。问题必须是第一段唯一的粗体内容，限制 1–160 字符；每个答案段落限制 1–600 字符，每题 1–3 段且总计不超过 1,200 字符。同组问题经 NFKC + `zh-CN` 小写规范化后不能重复。

答案只接受文本、行内代码/公式、简单强调、链接和链接引用；图片、HTML、脚注、硬换行、嵌套列表、任务 checkbox 与额外段落失败关闭。`[!faq]+`/`[!faq]-` 不代表合法 FAQ；FAQ 必须位于正文顶层，专用解析优先于通用 Callout。

阅读端将同一结构投影为带标题的语义 `<section>` 和原生 `<details>/<summary>` Answer Cabinet；首题默认展开，其余独立开合，不保存客户端状态。搜索保留标题、问题和答案并删除 marker；打印强制显示全部答案。内容构建、Studio 条目预检与共享生产预览复用同一权威校验器。

## 项目文件树

受约束项目文件树使用顶层块引用 marker 和递归无序列表；每个 list item 只含一行路径段与职责，可选子列表只允许挂在文件夹节点下：
...
标题为 1–120 字符；每棵树 2–32 个节点、最多 4 层；每篇最多 3 棵且合计不超过 64 个节点。节点必须写成 `` `路径段` — 说明 ``。路径段为 1–79 字符，不能含空白、`/`、`\`、反引号、控制字符，也不能是 `.` 或 `..`；文件夹只通过末尾 `/` 声明。完整路径由祖先和当前路径段派生，经 NFKC + `zh-CN` 小写规范化后不能重复。

只有文件夹可以拥有子列表。说明限制 1–240 字符，只接受文本、行内代码/公式、简单强调、链接和链接引用；图片、HTML、脚注、硬换行、任务 checkbox、额外段落和有序列表失败关闭。`[!filetree]+`/`[!filetree]-` 不代表合法文件树；文件树必须位于正文顶层，专用解析优先于通用 Callout。

阅读端将同一结构投影为带标题的语义 `<section>` 和嵌套 `<ul>/<li>`，视觉为 Repository Slice；搜索保留标题、路径名和职责，删除 marker、目录末尾 `/` 与视觉分隔符噪声。内容构建、Studio 条目预检与共享生产预览复用同一权威校验器。它不读取远程仓库、不自动扫描目录、不保存客户端折叠状态，也不提供文件浏览或编辑。

## 项目里程碑时间线

受约束项目时间线使用顶层块引用 marker 和无序列表；每个 list item 第一段保存日期、类型和粗体标题，随后恰好一段说明：
...
标题为 1–120 字符；每条时间线 2–16 个事件；每篇最多 3 条且合计不超过 32 个事件。日期必须是真实 `YYYY-MM-DD` 且非递减，类型只允许 `START / DECISION / SHIP / CHANGE / VERIFY / RETIRE`。同日事件保留作者顺序，但同日且 NFKC + `zh-CN` 小写标题相同的事件不能重复。仓库构建与 Studio 草稿预检都拒绝晚于当前上海内容日期的事件。

事件标题限制 1–120 字符，只接受文本和行内代码；说明限制 1–600 字符，只接受文本、行内代码/公式、简单强调、删除、链接和链接引用。图片、HTML、脚注、硬换行、任务 checkbox、嵌套列表、额外段落和有序列表失败关闭。`[!timeline]+`/`[!timeline]-` 不代表合法时间线；时间线必须位于正文顶层，专用解析优先于通用 Callout。

阅读端将同一结构投影为带标题的语义 `<section>`、原生 `<ol>/<li>` 与 `<time datetime>`，视觉为 Release Tape；搜索保留标题、日期、事件标题和说明，删除 marker 与事件类型噪声。内容构建、Studio 条目预检与共享生产预览复用同一权威校验器。它只记录已发生历史，不保存未来计划、提醒、完成状态或外部项目管理数据。
