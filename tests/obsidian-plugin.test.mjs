...
import { getMarkdownHttpIssue } from "../lib/markdown-http.ts";
...
function authorDoctorReport({
  ...
  const provenanceReady = provenanceStatus === "verified";
  const provenanceFiles = [
    ...
    const oid = String.fromCharCode("b".charCodeAt(0) + index).repeat(40);
    const changed = provenanceStatus === "changed" && name === "styles.css";
    return {
      headBlobOid: oid,
      indexBlobOid: oid,
      indexStatus: "clean",
      ...
      present: true,
      status: changed ? "unverified" : "verified",
      worktreeStatus: changed ? "changed" : "clean",
    };
  });
  const checkDefinitions = [
    ...
    [
      ...
      "Plugin provenance",
      provenanceReady
        ? `HEAD ${"a".repeat(12)} · 4/4 tracked clean`
        : `HEAD ${"a".repeat(12)} · 3/4 tracked clean · styles.css worktree changed`,
      "4 plugin bundle paths tracked and equal to HEAD/index",
    ],
  ];
  const scripts = [
  ...
  return {
    ...
    mode: "read-only",
    status:
      bundleReady && provenanceReady ? "ready" : "needs-attention",
    observation: {
    ...
    summary: {
      attention: Number(!bundleReady) + Number(!provenanceReady),
      passed: 15 - Number(!bundleReady) - Number(!provenanceReady),
      total: 15,
    },
    checks: checkDefinitions.map(([id, group, label, observed, expected]) => {
      const bundleFailed = id === "publisher-bundle" && !bundleReady;
      const provenanceFailed =
        id === "publisher-provenance" && !provenanceReady;
      return {
        expected,
        group,
        id,
        label,
        observed,
        resolution: bundleFailed
          ? "运行 npm run plugin:bundle -- --write 后重新加载插件"
          : provenanceFailed
            ? "恢复四个插件 bundle 路径与 Git HEAD/index 一致后重载插件"
            : null,
        status: bundleFailed || provenanceFailed ? "attention" : "pass",
      };
    }),
    safety: {
...
function authorDoctorAttentionReport() {
  ...
  report.summary = { attention: 1, passed: 14, total: 15 };
...
test("inserts a slug-aware portable gallery template without running Git or network commands", async () => {
  const harness = await createPluginHarness({
    activeFilePath: "content/inbox/gallery-draft.md",
    files: ["content/inbox/gallery-draft.md"],
  });
  const command = findCommand(harness, "insert-gallery-template");
  let inserted = "";
  command.editorCallback(
    { replaceSelection(value) { inserted = value; } },
    { file: { path: "content/inbox/gallery-draft.md" } },
  );

  assert.match(inserted, /^> \[!gallery\] /u);
  assert.equal((inserted.match(/^> - !\[/gmu) ?? []).length, 2);
  assert.match(inserted, /\(\/uploads\/gallery-1\.png "第一帧短标题"\)/u);
  assert.equal(harness.spawned.length, 0);
  assert.match(harness.notices.at(-1)?.message ?? "", /已插入两帧画廊模板/u);

  command.editorCallback(
    { replaceSelection(value) { inserted = value; } },
    { file: { path: "content/projects/myblog.md" } },
  );
  assert.match(inserted, /\(\/uploads\/myblog\/gallery-1\.png/u);
});

test("inserts a portable constrained technical table without running Git or network commands", async () => {
  const harness = await createPluginHarness({
    activeFilePath: "content/inbox/table-draft.md",
    files: ["content/inbox/table-draft.md"],
  });
  const command = findCommand(harness, "insert-table-template");
  let inserted = "";
  command.editorCallback(
    { replaceSelection(value) { inserted = value; } },
    { file: { path: "content/inbox/table-draft.md" } },
  );

  assert.match(inserted, /^> \[!table\] /u);
  assert.match(inserted, /^> \| 环境 \| P50 \| P95 \|$/mu);
  assert.match(inserted, /^> \| --- \| ---: \| ---: \|$/mu);
  assert.equal((inserted.match(/^> \|/gmu) ?? []).length, 4);
  assert.equal(harness.spawned.length, 0);
  assert.match(harness.notices.at(-1)?.message ?? "", /已插入三列技术表格模板/u);
...
test("inserts a portable read-only task list without running Git or network commands", async () => {
  const harness = await createPluginHarness({
    activeFilePath: "content/inbox/task-draft.md",
    files: ["content/inbox/task-draft.md"],
  });
  const command = findCommand(harness, "insert-task-list-template");
  let inserted = "";
  command.editorCallback(
    { replaceSelection(value) { inserted = value; } },
    { file: { path: "content/inbox/task-draft.md" } },
  );

  assert.match(inserted, /^> \[!tasks\] 发布准备$/mu);
  assert.equal((inserted.match(/^> - \[[ x]\]/gmu) ?? []).length, 3);
  assert.match(inserted, /^> - \[x\] 已完成的步骤$/mu);
  assert.equal(harness.spawned.length, 0);
  assert.match(harness.notices.at(-1)?.message ?? "", /已插入三项只读任务清单模板/u);
...
test("inserts a transcript-first local audio note without running Git or network commands", async () => {
  const harness = await createPluginHarness({
    activeFilePath: "content/inbox/audio-draft.md",
    files: ["content/inbox/audio-draft.md"],
  });
  const command = findCommand(harness, "insert-audio-template");
  let inserted = "";

  command.editorCallback(
    { replaceSelection(value) { inserted = value; } },
    { file: { path: "content/inbox/audio-draft.md" } },
  );

  assert.match(inserted, /^> \[!audio\] 说明这段录音的主题$/mu);
  assert.match(inserted, /> \[下载 MP3\]\(\/uploads\/audio-note\.mp3 "说明这段录音的主题"\)/u);
  assert.match(inserted, /> \*\*文字稿\*\*/u);
  assert.match(inserted, /等价的完整文字内容/u);
  assert.equal(harness.spawned.length, 0);
});

test("inserts a portable reference list without running Git or network commands", async () => {
  const harness = await createPluginHarness({
    activeFilePath: "content/inbox/references-draft.md",
    files: ["content/inbox/references-draft.md"],
  });
  const command = findCommand(harness, "insert-references-template");
  let inserted = "";

  command.editorCallback(
    { replaceSelection(value) { inserted = value; } },
    { file: { path: "content/inbox/references-draft.md" } },
  );

  assert.match(inserted, /^> \[!references\] 延伸阅读$/mu);
  assert.equal((inserted.match(/^> \d+\. \[/gmu) ?? []).length, 2);
  assert.match(inserted, /\[官方文档标题\]\(https:\/\/example\.com\/docs\)/u);
  assert.match(inserted, /\[本站相关记录\]\(\/projects\/myblog\)/u);
  assert.equal(harness.spawned.length, 0);
  assert.match(harness.notices.at(-1)?.message ?? "", /已插入两条参考资料模板/u);
...
test("inserts a portable ordered procedure without running Git or network commands", async () => {
  const harness = await createPluginHarness({
    activeFilePath: "content/inbox/procedure-draft.md",
    files: ["content/inbox/procedure-draft.md"],
  });
  const command = findCommand(harness, "insert-steps-template");
  let inserted = "";

  command.editorCallback(
    { replaceSelection(value) { inserted = value; } },
    { file: { path: "content/inbox/procedure-draft.md" } },
  );

  assert.match(inserted, /^> \[!steps\] 发布流程$/mu);
  assert.equal((inserted.match(/^> \d+\. \*\*/gmu) ?? []).length, 2);
  assert.equal((inserted.match(/^> {4}\*\*验证：\*\*/gmu) ?? []).length, 2);
  assert.match(inserted, /执行 `npm run release:check`/u);
  assert.match(inserted, /将已审阅提交推送到 `main`/u);
  assert.equal(harness.spawned.length, 0);
  assert.match(harness.notices.at(-1)?.message ?? "", /已插入两步操作流程模板/u);
...
test("inserts a portable glossary without running Git or network commands", async () => {
  const harness = await createPluginHarness({
    activeFilePath: "content/inbox/glossary-draft.md",
    files: ["content/inbox/glossary-draft.md"],
  });
  const command = findCommand(harness, "insert-glossary-template");
  let inserted = "";

  command.editorCallback(
    { replaceSelection(value) { inserted = value; } },
    { file: { path: "content/inbox/glossary-draft.md" } },
  );

  assert.match(inserted, /^> \[!glossary\] React 核心概念$/mu);
  assert.equal((inserted.match(/^> - \*\*/gmu) ?? []).length, 2);
  assert.equal((inserted.match(/^> {3}\*\*别名：\*\*/gmu) ?? []).length, 2);
  assert.equal((inserted.match(/^> {3}\*\*上下文：\*\*/gmu) ?? []).length, 2);
  assert.match(inserted, /Server Component/u);
  assert.match(inserted, /React 在已有服务端 HTML 上绑定客户端行为/u);
  assert.equal(harness.spawned.length, 0);
  assert.match(harness.notices.at(-1)?.message ?? "", /已插入两条术语定义模板/u);
...
test("inserts a portable FAQ without running Git or network commands", async () => {
  const harness = await createPluginHarness({
    activeFilePath: "content/inbox/faq-draft.md",
    files: ["content/inbox/faq-draft.md"],
  });
  const command = findCommand(harness, "insert-faq-template");
  let inserted = "";

  command.editorCallback(
    { replaceSelection(value) { inserted = value; } },
    { file: { path: "content/inbox/faq-draft.md" } },
  );

  assert.match(inserted, /^> \[!faq\] 发布常见问题$/mu);
  assert.equal((inserted.match(/^> - \*\*/gmu) ?? []).length, 2);
  assert.match(inserted, /应该使用 Studio 还是 Obsidian/u);
  assert.match(inserted, /不会跨访问保存/u);
  assert.equal(harness.spawned.length, 0);
  assert.match(harness.notices.at(-1)?.message ?? "", /已插入两条 FAQ 模板/u);
...
test("inserts a portable project file tree without running Git or network commands", async () => {
  const harness = await createPluginHarness({
    activeFilePath: "content/inbox/filetree-draft.md",
    files: ["content/inbox/filetree-draft.md"],
  });
  const command = findCommand(harness, "insert-filetree-template");
  let inserted = "";

  command.editorCallback(
    { replaceSelection(value) { inserted = value; } },
    { file: { path: "content/inbox/filetree-draft.md" } },
  );

  assert.match(inserted, /^> \[!filetree\] MyBlog 核心结构$/mu);
  assert.equal((inserted.match(/^> (?: {2})*- `/gmu) ?? []).length, 5);
  assert.match(inserted, /^> {3}- `studio\/`/mu);
  assert.match(inserted, /^> {5}- `page\.tsx`/mu);
  assert.equal(harness.spawned.length, 0);
  assert.match(harness.notices.at(-1)?.message ?? "", /已插入五个项目文件树节点/u);
...
test("inserts a portable project timeline without running Git or network commands", async () => {
  const harness = await createPluginHarness({
    activeFilePath: "content/inbox/timeline-draft.md",
    files: ["content/inbox/timeline-draft.md"],
  });
  const command = findCommand(harness, "insert-timeline-template");
  let inserted = "";

  command.editorCallback(
    { replaceSelection(value) { inserted = value; } },
    { file: { path: "content/inbox/timeline-draft.md" } },
  );

  assert.match(inserted, /^> \[!timeline\] MyBlog 交付里程碑$/mu);
  assert.equal((inserted.match(/^> - `\d{4}-\d{2}-\d{2}` `/gmu) ?? []).length, 3);
  assert.match(inserted, /`START` \*\*建立内容契约\*\*/u);
  assert.match(inserted, /`VERIFY` \*\*完成生产验证\*\*/u);
  assert.equal(harness.spawned.length, 0);
  assert.match(harness.notices.at(-1)?.message ?? "", /已插入三个项目里程碑/u);
...
test("inserts a portable technical decision without running Git or network commands", async () => {
  const harness = await createPluginHarness({
    activeFilePath: "content/inbox/decision-draft.md",
    files: ["content/inbox/decision-draft.md"],
  });
  const command = findCommand(harness, "insert-decision-template");
  let inserted = "";

  command.editorCallback(
    { replaceSelection(value) { inserted = value; } },
    { file: { path: "content/inbox/decision-draft.md" } },
  );

  assert.match(inserted, /^> \[!decision\] 选择 Vercel 原生托管$/mu);
  assert.match(inserted, /^> \*\*STATUS:\*\* `ACCEPTED` · \*\*DATE:\*\* `2026-08-12`$/mu);
  assert.match(inserted, /^> \*\*ALTERNATIVES\*\*$/mu);
  assert.match(inserted, /^> - `NEGATIVE` 托管能力与 Vercel 平台耦合。$/mu);
  assert.equal(harness.spawned.length, 0);
  assert.match(harness.notices.at(-1)?.message ?? "", /已插入技术决策记录/u);
...
test("inserts a portable technical experiment without running Git or network commands", async () => {
  const harness = await createPluginHarness({
    activeFilePath: "content/inbox/experiment-draft.md",
    files: ["content/inbox/experiment-draft.md"],
  });
  const command = findCommand(harness, "insert-experiment-template");
  let inserted = "";

  command.editorCallback(
    { replaceSelection(value) { inserted = value; } },
    { file: { path: "content/inbox/experiment-draft.md" } },
  );

  assert.match(inserted, /^> \[!experiment\] 验证博客完整发布门耗时$/mu);
  assert.match(inserted, /^> \*\*STATUS:\*\* `SUPPORTED` · \*\*DATE:\*\* `2026-08-12`$/mu);
  assert.match(inserted, /^> \*\*MEASUREMENTS\*\*$/mu);
  assert.match(inserted, /^> - \*\*单机范围\*\* — 结果只覆盖当前硬件、系统与依赖版本。$/mu);
  assert.equal(harness.spawned.length, 0);
  assert.match(harness.notices.at(-1)?.message ?? "", /已插入技术实验记录/u);
...
test("inserts portable redacted HTTP evidence without sending a request", async () => {
  const harness = await createPluginHarness({
    activeFilePath: "content/inbox/http-draft.md",
    files: ["content/inbox/http-draft.md"],
  });
  const command = findCommand(harness, "insert-http-template");
  let inserted = "";

  command.editorCallback(
    { replaceSelection(value) { inserted = value; } },
    { file: { path: "content/inbox/http-draft.md" } },
  );

  assert.match(inserted, /^> \[!http\] 创建草稿文章$/mu);
  assert.match(inserted, /^> \*\*METHOD:\*\* `POST` · \*\*STATUS:\*\* `201` · \*\*DATE:\*\* `2026-08-13`$/mu);
  assert.match(inserted, /^> \*\*REQUEST BODY:\*\* `json`$/mu);
  assert.match(inserted, /^> \*\*RESPONSE BODY:\*\* `json`$/mu);
  assert.equal(getMarkdownHttpIssue(inserted), undefined);
  assert.equal(harness.spawned.length, 0);
  assert.match(harness.notices.at(-1)?.message ?? "", /静态脱敏 HTTP 交换证据/u);
...
test("renders a local-only author preflight circuit without repairing it", async () => {
  ...
  assert.match(text, /15 PASS \/ 0 ATTENTION/u);
...
test("holds every Git writer on an explicit runtime and disk plugin version mismatch", async (t) => {
  ...
  await t.test("normal author transaction", async () => {
    ...
    assert.match(text, /RUNNING CODE.*1\.56\.0.*RUNTIME MANIFEST.*1\.40\.0.*DISK.*1\.56\.0/su);
...
test("keeps future disk patch and minor plugin versions structured for reload guidance", async (t) => {
  for (const diskVersion of ["1.56.1", "1.57.0"]) {
  ...
  await t.test("forged future version evidence", async () => {
    ...
    const report = authorDoctorReport({ pluginVersion: "1.56.0" });
...
test("fails recovery closed when the disk plugin version identity is unavailable", async () => {
  ...
  report.summary = { attention: 2, passed: 13, total: 15 };
  const publisher = report.checks.find(
    (check) => check.id === "publisher-plugin",
  );
  publisher.observed = "missing";
  publisher.resolution = "重新安装或启用 MyBlog Publisher 1.56.0";
...
test("holds normal and recovery Git writers on unverified plugin provenance", async (t) => {
  for (const commandId of [
    "publish-current-note",
    "deliver-pending-review",
  ]) {
    await t.test(commandId, async () => {
      ...
      harness.spawned[0].child.stdout.emit(
        "data",
        Buffer.from(
          JSON.stringify(authorDoctorReport({ provenanceStatus: "changed" })),
        ),
      );
      harness.spawned[0].child.emit("close", 1);
      ...
      const text = allElements(harness.modals[0].contentEl)
        .map((element) => element.text)
        .join(" ");
      assert.match(text, /PLUGIN PROVENANCE UNVERIFIED/u);
      assert.match(text, /styles\.css.*WORKTREE CHANGED/su);
      assert.match(text, /HEAD.*INDEX/su);
      assert.match(text, /Git add.*commit.*push.*fetch.*reset/su);
    });
  }
});
