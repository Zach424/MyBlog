...
export function hasPotentialStudioDecision(markdown) {
  let fenceCharacter = "";
  let fenceLength = 0;
  ...
  for (const sourceLine of textValue(markdown).split(/\r?\n/u)) {
    ...
    if (/^[ \t]*(?:>[ \t]*)+\[!decision\](?:[+\-]|[ \t]|$)/iu.test(sourceLine)) return true;
  }
  return false;
}

export function hasPotentialStudioExperiment(markdown) {
  let fenceCharacter = "";
  let fenceLength = 0;
  ...
  for (const sourceLine of textValue(markdown).split(/\r?\n/u)) {
    ...
    if (/^[ \t]*(?:>[ \t]*)+\[!experiment\](?:[+\-]|[ \t]|$)/iu.test(sourceLine)) return true;
  }
  return false;
}
...
export function hasPotentialStudioHttpExchange(markdown) {
  let fenceCharacter = "";
  let fenceLength = 0;
  ...
  for (const sourceLine of textValue(markdown).split(/\r?\n/u)) {
    ...
    if (/^[ \t]*(?:>[ \t]*)+\[!http\](?:[+\-]|[ \t]|$)/iu.test(sourceLine)) return true;
  }
  return false;
}
...
export function hasPotentialStudioRichMarkdown(markdown) {
  return (
    ...
    hasPotentialStudioHttpExchange(markdown) ||
  ...
  switch (state?.status) {
    ...
    case "ready": {
      ...
      if (state.decisionCount > 0) {
        evidence.push(
          `${state.decisionCount} 个技术决策 / ${state.decisionAlternativeCount} 个备选 / ${state.decisionConsequenceCount} 条影响`,
        );
      }
      if (state.experimentCount > 0) {
        evidence.push(
      ...
      if (state.httpExchangeCount > 0) {
        evidence.push(
          `${state.httpExchangeCount} 个 HTTP 交换 / ${state.httpExchangeHeaderCount} 个安全头字段 / ${state.httpExchangeBodyLineCount} 行正文`,
        );
      }
      if (state.videoCount > 0) evidence.push(`${state.videoCount} 段视频`);
      return {
        detail: "这里与正式页面共享 remark、rehype、受限 KaTeX、Callout、画廊、技术表格、只读任务清单、参考资料清单、步骤流程、术语定义表、FAQ、项目文件树、项目时间线、技术决策记录、技术实验记录、代码变更证据、HTTP 请求 / 响应证据、Mermaid、本地音频与本地视频渲染配置。",
    ...
    case "invalid": {
      ...
      const isHttpExchange = state.issue?.kind === "http";
      return {
        detail: `${line}${state.issue?.message || "请检查增强 Markdown 语法。"}`,
        label: isHttpExchange
  ...
  return createClass({
    ...
    getInitialState() {
      return {
        ...
        httpExchangeBodyLineCount: 0,
        httpExchangeCount: 0,
        httpExchangeHeaderCount: 0,
        diagramCount: 0,
        decisionAlternativeCount: 0,
        decisionConsequenceCount: 0,
        decisionCount: 0,
        experimentCount: 0,
        experimentLimitationCount: 0,
        experimentMeasurementCount: 0,
        faqCount: 0,
        faqQuestionCount: 0,
        fileTreeCount: 0,
        fileTreeMaxDepth: 0,
      ...
      try {
        ...
        if (result.ok) {
          this.setState({
            ...
            httpExchangeBodyLineCount: result.httpExchangeBodyLineCount,
            httpExchangeCount: result.httpExchangeCount,
            httpExchangeHeaderCount: result.httpExchangeHeaderCount,
            ...
            experimentLimitationCount: result.experimentLimitationCount,
            experimentMeasurementCount: result.experimentMeasurementCount,
