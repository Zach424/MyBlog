...
MyBlog Publisher 1.16.0 会在“检查当前草稿”“发布当前草稿并同步 GitHub”“检查当前正式内容复核”“提交并同步当前正式内容复核”真正启动前自动运行同一 JSON doctor，并用一个 single-flight lease 串行化四类新事务。租约从 doctor 启动跨越领域命令与诊断降级，直到成功、失败或 spawn error 才释放；占用期间再次调用只显示同一只读快照，不启动第二个进程链。运行“查看当前作者事务”可在 active/idle 两态查看操作、冻结路径、`preflight / domain / diagnostic` 阶段、ISO 开始时间和动态已运行时长；BUSY 使用完全相同的事实。attention 仍打开 `TRANSACTION INTERLOCK / HELD`；不可信 JSON 改读纯文本并失败关闭。状态命令不取消、不重试、不排队、不访问网络；其他只读状态、统一分诊和两类待交付重送也继续绕过租约，避免阻断正确恢复。
