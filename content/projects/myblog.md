...
下一轮优先补齐网页 Studio 的数学公式作者预览：生产阅读端与 Obsidian 已共享 `$...$` / `$$...$$`，Studio 也应在编辑时给出一致或明确说明差异的公式反馈，同时不建立第二份内容契约、外部 CDN 或全站客户端公式脚本。之后继续按维护手册发布真实记录并观察构建体积；自定义域名、公开邮箱、评论与统计保持可选。
