...
当前站点使用原生 Next.js、GitHub 自动 Production 和稳定域名冒烟；Studio GitHub OAuth、全字段发布预检、公式预览、媒体冲突保护与全库只读维护队列，Obsidian 模板/附件/封面/链接发布、收件箱就绪总览与已发布内容复核 Modal，构建期内容关系、文章与项目反向引用、Vercel 回滚与恢复均已验收。Iteration 0051 的本地完整发布门通过 155 项单元测试、19 项生产 HTTP/质量测试和 45 个页面生成任务，生产依赖审计为 0；插件行为测试覆盖 Windows/POSIX 命令、成功、spawn error、迟到 close、移动端禁用和卸载清理。项目保持 `maintained`，当前生产站面向公众访问且不依赖 Cloudflare。
...
下一轮把 Obsidian 的纯文本维护报告升级为结构化复核账本：使用受校验的 JSON 输出显示四级计数和逐条记录，并为仓库内 sourcePath 提供“一键打开笔记”，但保留纯文本降级、零网络、零自动改日期和零外部提醒。之后继续按维护手册发布真实记录并观察构建体积，自定义域名、公开邮箱、评论与统计保持可选。
