...
MyBlog 当前运行在 Vercel，稳定公开地址是 [blog-iota-five-59.vercel.app](https://blog-iota-five-59.vercel.app)。仓库使用原生 Next.js 16.3、React 19 和 TypeScript；GitHub `main` 自动触发 Production，质量门和独立线上冒烟共同验证交付。网页 Studio 与 Obsidian 都可以由作者独立发布；MyBlog Publisher 1.6.0 会在正式复核 Proof 中区分唯一提交路径和保留在本地的并行草稿/根新附件，代码、正式内容和未知路径仍阻断。
...
当前站使用原生 Next.js、GitHub 自动 Production 和稳定域名冒烟；Studio/Obsidian 双发布、内容维护、关系门与 Vercel 恢复均已验收。Iteration 0055 的完整门通过 182 项单元测试、19 项生产测试、45 个页面生成任务和零生产依赖漏洞；并行草稿保留、单文件远端提交、门后重分类和 intent-to-add 阻断都有真实 Git 行为证据。项目保持 `maintained`，生产站公开且不依赖 Cloudflare。
...
下一轮用 SHA-256 把正式复核的门前候选、门后文件、暂存 blob 和提交 tree 绑定为同一内容，并在 Proof 中显示短指纹；长检查期间再次编辑目标必须失败。自定义域名、公开邮箱、评论与统计保持可选。
