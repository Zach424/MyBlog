...
目前工程基线、内容契约、正式首页、响应式设计、深色偏好、Open Graph 卡片、内容校验管线、完整核心阅读路径、站内搜索与发布发现端点已经完成。页面、RSS、Sitemap、robots 和 404 都通过 Worker 集成测试。每一轮都有独立提交、测试结果和 `docs/iterations` 归档。线上部署尚未完成，因此项目状态仍然是 `building`，不会提前写成已上线。
...
进入上线候选验收：检查桌面与移动关键流程、键盘访问、颜色对比、HTML 语义、资源体积、缓存与安全响应头；通过后再部署到 Cloudflare。上线后根据真实发布体验评估评论与其他持久化能力。
