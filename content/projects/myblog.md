...
MyBlog 运行在 [Vercel](https://blog-iota-five-59.vercel.app)，使用 Next.js 16.3、React 19 与 TypeScript；GitHub `main` 自动交付 Production。Studio 和 Obsidian 均可自主发布；MyBlog Publisher 1.30.0 显示当前草稿的媒体变换、逐次替代文本与站内链接 trace，并可从 ALT 或 REF 标签安全定位到精确源码行。
...
当前草稿意图复用正式解析，不在插件中另写 Markdown/YAML 规则或读取图片。version 5 将媒体用途、来源行、最终替代文本及作者来源一一对应，也让每个精确站内目标保留逐次来源行。1.30.0 让 `ALT · L<n> · <SOURCE>` 与 `REF · L<n>` 都复用同一本地导航守卫；点击时重新核对冻结/活动路径、同一 Vault `TFile` 与磁盘/编辑器行界，成功才打开精确行，失败则保留摘要且不写入、不发布、不联网。
...
Studio/Obsidian 双发布、内容维护、关系门与 Vercel 恢复均已验收。Iteration 0079 把每次 LINK TRACE 引用也变为安全的精确源码行导航；完整门通过 354/354、45 页、19/19 与生产审计 0。生产站公开且不依赖 Cloudflare。
...
下一轮为 source-scoped readiness 增加草稿内容 SHA-256，并在显示摘要和定位前重新核对，阻止同一文件在报告后被编辑却仍使用旧行号；保持只读、零网络。
