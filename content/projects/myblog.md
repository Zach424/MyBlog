...
MyBlog 运行在 [Vercel](https://blog-iota-five-59.vercel.app)，使用 Next.js 16.3、React 19 与 TypeScript；GitHub `main` 自动交付 Production。网页 Studio 和 Obsidian 均可自主发布；MyBlog Publisher 1.25.0 为当前草稿显示正式验证过的精确媒体变换与链接 trace，同时只派生目标媒体并保留全库共享证据。
...
当前草稿意图复用正式解析，不在插件中解析 Markdown/YAML 或读取图片。version 2 报告继续从同一次链接验证循环返回目标、fragment、源码行和重复次数；1.25.0 再严格消费已有 attachment preparation，以 `MEDIA TRACE` 显示 source/target/public URL、输入输出格式/尺寸/帧数/字节和 optimized/preserved/unproven 状态。报告仍轻量解析全草稿并应用共享/目标问题，只为当前来源生成媒体候选；证据或活动文件漂移时失败关闭。
...
Studio/Obsidian 双发布、内容维护、关系门与 Vercel 恢复均已验收。Iteration 0074 增加精确媒体来源、目标、公开 URL、输入/输出包络与优化状态；完整门通过 321/321、45 页、19/19 与生产审计 0。生产站公开且不依赖 Cloudflare。
...
下一轮为媒体 trace 补充 COVER/BODY 角色、出现次数与源码行；云 API、评论、统计、自动发布与自动修复保持范围外。
