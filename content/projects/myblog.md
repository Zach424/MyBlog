...
目前工程基线、内容契约、正式首页、响应式设计、深色偏好、Open Graph 卡片、内容校验管线和完整核心阅读路径已经完成。文章、项目、专题、标签与关于页面都通过 Worker 服务端渲染测试。每一轮都有独立提交、测试结果和 `docs/iterations` 归档。线上部署尚未完成，因此项目状态仍然是 `building`，不会提前写成已上线。
...
继续完成站内搜索、RSS、Sitemap 和 robots 端点；随后进行浏览器验收、可访问性与性能检查，再部署到 Cloudflare。上线后再根据真实发布体验评估评论与其他持久化能力。
